...
export default function OPDCasualBillPaymentPage() {
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API}/casual-bill-payments?from_date=${fromDate}&to_date=${toDate}`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 0 }}>
      <div className={styles.toolbar} style={{ flexWrap: "wrap", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)" }}>OPD Casual Bill Payments</span>
      </div>
      ...
      <div className={styles.sectionBox} style={{ padding: 0, flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={12} style={{ padding: "40px 0", textAlign: "center", color: "var(--text-secondary)" }}>
                      No casual bill payment records found.
                    </td>
                  </tr>
                ) : (
                  filtered.map(r => {
                    const selected = selectedRow?.ObpCode === r.ObpCode;
                    return (
                      <tr key={r.ObpCode} onClick={() => setSelectedRow(r)}
        ...
        {filtered.length > 0 && (
          <div style={{ padding: "8px 16px", borderTop: "2px solid var(--border-light)", background: "var(--bg-secondary)", display: "flex", justifyContent: "flex-end", gap: 24, fontSize: 13, fontWeight: 700 }}>
            ...
            <span style={{ color: "var(--status-success)" }}>Total Settled: ₹{grandTotal.toFixed(2)}</span>
          </div>
        )}
      </div>
    </div>
