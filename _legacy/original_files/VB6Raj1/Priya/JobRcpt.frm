...
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsJRC As clsPmtMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
...
Dim fcmbPrtName As clsPmtFlexSearch, fcmbSimName As clsPmtFlexSearch
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .Open "Select * from JobRcpt where JrcCode=" & CStr(mJrcCode) & " and JrcICode=" & CStr(mJrcICode) & "", dbPmtDatabase, adOpenKeyset, adLockOptimistic
...
Set clsJRC = New clsPmtMultiTranEntry
Set clsJRC.dbAcDatabase = dbAcDatabase
Set clsJRC.dbStDatabase = dbStDatabase
Set clsJRC.dbPmtDatabase = dbPmtDatabase
...
fcmbVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cPMT_VTYPE_JOB_RCPT) & " order by VtmName"
fcmbVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.Open "Select * from JobRcHdr inner join PartyMast on JobRcHdr.JrhPrtCode=PartyMast.PrtCode where JrhCode=" & CStr(mJrcCode) & " order by JrhDate,JrhTime", dbPmtDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from JobRcHdr inner join PartyMast on JobRcHdr.JrhPrtCode=PartyMast.PrtCode order by JrhDate,JrhTime", dbPmtDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
    ...
    If mJrcVchNo <> mJrcVchNo_old Or Val(fcmbVtmName.BoundText) <> mJrcVtmCode_old Or Ctod(dtpJrcDate.Text) <> mJrcDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbPmtDatabase, dbAcDatabase, mTableName:="JobRcHdr", mChkFieldName:="JrhVchNo", mChkFieldValue:=mJrcVchNo, mKeyFieldName:="JrhCode", mKeyFieldValue:=mJrcCode, mVtmFieldName:="JrhVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="JrhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="JrhDate", mDateFieldValue:=Ctod(dtpJrcDate.Text)) = True Then
            ...
            If FormAddEditMode = cFORM_ADDMODE Then
                mJrcVchNo = GetNextVTypeNo(dbPmtDatabase, dbAcDatabase, dbComDatabase, mTableName:="JobRcHdr", mFieldName:="JrhVchNo", mVtmFieldName:="JrhVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="JrhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="JrhDate", mDateFieldValue:=Ctod(dtpJrcDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
...
With tRecset
.Open "Select top 1 * from JobRcHdr order by JrhCode desc", dbPmtDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
Else
    ...
    With t1Recset
    .Open "Select top 1 * from VTypMast where VtmSysCode=" & CStr(cPMT_VTYPE_JOB_RCPT) & "", dbAcDatabase, adOpenKeyset, adLockOptimistic
    If .EOF = False Then
...
With tRecset
.Open "Select * from JobRcpt where JrcCode=" & CStr(mJrcCode) & " order by JrcSno", dbPmtDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbPmtDatabase.Execute "Update JobRcHdr set JrhStkCode=" & CStr(mJrcStkCode) & " where JrhCode=" & CStr(mJrcCode)
            dbPmtDatabase.Execute "Update JobRcpt set JrcStkCode=" & CStr(mJrcStkCode) & " where JrcCode=" & CStr(mJrcCode)
