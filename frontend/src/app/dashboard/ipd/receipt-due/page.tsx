...
export default function IPDReceiptDuePage() {
  ...
  const [selectedRow, setSelectedRow] = useState<PaymentRecord | null>(null);
  ...
  const loadPayments = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/receipt-payments?from_date=${fromDate}&to_date=${toDate}`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API}/receipt-payments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          IphIpgCode: selectedPat.IhdCode,
          IphPttCode: selectedPat.PttCode,
          IphDepoAmt: payAmt,
          IphRemark:  `Mode: ${payMode}. ${remark}`.trim(),
        }),
      });

      if (!res.ok) throw new Error("Saving payment failed");
      showToast("Receipt payment collected successfully!");
      setShowModal(false);
      loadPayments();
    } catch {
  ...
  const totCollected = payments.reduce((s, r) => s + (r.DepoAmt || 0), 0);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 110px)", gap: 10 }}>
      ...
      {showModal && (
        <div className={styles.modalOverlay} style={{ zIndex: 1000 }}>
          <div className={styles.modalContent} style={{ maxWidth: 440 }}>
            <div className={styles.modalHeader}>
              <h3 style={{ fontSize: 16, fontWeight: 700 }}>Collect Receipt Due Payment</h3>
      ...
      <div className={styles.sectionBox} style={{ padding: 0, flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: "center" }}>
              ...
              <span style={{ color: "var(--text-secondary)" }}>Loading payments...</span>
            </div>
          ) : filteredPayments.length === 0 ? (
            <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)" }}>
              No receipt payment records found.
            </div>
          ) : (
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1200, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredPayments.map(p => {
                  const selected = selectedRow?.IphCode === p.IphCode;
                  return (
                    <tr
                      key={p.IphCode}
                    ...
                    >
                      <td>{p.Date}</td>
                      <td style={{ color: "var(--text-secondary)", fontSize: 11 }}>{p.Time || "—"}</td>
                      <td>Receipt Due Payment</td>
                      <td>
                        <strong style={{ color: "var(--accent-color)" }}>#{p.VchNo}</strong>
                      </td>
                      <td>
                        <strong>{p.PttName}</strong>
                      </td>
                      <td>{p.PttRegNo || "—"}</td>
                      <td>{p.Scheme || "—"}</td>
                      <td>{p.PymtMode}</td>
                      ...
                      <td style={{ fontSize: 11, color: "var(--text-secondary)" }}>{p.Remark || "—"}</td>
                      <td>{p.CareOf || "—"}</td>
                      <td>{p.Address || "—"}</td>
                    </tr>
                  );
        ...
        {!loading && filteredPayments.length > 0 && (
          <div style={{
            ...
            <div>GRAND TOTAL</div>
            <div style={{ textAlign: "right" }}>₹{totCollected.toFixed(2)}</div>
            <div></div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        {[
          ...
          { text: "Refresh List (F5)", color: "#f9e6ff", onClick: loadPayments },
