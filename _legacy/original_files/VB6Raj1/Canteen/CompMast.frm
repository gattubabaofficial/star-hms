...
If InList(KeyCode, Array(vbKeyDelete, vbKeySubtract)) And mCmpCode > 0 Then
    ErrorBox "Financial Year Deletion not Supported !!!"
    Exit Sub
...
If FormAddEditMode = cFORM_ADDMODE Then
    ...
    CreateCtnCmpDatabase mAddMode:=True, FormProgStatus:=MyProgStatForm
    CreateCtnMastDatabase mAddMode:=True, FormProgStatus:=MyProgStatForm
    CreateCtnTranDatabase mAddMode:=False, FormProgStatus:=MyProgStatForm    ' already created just above
...
ErrorBox "Company Deletion not Supported !!!"
Exit Sub
...
clsListStru.AddFields mExpr:="CmpName", mTitle:="Company Name", mAlign:=1, mWidth:=4000, mShowItem:=True
