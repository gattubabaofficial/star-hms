...
Dim dbAcDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbPmtDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsJIS As clsPmtMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
Dim clsPRT As clsPmtMasterEntry, clsSIM As clsPmtMasterEntry
Dim mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbVtmName As clsComFlexSearch
Dim fcmbPrtName As clsPmtFlexSearch, fcmbSimName As clsPmtFlexSearch, fcmbSimRefNo As clsPmtFlexSearch
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    RefreshDatabase dbPmtDatabase
    ChkAnyExcessIsuExist mJisICode
    ChkAnyMisLinkedRcExist mJisICode
    
    With tRecset
    .Open "Select * from JobIssu where JisCode=" & CStr(mJisCode) & " and JisICode=" & CStr(mJisICode) & "", dbPmtDatabase, adOpenKeyset, adLockOptimistic
...
OpenPmtDataSource dbPmtDatabase
...
Set clsJIS = New clsPmtMultiTranEntry
Set clsJIS.dbAcDatabase = dbAcDatabase
Set clsJIS.dbStDatabase = dbStDatabase
Set clsJIS.dbPmtDatabase = dbPmtDatabase
clsJIS.blnJobIssueTran = True
clsJIS.Init

Set clsPRT = New clsPmtMasterEntry
Set clsPRT.dbAcDatabase = dbAcDatabase
Set clsPRT.dbPmtDatabase = dbPmtDatabase
clsPRT.blnPartyMast = True
clsPRT.Init

Set clsSIM = New clsPmtMasterEntry
Set clsSIM.dbPmtDatabase = dbPmtDatabase
...
fcmbVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cPMT_VTYPE_JOB_ISSUE) & " order by VtmName"
fcmbVtmName.Init

Set fcmbSimName = New clsPmtFlexSearch
Set fcmbSimName.dbPmtDatabase = dbPmtDatabase
Set fcmbSimName.dbComDatabase = dbComDatabase
fcmbSimName.blnSubItmMastList = True
fcmbSimName.Init

Set fcmbSimRefNo = New clsPmtFlexSearch
Set fcmbSimRefNo.dbPmtDatabase = dbPmtDatabase
Set fcmbSimRefNo.dbComDatabase = dbComDatabase
fcmbSimRefNo.mFiltCond = "True=False"
fcmbSimRefNo.blnBalSimRefNoList = True
fcmbSimRefNo.Init

Set fcmbPrtName = New clsPmtFlexSearch
Set fcmbPrtName.dbPmtDatabase = dbPmtDatabase
...
If mRemoteAccess = True Then
    datRecset.Open "Select * from JobIsHdr inner join PartyMast on JobIsHdr.JihPrtCode=PartyMast.PrtCode where JihCode=" & CStr(mJisCode) & " order by JihDate,JihTime", dbPmtDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from JobIsHdr inner join PartyMast on JobIsHdr.JihPrtCode=PartyMast.PrtCode order by JihDate,JihTime", dbPmtDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
Else
    If FormAddEditMode = cFORM_ADDMODE Or dtlAddMode = True Or Val(fcmbSimName.BoundText) <> mJisSimCode_old Or Val(mskJisItmCode.Text) <> mJisItmCode_old Or Val(fcmbSimRefNo.BoundText) <> mJisItrCode_old Then
        With tRecset
        .Open "Select * from JobRcpt jrc inner join JobRcHdr jrh on jrc.JrcCode=jrh.JrhCode where jrc.JrcPrtCode=" & CStr(Val(fcmbPrtName.BoundText)) & " and jrc.JrcSimCode=" & CStr(Val(fcmbSimName.BoundText)) & " and jrc.JrcItmCode=" & CStr(Val(mskJisItmCode.Text)) & " and jrc.JrcItrCode=" & CStr(Val(fcmbSimRefNo.BoundText)) & "", dbPmtDatabase, adOpenKeyset, adLockOptimistic
...
With tRecset
.Open "Select top 1 * from JobIsHdr order by JihCode desc", dbPmtDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
Else
    ...
    With t1Recset
    .Open "Select top 1 * from VTypMast where VtmSysCode=" & CStr(cPMT_VTYPE_JOB_ISSUE) & "", dbAcDatabase, adOpenKeyset, adLockOptimistic
    If .EOF = False Then
...
With tRecset
.Open "Select * from JobIssu where JisCode=" & CStr(mJisCode) & " order by JisSno", dbPmtDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbPmtDatabase.Execute "Update JobIsHdr set JihStkCode=" & CStr(mJisStkCode) & " where JihCode=" & CStr(mJisCode)
            dbPmtDatabase.Execute "Update JobIssu set JisStkCode=" & CStr(mJisStkCode) & " where JisCode=" & CStr(mJisCode)
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPartywise.Value = True Then
    If optMonthwise.Value = True Then
        ...
        datRecset.Open "Select format(JihDate,'yyyy/MM') as JisYrMonth" _
         ...
         , dbPmtDatabase, adOpenKeyset, adLockReadOnly
...
.Open "Select top 1 * from ((JobRcpt" _
    ...
    , dbPmtDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
