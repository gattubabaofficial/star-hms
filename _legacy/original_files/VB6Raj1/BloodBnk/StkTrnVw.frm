...
Dim dbAcDatabase As New ADODB.Connection, dbMdsDatabase As New ADODB.Connection, dbBdkDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim fcmbPrtName As clsBdkFlexSearch, fcmbSimName As clsBdkFlexSearch
Dim frmTrn As Form
    ...
    If optMonthwise.Value = True Then
        ...
        If chkFByPurch.Value = vbChecked Then
            ...
            mQryStr = mQryStr & " Select PurDate as TrnDate" _
             ...
             & " inner join (MedPurchHdr inner join VTypMast on MedPurchHdr.PuhVtmCode = VTypMast.VtmCode) on MedPurchDtl.PurCode = MedPurchHdr.PuhCode" _
             & " where PurDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
             & " and PurCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODDONATE) _
             & IIf(mQryPrtCode > 0, " and PurPrtCode = " & CStr(mQryPrtCode), "") _
             & IIf(mQrySimCode > 0, " and PurSimCode =" & CStr(mQrySimCode), "")
        End If
        Rem sales return
        If chkFByRetnInw.Value = vbChecked Then
            ...
            mQryStr = mQryStr & " Select PurDate as TrnDate" _
             ...
             & " inner join (MedPurchHdr inner join VTypMast on MedPurchHdr.PuhVtmCode = VTypMast.VtmCode) on MedPurchDtl.PurCode = MedPurchHdr.PuhCode" _
             & " where PurDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
             & " and PurCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODFILTER) _
             & IIf(mQryPrtCode > 0, " and PurPrtCode = " & CStr(mQryPrtCode), "") _
             & IIf(mQrySimCode > 0, " and PurSimCode =" & CStr(mQrySimCode), "")
        End If
        Rem sales ---------------------
        If chkFBySale.Value = vbChecked = True Then
            ...
            mQryStr = mQryStr & " Select SalDate as TrnDate" _
             ...
             & " and SalCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODISSUED) _
        ...
        If chkFByRetnOutw.Value = vbChecked Then
            ...
            mQryStr = mQryStr & " Select SalDate as TrnDate" _
             ...
             & " inner join (MedSaleHdr inner join VTypMast on MedSaleHdr.SahVtmCode = VTypMast.VtmCode) on MedSaleDtl.SalCode = MedSaleHdr.SahCode" _
             & " where SalDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
             & " and SalCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODCONS) _
             & IIf(mQryPrtCode > 0, " and SalPrtCode = " & CStr(mQryPrtCode), "") _
             & IIf(mQrySimCode > 0, " and SalSimCode = " & CStr(mQrySimCode), "")
        End If
        Rem ----------------------------------
...
ElseIf optDetailed.Value = True Then    ''' ------------------------------------------------------------------------
    ...
    If chkFByPurch.Value = vbChecked Then
        ...
        mQryStr = mQryStr & " Select " _
         ...
         & " inner join VTypMast on MedPurchHdr.PuhVtmCode = VTypMast.VtmCode" _
         & " where PuhDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and PuhCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODDONATE) _
         & IIf(mQryPrtCode > 0, " and PuhPrtCode = " & CStr(mQryPrtCode), "")
    End If
    Rem sales return
    If chkFByRetnInw.Value = vbChecked Then
        ...
        mQryStr = mQryStr & " Select " _
         ...
         & " inner join VTypMast on MedPurchHdr.PuhVtmCode = VTypMast.VtmCode" _
         & " where PuhDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and PuhCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODFILTER) _
         & IIf(mQryPrtCode > 0, " and PuhPrtCode = " & CStr(mQryPrtCode), "")
    End If
    Rem sales ---------------------
    If chkFBySale.Value = vbChecked = True Then
        ...
        mQryStr = mQryStr & " Select " _
         ...
         & " and SahCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODISSUED) _
    ...
    If chkFByRetnOutw.Value = vbChecked Then
        ...
        mQryStr = mQryStr & " Select " _
         ...
         & " inner join VTypMast on MedSaleHdr.SahVtmCode = VTypMast.VtmCode" _
         & " where SahDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and SahCode > 0 and VtmSysCode = " & CStr(cBDK_VTYPE_BLOODCONS) _
         & IIf(mQryPrtCode > 0, " and SahPrtCode = " & CStr(mQryPrtCode), "")
    End If
    Rem ----------------------------------
    mQryStr = mQryStr & ") as t1"
