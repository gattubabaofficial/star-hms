...
Dim clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
...
If mRemoteAccess = True Then
    'datRecset.Open "Select * from MedPurchDtl left join MedPrtMast on MedPurchDtl.PurMptCode=MedPrtMast.MptCode where PurCode=" & CStr(mPurCode) & " order by PurDate,OpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    'datRecset.Open "Select * from MedPurchDtl left join MedPrtMast on MedPurchDtl.PurMptCode=MedPrtMast.MptCode order by PurDate,OpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPartyNamewise.Value = True Or optItemWise.Value = True Or optVTypewise.Value = True Then
    ...
    ElseIf optPartyNamewise.Value = True Then
        clsListStru.AddFields mExpr:="PartyName", mTitle:="Party Name", mShowItem:=True, mAlign:=1, mWidth:=3000
        clsListStru.AddFields mExpr:="TrnCount", mTitle:="Count", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="TrnQty_sum", mTitle:="Qty", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="TrnAmount_sum", mTitle:="Amount", mShowItem:=True, mAlign:=7, mWidth:=1500
    ...
    ElseIf optItemWise.Value = True Then
        clsListStru.AddFields mExpr:="TrnMdiName", mTitle:="Item Name", mShowItem:=True, mAlign:=1, mWidth:=3000
        clsListStru.AddFields mExpr:="TrnCount", mTitle:="Count", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="TrnQty_sum", mTitle:="Qty", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="TrnAmount_sum", mTitle:="Amount", mShowItem:=True, mAlign:=7, mWidth:=1500
    ...
    ElseIf optVTypewise.Value = True Then
        ...
        mQryStr = mQryStr & "Select PuhVtmCode as TrnVtmCode" _
         ...
         & " where PurDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and PurCode > 0 and True = False" _
         & IIf(mQryMptCode > 0, " and PuhVtmCode = " & CStr(mQryMptCode), "") _
         & IIf(mQryPttCode > 0, " and PurPttCode = " & CStr(mQryPttCode), "") _
         & IIf(mQryMdiCode > 0, " and PurMdiCode = " & CStr(mQryMdiCode), "") _
         & IIf(mQryVtmCode > 0, " and PuhVtmCode = " & CStr(mQryVtmCode), "")
        Rem opening stock
        ...
        If chkFBySale.Value = vbChecked = True Then
            ...
            mQryStr = mQryStr & " Select SahVtmCode as TrnVtmCode" _
             ...
             & " where SalDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
             & " and SalCode > 0 and VtmSysCode = " & CStr(cVGN_VTYPE_MEDSALE) _
             & IIf(mQryMptCode > 0, " and SahVtmCode = " & CStr(mQryMptCode), "") _
             & IIf(mQryPttCode > 0, " and SalPttCode = " & CStr(mQryPttCode), "") _
             & IIf(mQryMdiCode > 0, " and SalMdiCode = " & CStr(mQryMdiCode), "") _
             & IIf(mQryVtmCode > 0, " and SahVtmCode = " & CStr(mQryVtmCode), "")
        End If
...
ElseIf optExpanded.Value = True Then
    ...
    clsListStru.AddFields mExpr:="PartyName", mTitle:="Party Name", mAlign:=1, mWidth:=2500, mShowItem:=True
    ' --------------------------------------
...
ElseIf optDetailed.Value = True Then    ''' ------------------------------------------------------------------------
    ...
    If chkFBySale.Value = vbChecked = True Then
        ...
        mQryStr = mQryStr & " Select " _
         ...
         & IIf(mQryMptCode > 0, " and SahPttCode = " & CStr(mQryMptCode), "") _
