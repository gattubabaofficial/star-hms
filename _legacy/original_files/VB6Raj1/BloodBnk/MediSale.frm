...
Dim dbAcDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbBdkDatabase As New ADODB.Connection, dbMdsDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim mSalSimCode_old As Long, mSalItmCode_old As Long, mSalItrCode_old As Long, mSalPurCode_old As Long, mSalPurICode_old As Long, mSalAmtBefDisc_old As Double, mSalDiscPer_old As Double, mSalAmtBefTax_old As Double, mSalTaxPer_old As Double
Dim mSahAmount_old As Double
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsSAL As clsMdsMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
Dim clsPRT As clsBdkMasterEntry, clsSIM As clsBdkMasterEntry
Dim mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbVtmName As clsComFlexSearch
Dim fcmbPrtName As clsBdkFlexSearch, fcmbSimName As clsBdkFlexSearch, fcmbSimRefNo As clsBdkFlexSearch
Dim mPartyMastTblName As String, mPrtNameTitle As String, mPrtNameField As String, mPrtCodeField As String, mSahPrtCodeField As String, mPrtAraCodeField As String, mPrtStnCodeField As String, mPrtAddrField As String, mPrtTelNoField As String, mPrtSmsNoField As String
        ...
        If .EOF = False And .BOF = False Then
            ...
            Else
                ...
                ElseIf optPartywise.Value = True Then
                    mQryPrtCode = .fields(mSahPrtCodeField)
                    fcmbPrtName.BoundText = CStr(mQryPrtCode)
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    Else
        .mSalPttCode_lng = Val(fcmbPrtName.BoundText)
    End If
    ...
    .open "Select * from MedSaleDtl" _
        ...
        & " on MedSaleDtl.SalPurCode = MedPurchDtl.PurCode and MedSaleDtl.SalPurICode = MedPurchDtl.PurICode and MedSaleDtl.SalSimCode = MedPurchDtl.PurSimCode and MedSaleDtl.SalItmCode = MedPurchDtl.PurItmCode and MedSaleDtl.SalItrCode = MedPurchDtl.PurItrCode" _
        & " where SalCode=" & CStr(mSalCode) _
        & " and SalICode=" & CStr(mSalICode) _
        & " order by SalSno", dbMdsDatabase, adOpenKeyset, adLockOptimistic
...
Set clsSAL = New clsMdsMultiTranEntry
Set clsSAL.dbAcDatabase = dbAcDatabase
Set clsSAL.dbStDatabase = dbStDatabase
Set clsSAL.dbBdkDatabase = dbBdkDatabase
Set clsSAL.dbMdsDatabase = dbMdsDatabase
...
If mRemoteAccess = True Then
    datRecset.open "Select * from MedSaleHdr" _
        & " inner join " & mPartyMastTblName & " prt on MedSaleHdr." & mSahPrtCodeField & " = prt." & mPrtCodeField & "" _
...
With tRecset
.open "Select * from (" & mPartyMastTblName & " prt inner join AreaMast ara on prt." & mPrtAraCodeField & " = ara.AraCode) inner join StsnMast stn on prt." & mPrtStnCodeField & " = stn.StnCode where prt." & mPrtCodeField & " = " & CStr(mSalPrtCode) & "", dbComDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
If mFormSysVchType = cCOM_VTYPE_RETNOUTW Then
    fcmbPrtName.BoundText = CStr(.mSahPrtCode_lng)
Else
    fcmbPrtName.BoundText = CStr(.mSahPttCode_lng)
End If
txtPrtName.Text = fcmbPrtName.Text
...
With tRecset
.open "Select top 1 * from MedSaleHdr where SahVtmCode IN (" & mFormVchTypeList & ") and SahCmpCode = " & CStr(sFinYrCmpCode) & " order by SahCode desc", dbMdsDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
Else
    .mSahPttCode_lng = Val(fcmbPrtName.BoundText)
End If
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbMdsDatabase.Execute "Update MedSaleHdr set SahStkCode=" & CStr(mSalStkCode) & " where SahCode=" & CStr(mSalCode)
            dbMdsDatabase.Execute "Update MedSaleDtl set SalStkCode=" & CStr(mSalStkCode) & " where SalCode=" & CStr(mSalCode)
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPartywise.Value = True Then
    If optMonthwise.Value = True Then
        ...
        datRecset.open "Select format(SahDate,'yyyy/MM') as SalYrMonth" _
         ...
         & IIf(mQryPrtCode > 0, " and " & mSahPrtCodeField & " = " & CStr(mQryPrtCode), "") _
         & " group by format(SahDate,'yyyy/MM')" _
         & " order by format(SahDate,'yyyy/MM')" _
         , dbMdsDatabase, adOpenKeyset, adLockReadOnly
    ...
    ElseIf optPartywise.Value = True Then
        ...
        datRecset.open "Select prt." & mPrtNameField & ",MedSaleHdr." & mSahPrtCodeField & "" _
         & ",Count(SahCode) as SalCount" _
         & ",Sum(SahQty) as SalQty_sum" _
         & ",Sum(SahAmount) as SalAmount_sum" _
         & " from MedSaleHdr inner join " & mPartyMastTblName & " prt on MedSaleHdr." & mSahPrtCodeField & " = prt." & mPrtCodeField & "" _
         & " where SahDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and SahCode > 0 and SahVtmCode IN (" & mFormVchTypeList & ")" _
         & IIf(mQryPrtCode > 0, " and " & mSahPrtCodeField & " = " & CStr(mQryPrtCode), "") _
         & " group by prt." & mPrtNameField & ",MedSaleHdr." & mSahPrtCodeField & "" _
         & " order by prt." & mPrtNameField & ",MedSaleHdr." & mSahPrtCodeField & "" _
...
ElseIf optDetailed.Value = True Then
    ...
    datRecset.open "Select MedSaleHdr.*,prt.*" _
...
If .RecordCount > 0 Then
    Do While .EOF = False
        ...
        ElseIf optDetailed.Value = True Then
            clsARA.GetData .fields(mPrtAraCodeField)
            clsSTN.GetData .fields(mPrtStnCodeField)
...
Else    ' system field search has been requested by programmer
    ...
    ElseIf optPartywise.Value = True Then
        SearchFlex Mfgrd1, clsListStru.GetItemIndex(mPrtNameField), mText
    End If
End If
...
With tRecset
'If FormAddEditMode = cFORM_ADDMODE Then
'    .Open "Select top 1 sah.*,vtm.VtmName from MedSaleHdr sah inner join VTypMast vtm on sah.SahVtmCode=vtm.VtmCode where sah.SahPrtCode=" & CStr(mSalPrtCode) & " order by sah.SahDate Desc", dbComDatabase, adOpenKeyset, adLockOptimistic
'Else
    .open "Select top 1 sah.*,vtm.VtmName from MedSaleHdr sah" _
        & " inner join VTypMast vtm on sah.SahVtmCode = vtm.VtmCode" _
        & " where sah." & mSahPrtCodeField & " = " & CStr(mSalPrtCode) _
...
srow = Mfgrd2.Row
mDataOk = (Val(fcmbPrtName.BoundText) > 0 And Val(UnMyNumFmt(mskSalSno.Text)) > 0 And Val(fcmbSimName.BoundText) > 0 And Val(mskSalItmCode.Text) > 0 And Val(fcmbSimRefNo.BoundText) > 0 And Val(mskSalPurCode.Text) > 0 And Val(mskSalPurICode.Text) > 0 And Val(UnMyNumFmt(mskSalQty.Text)) > 0 And Val(UnMyNumFmt(mskSalRate.Text)) >= 0 And Val(UnMyNumFmt(mskSalDiscAmt.Text)) >= 0 And Val(UnMyNumFmt(mskSalTaxAmt.Text)) >= 0 And Val(UnMyNumFmt(mskSalAmount.Text)) >= 0)
...
.open "Select top 1 * from (" _
    ...
    & " Union All " _
    & "Select sal.SalPurICode as PurICode,sal.SalSimCode as SimCode,sal.SalItmCode as ItmCode,sal.SalItrCode as ItrCode,sal.SalSimRefNo as SimRefNo,(sal.SalQty*-1) as Qty from ((MedSaleDtl" _
    & " left join MedSaleHdr on MedSaleDtl.SalCode = MedSaleHdr.SahCode)" _
