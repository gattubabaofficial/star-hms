...
export default function IPDBillListPage() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const data: BillRow[] = await res.json();
      
      // Filter out only Final Bills
      const bills = data.filter(r => r.BillType === "Final Bill");
      setRows(bills);
    } catch (e) {
      showToast("Failed to load bill invoices from server.", false);
    } finally {
      setLoading(false);
    }
  ...
  function handlePrint() {
    if (!selected) { showToast("Select an invoice to print.", false); return; }
    router.push(`/dashboard/ipd/bill/${selected}/print`);
  }

  function handleExport() {
    const header = ["Invoice No","Date","Voucher","IPD No","Patient","UHID","Doctor","Gross","Discount","Net","Status"];
    const csvRows = [
      header.join(","),
      ...filtered.map(r => [
        r.ChargeNo, r.Date, r.Voucher, r.IpdNo ?? "",
        `"${r.PttName}"`, r.PttRegNo ?? "", `"${r.DctName}"`,
        r.GrossAmt, r.DiscAmt, r.NetAmt, r.Status,
      ].join(",")),
    ];
    const blob = new Blob([csvRows.join("\n")], { type: "text/csv;charset=utf-8;" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a"); a.href = url;
    a.download = `ipd_final_bills_${today}.csv`; a.click();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div className={styles.modalContent} style={{ maxWidth: 420 }}>
            ...
            <div className={styles.modalBody}>
              <p style={{ color: "var(--text-secondary)", fontSize: 14, lineHeight: 1.6 }}>
                Are you sure you want to delete <strong>Invoice #{selectedRow?.ChargeNo}</strong> for
                &nbsp;<strong>{selectedRow?.PttName}</strong>?
                <br />This will revert the bill calculations. This action cannot be undone.
              </p>
            </div>
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: "var(--text-primary)", margin: 0 }}>
            IPD Invoices & Final Stays
          </h2>
          <p style={{ color: "var(--text-secondary)", fontSize: 13, marginTop: 3 }}>
            Patient stay settlement invoices, billing breakdowns, and check-out ledger summaries.
          </p>
        </div>
        <span style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {[
          { label: "Billed Patients", value: filtered.length, icon: <FileText size={20} />, color: "var(--accent-color)", bg: "var(--accent-light)" },
          { label: "Gross Total Billed", value: fmtAmt(totalGross), icon: <IndianRupee size={20} />, color: "#7c3aed", bg: "#ede9fe" },
          { label: "Net Total Billed",   value: fmtAmt(totalNet),   icon: <IndianRupee size={20} />, color: "var(--status-success)", bg: "var(--status-success-light)" },
        ].map(k => (
      ...
      <div className={styles.sectionBox} style={{ padding: 0, overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div className={styles.searchBar} style={{ maxWidth: 280, flex: 1 }}>
            ...
            <input
              id="bill-search"
              type="text"
              placeholder="Search patient, invoice number, IPD #…"
              value={search}
        ...
        <div style={{
          ...
          <button
            ...
            onClick={() => router.push("/dashboard/ipd/bill/new")}
          >
          ...
          <button
            ...
            disabled={!selected}
            onClick={() => selected && router.push(`/dashboard/ipd/bill/${selected}/edit`)}
          >
            <Pencil size={15} /> Edit
          </button>
          ...
          {selected && (
            <span style={{ marginLeft: "auto", fontSize: 12, color: "var(--text-secondary)", fontStyle: "italic" }}>
              Selected: Invoice #{selectedRow?.ChargeNo} — {selectedRow?.PttName}
            </span>
          )}
        </div>
        ...
        {loading ? (
          <div style={{ padding: 60, textAlign: "center" }}>
            ...
            <p style={{ marginTop: 12, color: "var(--text-secondary)", fontSize: 14 }}>Loading invoices…</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 60, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "var(--text-secondary)" }}>
              Change the search query or date range, or click Create Invoice.
            </p>
          </div>
        ) : (
          <>
                ...
                <tbody>
                  {filtered.map(r => (
                    <tr
                      ...
                      onDoubleClick={() => router.push(`/dashboard/ipd/bill/${r.IbhCode}/edit`)}
                      style={{
                        cursor: "pointer",
            ...
            <div style={{
              ...
              <span>
                Net Total Billed: <span style={{ color: "var(--status-success)", fontSize: 15 }}>{fmtAmt(totalNet)}</span>
              </span>
            </div>
          </>
        )}
