...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsVgnSnglTranEntry, clsPAT As clsVgnMasterEntry, clsPCG As clsVgnMasterEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsSRV As clsVgnMasterEntry    ''' , clsVGNOPT As clsVgnOpdOptionsEntry
...
EntryDate = Ctod(dtpOpgDate.Text)
End Property
...
'clsVGNOPT.blnOutdRegOptMast = True
'clsVGNOPT.Init
'clsVGNOPT.GetOutdRegOptData
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdReg inner join PatMast on OutdReg.OpgPttCode = PatMast.PttCode where OpgCode=" & CStr(mOpgCode) & " order by OpgDate,OpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("OpgDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("OpgDate"))
    AddSmryParaLayer mClear:=True
Else
    datRecset.open "Select * from OutdReg inner join PatMast on OutdReg.OpgPttCode = PatMast.PttCode order by OpgDate,OpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
'        If FormAddEditMode = cFORM_ADDMODE And mPttRegNo <> mPttRegNo_old And clsVGNOPT.mOpgPttRegNoAddModeChange_bln = False Then
'            AlertBox "Change Prohibited in Add Mode !!!"
'            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
'            mskPttRegNo.SetFocus
'            GoTo EndSub
'
'        ElseIf FormAddEditMode = cFORM_EDITMODE And mPttRegNo <> mPttRegNo_old And clsVGNOPT.mOpgPttRegNoEditModeChange_bln = False Then
'            AlertBox "Change Prohibited in Edit Mode !!!"
'            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
'            mskPttRegNo.SetFocus
'            GoTo EndSub
'
'        ElseIf mPttRegNo = 0 And clsVGNOPT.mOpgPttRegNoNotZero_bln = True Then
'            AlertBox "Prohibited Zero Patient Reg.No. !!!"
'            If FormAddEditMode = cFORM_ADDMODE Then
'                If clsVGNOPT.mOpgPttRegNoAutoNumber_bln = True Then
...
'            GoTo EndSub
        ...
        If mPttRegNo <> 0 Then
            If ChkIsDuplicate(dbVgnDatabase, "PatMast", "PttRegNo", mPttRegNo, "PttCode", mOpgPttCode) = True Then
...
If IsDate(dtpPttDob.Text) = False Then
    ...
    GoTo EndSub
End If
If Val(fcmbAraName.BoundText) < 1 Then
    ErrorBox "Invalid Selection !!!"
    txtAraName.SetFocus
    GoTo EndSub
End If
If Val(fcmbStnName.BoundText) < 1 Then
    ErrorBox "Invalid Selection !!!"
    txtAraName.SetFocus
    GoTo EndSub
End If
If Val(fcmbPcgName.BoundText) < 1 Then
    ErrorBox "Invalid Selection !!!"
    txtPcgName.SetFocus
    GoTo EndSub
End If
If Val(UnMyPerFmt(mskPttDiscPer.Text)) < 0 Then
    ...
    GoTo EndSub
End If
If Val(fcmbAhName.BoundText) < 1 Then
    ...
    If mOpgVchNo <> mOpgVchNo_old Or Val(fcmbVtmName.BoundText) <> mOpgVtmCode_old Or Ctod(dtpOpgDate.Text) <> mOpgDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="OutdReg", mChkFieldName:="OpgVchNo", mChkFieldValue:=mOpgVchNo, mKeyFieldName:="OpgCode", mKeyFieldValue:=mOpgCode, mVtmFieldName:="OpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="OpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OpgDate", mDateFieldValue:=Ctod(dtpOpgDate.Text)) = True Then
            ...
            If FormAddEditMode = True Then
                mOpgVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="OutdReg", mFieldName:="OpgVchNo", mVtmFieldName:="OpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="OpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OpgDate", mDateFieldValue:=Ctod(dtpOpgDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormOpgAcsPermFileName)
            Else
...
If Val(fcmbCDctName.BoundText) < 1 Then
    ...
    GoTo EndSub
End If
'If Val(fcmbRByName.BoundText) < 1 Then
'    ErrorBox "Invalid Selection !!!"
'    txtRByName.SetFocus
'    GoTo EndSub
'End If
    ...
    If Index = 0 Then
        If mOpgVchNo <> mOpgVchNo_old Or Val(fcmbVtmName.BoundText) <> mOpgVtmCode_old Or Ctod(dtpOpgDate.Text) <> mOpgDate_old Or FormAddEditMode = cFORM_ADDMODE Then
            If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="OutdReg", mChkFieldName:="OpgVchNo", mChkFieldValue:=CStr(mOpgVchNo), mKeyFieldName:="OpgCode", mKeyFieldValue:=mOpgCode, mVtmFieldName:="OpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="OpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OpgDate", mDateFieldValue:=Ctod(dtpOpgDate.Text)) = True Then
                ...
                mskOpgVchNo(Index).Text = ToMyNumFmt(mOpgVchNo, mDecimals:=0)
            Rem Else
            Rem     GetNextVTypeNo dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="OutdReg", mFieldName:="OpgVchNo", mVtmFieldName:="OpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="OpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OpgDate", mDateFieldValue:=Ctod(dtpOpgDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormOpgAcsPermFileName, mDeletedVchNo:=mOpgVchNo
            End If
        End If
        ...
        End If
    Else
        If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="OutdReg", mChkFieldName:="OpgVchNo", mChkFieldValue:=CStr(mOpgVchNo), mVtmFieldName:="OpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="OpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
If Val(UnMyNumFmt(mskOpgUnit.Text)) < 1 Then
    ErrorBox "Invalid Input !!!"
    mskOpgUnit.SetFocus
    Cancel = True
Else
...
'fcmbAraName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgAraCode"))
'fcmbStnName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgStnCode"))
'fcmbPcgName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgPcgCode"))
'fcmbAhName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgPttAhCode"))
...
'fcmbVtmName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgVtmCode"))
'fcmbCDctName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgCDctCode"))
'fcmbRByName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgRByCode"))
'fcmbFDigName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgFDigCode"))
'fcmbSrvName.BoundText = CStr(clsVGNOPT.GetOutdRegOptBoundCode("OpgSrvCode"))
...
If Val(fcmbVtmName.BoundText) = 0 Or Val(fcmbCDctName.BoundText) = 0 Or Val(fcmbRByName.BoundText) = 0 Or Val(fcmbFDigName.BoundText) = 0 Or Val(fcmbSrvName.BoundText) = 0 Then
    With tRecset
    .open "Select top 1 * from OutdReg order by OpgCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
    If .EOF = False Then
...
'        clsVGNOPT.mOpgAraCodeOld_lng = clsPAT.mPttAraCode_lng
'        clsVGNOPT.mOpgStnCodeOld_lng = clsPAT.mPttStnCode_lng
'        clsVGNOPT.mOpgPcgCodeOld_lng = clsPAT.mPttPcgCode_lng
'        clsVGNOPT.mOpgPttAhCodeOld_lng = clsPAT.mPttAhCode_lng
'        clsVGNOPT.mOpgVtmCodeOld_lng = .mVtmCode_lng
'        clsVGNOPT.mOpgCDctCodeOld_lng = .mOpgCDctCode_lng
'        clsVGNOPT.mOpgRByCodeOld_lng = .mOpgRByCode_lng
'        clsVGNOPT.mOpgFDigCodeOld_lng = .mOpgFDigCode_lng
'        clsVGNOPT.mOpgSrvCodeOld_lng = .mOpgSrvCode_lng
'        clsVGNOPT.UpdateOutdRegOld
...
If FormAddEditMode = cFORM_ADDMODE Or Val(fcmbSrvName.BoundText) <> mOpgSrvCode_old Or mOpgCDctCode_old <> Val(fcmbCDctName.BoundText) Or mPttPcgCode_old <> Val(fcmbPcgName.BoundText) Or mOpgRByCode_old <> Val(fcmbRByName.BoundText) Or mOpgDate_old <> Ctod(dtpOpgDate.Text) Or mOpgTime_old <> TimeToMin(txtOpgTime_str) Then     ' Or mOpgRToCode_old <> Val(fcmbRToName.BoundText)
    GetServRateValues cnn:=dbVgnDatabase, mSrvCode:=Val(fcmbSrvName.BoundText), mTranDate:=Ctod(dtpOpgDate.Text), mTranTime:=TimeToMin(txtOpgTime_str.Text), mSrmPttCode:=mPttCode, mSrmDctCode:=Val(fcmbCDctName.BoundText), mSrmPcgCode:=Val(fcmbPcgName.BoundText), mSrmRByCode:=Val(fcmbRByName.BoundText), mSrmRToCode:=0, mSrmRate:=mOpgRate, mSrmDiscPer:=mOpgDiscPer, mSrmInfByPer:=mOpgInfByPer, mSrmDefByPer:=mOpgDefByPer
...
mskOpgDiscAmt.ForeColor = IIf(mOpgDiscAmt <> 0, vbRed, vbWindowText)
...
With tRecset
.open "Select * from OutdReg where OpgCode=" & CStr(mOldOpgCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("OpgVtmCode")
...
If fcmbOldOpgByOpgNo.CallFromText_Change = False Then
    ...
    If InterActiveChange(txtVtmName(Index)) = True Then
        ...
        If fcmbOldOpgByOpgNo.ListSelected = True And Val(fcmbOldOpgByOpgNo.BoundText) > 0 Then
            ...
            tRecset.open "Select * from OutdReg where OpgCode=" & CStr(mOldOpgCode) & " order by OpgDate,OpgVchNo,OpgCode", dbVgnDatabase, adOpenKeyset, adLockOptimistic
