...
Dim dbAcDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbCtnDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsPUR As clsCtnMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
Dim clsPRT As clsCtnMasterEntry, clsSIM As clsCtnMasterEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .Open "Select * from PurchDtl where PurCode=" & CStr(mPurCode) & " and PurICode=" & CStr(mPurICode) & "", dbCtnDatabase, adOpenKeyset, adLockOptimistic
...
Set clsPUR = New clsCtnMultiTranEntry
Set clsPUR.dbAcDatabase = dbAcDatabase
Set clsPUR.dbStDatabase = dbStDatabase
Set clsPUR.dbCtnDatabase = dbCtnDatabase
...
If mRemoteAccess = True Then
    datRecset.Open "Select * from PurchHdr" _
        ...
        & " order by PuhDate,PuhTime", dbCtnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from PurchHdr" _
        & " inner join PartyMast on PurchHdr.PuhPrtCode = PartyMast.PrtCode" _
...
With tRecset
.Open "Select top 1 * from PurchHdr where PuhVtmCode IN (" & mFormVchTypeList & ") and PuhCmpCode = " & CStr(sFinYrCmpCode) & " order by PuhCode desc", dbCtnDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
With tRecset
.Open "Select * from PurchDtl where PurCode=" & CStr(mPurCode) & " order by PurSno", dbCtnDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbCtnDatabase.Execute "Update PurchHdr set PuhStkCode=" & CStr(mPurStkCode) & " where PuhCode=" & CStr(mPurCode)
            dbCtnDatabase.Execute "Update PurchDtl set PurStkCode=" & CStr(mPurStkCode) & " where PurCode=" & CStr(mPurCode)
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPartywise.Value = True Then
    If optMonthwise.Value = True Then
        ...
        datRecset.Open "Select format(PuhDate,'yyyy/MM') as PurYrMonth" _
         ...
         , dbCtnDatabase, adOpenKeyset, adLockReadOnly
