  ...
  const detail = (
    <div style={{ padding: '10px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        <h2 style={{ margin: 0 }}>Investigation Entry & Billing</h2>
      ...
      <form onSubmit={handleSave}>
        ...
        <div className="dashboard-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <input className="form-control" type="date" value={currentRecord.LhdDate || ''} disabled={isReadonly} onChange={e => setCurrentRecord({...currentRecord, LhdDate: e.target.value})} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            <label className="form-label">Receipt Category</label>
            <select className="form-control" value={currentRecord.LhdReceiptType || 'Cash'} disabled={isReadonly} onChange={e => setCurrentRecord({...currentRecord, LhdReceiptType: e.target.value})}>
            ...
            <tbody>
              {details.map((row, idx) => (
                <tr key={idx} style={{ borderTop: '1px solid #333' }}>
                  ...
                  <td style={{ padding: '8px' }}>
                    <input className="form-control" type="number" value={row.LrdUnit} disabled={isReadonly} onChange={e => updateDetailRow(idx, { LrdUnit: parseFloat(e.target.value) || 0 })} />
                  </td>
                  <td style={{ padding: '8px' }}>
                    <input className="form-control" type="number" value={row.LrdRate} disabled={isReadonly} onChange={e => updateDetailRow(idx, { LrdRate: parseFloat(e.target.value) || 0 })} />
                  </td>
                  <td style={{ padding: '8px' }}>
                    <input className="form-control" type="number" value={row.LrdDiscPer} disabled={isReadonly} onChange={e => updateDetailRow(idx, { LrdDiscPer: parseFloat(e.target.value) || 0 })} />
                  </td>
                  <td style={{ padding: '8px' }}>
          ...
          <div style={{ width: '250px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {!isReadonly && (
              <div style={{ marginTop: 15, display: 'flex', gap: 10 }}>
                ...
                <button className="btn btn-primary" type="button" onClick={() => setActiveTab('summary')}>
                  Cancel
                </button>
              </div>
            )}
      ...
      summaryNode={
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ margin: 0 }}>Diagnostic Billing Registry</h2>
