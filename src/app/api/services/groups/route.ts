...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let whereClause: any = { SgpRecState: 1 };
    
    // For lookup filtering
    if (q) {
      if ('servGrpMst' === 'patMast') whereClause.PttName = { contains: q, mode: 'insensitive' };
    }

    const records = await prisma.servGrpMst.findMany({
      where: whereClause,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const record = await prisma.servGrpMst.create({ data });
