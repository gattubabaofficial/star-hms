...
export default function FinanceModule() {
  ...
  const mode = location.pathname.split('/')[1]; // opd, ipd, or lab
  ...
  const fetchBills = async () => {
    const endpoint = mode === 'opd' ? '/api/opd/bills' : mode === 'ipd' ? '/api/ipd-billing/headers' : '/api/lab/bills';
    try {
  ...
  const fetchHistory = async (billId: number) => {
    try {
      const res = await fetch(`http://localhost:3001/api/finance/${mode}/${billId}`);
      const data = await res.json();
      setHistory(data);
    } catch (e) {
      console.error(e);
  ...
  const handleTransaction = async (type: 'payment' | 'refund') => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:3001/api/finance/${mode}/${type}`, {
        method: 'POST',
