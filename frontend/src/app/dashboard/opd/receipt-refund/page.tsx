...
export default function OPDReceiptRefundPage() {
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API}/receipt-refunds?from_date=${fromDate}&to_date=${toDate}`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 0 }}>
      <div className={styles.toolbar} style={{ flexWrap: "wrap", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)" }}>OPD Receipt Refunds</span>
      </div>
            ...
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1150, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map(r => {
                    const selected = selectedRow?.OrhCode === r.OrhCode;
                    return (
                      <tr key={r.OrhCode} onClick={() => setSelectedRow(r)}
