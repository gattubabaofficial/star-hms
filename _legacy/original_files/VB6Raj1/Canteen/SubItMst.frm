...
Begin VB.Form frmSubItemMast 
   ...
   Begin VB.Frame frFormDtl 
      ...
      Top             =   1080
      Width           =   10935
      Begin VB.TextBox mskSimMaxAmtLimit 
         ...
         Width           =   1695
      End
      Begin VB.TextBox mskSimMaxCountLimit 
         Alignment       =   1  'Right Justify
         Appearance      =   0  'Flat
         BeginProperty DataFormat 
            Type            =   1
         ...
         Width           =   1455
      End
      Begin VB.TextBox mskSimPerDaysLimit 
         Alignment       =   1  'Right Justify
         Appearance      =   0  'Flat
         BeginProperty DataFormat 
            Type            =   1
         ...
         Width           =   1455
      End
      Begin VB.TextBox mskSimProfitPer 
         Alignment       =   1  'Right Justify
         Appearance      =   0  'Flat
         BeginProperty DataFormat 
            Type            =   1
...
Dim dbCtnDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, datRecset As New ADODB.Recordset
Dim mSimName_old As String, mSimMRPRate_old As Double
Dim clsSIM As clsCtnMasterEntry, clsSIG As clsCtnMasterEntry, clsIT As clsProduct, mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbSigName As clsCtnFlexSearch
...
Set clsSIM = New clsCtnMasterEntry
Set clsSIM.dbCtnDatabase = dbCtnDatabase
clsSIM.blnSubItmMast = True
clsSIM.Init

Set clsSIG = New clsCtnMasterEntry
Set clsSIG.dbCtnDatabase = dbCtnDatabase
...
datRecset.Open "Select * from SubItmMast" _
    ...
    & " order by SimName" _
    , dbCtnDatabase, adOpenKeyset, adLockOptimistic
Call SmryList
...
If Val(UnMyNumFmt(mskSimSaleRate.Text)) < 0 Then
    ...
    Exit Function
End If
If Val(UnMyPerFmt(mskSimProfitPer.Text)) < 0 Then
    ErrorBox "Invalid Input !!!"
    mskSimProfitPer.SetFocus
    Exit Function
End If

If txtSimName.Text <> mSimName_old Then
    If ChkIsDuplicate(dbCtnDatabase, "SubItmMast", "SimName", txtSimName.Text, "SimCode", Val(mskFormBoundField.Text)) = True Then
...
Private Sub mskSimMaxAmtLimit_GotFocus()
FlashActiveControl mskSimMaxAmtLimit, True
End Sub

Private Sub mskSimMaxAmtLimit_LostFocus()
FlashActiveControl mskSimMaxAmtLimit, False
End Sub

Private Sub mskSimMaxAmtLimit_Validate(Cancel As Boolean)
mskSimMaxAmtLimit.Text = ToMyNumFmt(mskSimMaxAmtLimit.Text, mUseAbs:=True)

End Sub

Private Sub mskSimMaxCountLimit_GotFocus()
FlashActiveControl mskSimMaxCountLimit, True
End Sub

Private Sub mskSimMaxCountLimit_LostFocus()
FlashActiveControl mskSimMaxCountLimit, False
End Sub

Private Sub mskSimMaxCountLimit_Validate(Cancel As Boolean)
mskSimMaxCountLimit.Text = ToMyNumFmt(mskSimMaxCountLimit.Text, mDecimals:=0, mUseAbs:=True)

End Sub

Private Sub mskSimPerDaysLimit_GotFocus()
FlashActiveControl mskSimPerDaysLimit, True
End Sub

Private Sub mskSimPerDaysLimit_LostFocus()
FlashActiveControl mskSimPerDaysLimit, False
End Sub

Private Sub mskSimPerDaysLimit_Validate(Cancel As Boolean)
mskSimPerDaysLimit.Text = ToMyNumFmt(mskSimPerDaysLimit.Text, mDecimals:=0, mUseAbs:=True)
...
Private Sub mskSimProfitPer_GotFocus()
FlashActiveControl mskSimProfitPer, True
End Sub

Private Sub mskSimProfitPer_LostFocus()
FlashActiveControl mskSimProfitPer, False
End Sub

Private Sub mskSimProfitPer_Validate(Cancel As Boolean)
mskSimProfitPer.Text = ToMyPerFmt(mskSimProfitPer.Text, mUseAbs:=True)
...
mskSimMaxAmtLimit.Text = ToMyNumFmt(0)
mskSimMaxCountLimit.Text = ToMyNumFmt(0, mDecimals:=0)
mskSimPerDaysLimit.Text = ToMyNumFmt(0, mDecimals:=0)
mskSimProfitPer.Text = ToMyPerFmt(0)
...
mskSimMaxAmtLimit.Text = ToMyNumFmt(.fields("SimMaxAmtLimit"))
mskSimMaxCountLimit.Text = ToMyNumFmt(.fields("SimMaxCountLimit"), mDecimals:=-1)
mskSimPerDaysLimit.Text = ToMyNumFmt(.fields("SimPerDaysLimit"), mDecimals:=-1)
mskSimProfitPer.Text = ToMyPerFmt(.fields("SimProfitPer"))
...
If mContinueUpdate = True Then   '''   .mItmCode_lng > 0   updated successfully
    ...
    .mSimMaxAmtLimit_dbl = Val(UnMyNumFmt(mskSimMaxAmtLimit.Text))
    .mSimMaxCountLimit_int = Val(UnMyNumFmt(mskSimMaxCountLimit.Text))
    .mSimPerDaysLimit_int = Val(UnMyNumFmt(mskSimPerDaysLimit.Text))
    ...
    .mSimProfitPer_dbl = Val(UnMyPerFmt(mskSimProfitPer.Text))
    If FormAddEditMode = cFORM_ADDMODE Then
        If .AddNew() = True Then
