...
Begin VB.Form frmIndoorBedStatus 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   ClientHeight    =   10755
   ClientLeft      =   60
   ...
   Begin VB.ComboBox cmbPatEntryMode 
      ...
      Enabled         =   0   'False
      Height          =   315
      ItemData        =   "IpdBedSt.frx":0000
      Left            =   1560
      List            =   "IpdBedSt.frx":000D
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      Enabled         =   0   'False
      Height          =   315
      ItemData        =   "IpdBedSt.frx":0034
      Left            =   0
      List            =   "IpdBedSt.frx":0041
      Style           =   1  'Simple Combo
      TabIndex        =   62
      TabStop         =   0   'False
   ...
   Begin VB.Frame frFormSmry 
      ...
      Begin VB.Frame frIpgTranDtl 
         ...
         End
      End
      Begin VB.CheckBox chkAdmittedOnly 
         Alignment       =   1  'Right Justify
         Appearance      =   0  'Flat
         BackColor       =   &H80000005&
      ...
      Begin VB.TextBox txtAraName1 
         ...
         Width           =   4575
      End
      Begin VB.CommandButton cmdShowFilter 
         Appearance      =   0  'Flat
         BackColor       =   &H00FFFF80&
         Caption         =   "Show Filter"
         BeginProperty Font 
      ...
      Begin VB.TextBox txtBdmName1 
         ...
         Width           =   1815
      End
      Begin VB.TextBox txtPttRefName1 
         Appearance      =   0  'Flat
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   8.25
   ...
   End
End
Attribute VB_Name = "frmIndoorBedStatus"
...
Dim mRemoteAccess As Boolean, mEntryAccessed As Boolean, mLastRowAccessed As Long, dtlAddEditMode As Boolean, dtlAddMode As Boolean, dtlEditCanceled As Boolean
...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsVgnSnglTranEntry, clsIBS As clsVgnSnglTranEntry, clsPAT As clsVgnMasterEntry, clsPCG As clsVgnMasterEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsBDM As clsVgnMasterEntry, clsVGNOPT As clsVgnIpdOptionsEntry
...
Dim fcmbAdmIpdByIpdNo As clsVgnFlexSearch, fcmbAdmIpdByPttRegNo As clsVgnFlexSearch, fcmbAdmIpdByPttName As clsVgnFlexSearch, fcmbPttName As clsVgnFlexSearch, fcmbPttRegNo As clsVgnFlexSearch, fcmbPcgName As clsVgnFlexSearch, fcmbCDctName As clsVgnFlexSearch, fcmbRByName As clsVgnFlexSearch, fcmbFDigName As clsVgnFlexSearch, fcmbBdmName As clsVgnFlexSearch, fcmbNextBdmName As clsVgnFlexSearch
Dim fcmbWrdName As clsVgnFlexSearch, fcmbFlrName As clsVgnFlexSearch
Dim frmTrn As Form
...
Private Sub chkAdmittedOnly_GotFocus()
FlashActiveControl chkAdmittedOnly, True
End Sub

Private Sub chkAdmittedOnly_LostFocus()
FlashActiveControl chkAdmittedOnly, False
End Sub
...
txtFlrName1.Text = ""
txtWrdName1.Text = ""
chkAdmittedOnly.Value = vbUnchecked

If frIpgTranDtl.Visible = True Then
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If .mCode_lng > 0 Then   ' updated successfully
        ...
        With tRecset
        .open "Select * from IBedState where IbsCode=" & CStr(mIbsCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
        ...
        If mIbsLIbsCode <> 0 Then
            With tRecset
            .open "Select * from IBedState where IbsCode=" & CStr(mIbsLIbsCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
    ...
    If dtlAddMode = False And Cancel = False Then
        If mIbsDate <> mIbsDate_old Then
            ...
            .open "Select * from ((IndrBlIbsDtl" _
                ...
                & " order by IblDate Desc" & "", dbVgnDatabase, adOpenKeyset, adLockReadOnly
...
clsVGNOPT.blnIndrRegOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrRegOptData
...
Set clsIBS = New clsVgnSnglTranEntry
Set clsIBS.dbAcDatabase = dbAcDatabase
Set clsIBS.dbVgnDatabase = dbVgnDatabase
...
Set clsBDM = New clsVgnMasterEntry
Set clsBDM.dbVgnDatabase = dbVgnDatabase
clsBDM.blnBedMast = True
clsBDM.Init

Set fcmbFlrName = New clsVgnFlexSearch
Set fcmbFlrName.dbVgnDatabase = dbVgnDatabase
fcmbFlrName.blnFloorMastList = True
fcmbFlrName.Init

Set fcmbWrdName = New clsVgnFlexSearch
Set fcmbWrdName.dbVgnDatabase = dbVgnDatabase
...
fcmbVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_REG) & " order by VtmName"
fcmbVtmName.Init
...
Set fcmbBdmName = New clsVgnFlexSearch
Set fcmbBdmName.dbVgnDatabase = dbVgnDatabase
fcmbBdmName.blnBedMastList = True
fcmbBdmName.Init

Set fcmbNextBdmName = New clsVgnFlexSearch
Set fcmbNextBdmName.dbVgnDatabase = dbVgnDatabase
...
Set fcmbAdmIpdByIpdNo = New clsVgnFlexSearch
Set fcmbAdmIpdByIpdNo.dbVgnDatabase = dbVgnDatabase
Set fcmbAdmIpdByIpdNo.dbComDatabase = dbComDatabase
fcmbAdmIpdByIpdNo.blnAdmIpgListByIpgNo = True
fcmbAdmIpdByIpdNo.Init

Set fcmbAdmIpdByPttRegNo = New clsVgnFlexSearch
Set fcmbAdmIpdByPttRegNo.dbVgnDatabase = dbVgnDatabase
Set fcmbAdmIpdByPttRegNo.dbComDatabase = dbComDatabase
fcmbAdmIpdByPttRegNo.blnAdmIpgListByPttRegNo = True
fcmbAdmIpdByPttRegNo.Init

Set fcmbAdmIpdByPttName = New clsVgnFlexSearch
Set fcmbAdmIpdByPttName.dbVgnDatabase = dbVgnDatabase
Set fcmbAdmIpdByPttName.dbComDatabase = dbComDatabase
fcmbAdmIpdByPttName.blnAdmIpgListByPttName = True
fcmbAdmIpdByPttName.Init
...
If mRemoteAccess = True Then
    'datRecset.Open "Select * from IndrReg inner join PatMast on IndrReg.IpgPttCode=PatMast.PttCode where IpgCode=" & CStr(mIpgCode) & " order by IpgDate,IpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    'datRecset.Open "Select * from IndrReg inner join PatMast on IndrReg.IpgPttCode=PatMast.PttCode order by IpgDate,IpgTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
Set clsVTM = Nothing
Set clsBDM = Nothing
Set fcmbAdmIpdByIpdNo = Nothing
Set fcmbAdmIpdByPttRegNo = Nothing
Set fcmbAdmIpdByPttName = Nothing
...
'        If FormAddEditMode = cFORM_ADDMODE And mPttRegNo <> mPttRegNo_old And clsVGNOPT.mIpgPttRegNoAddModeChange_bln = False Then
'            AlertBox "Change Prohibited in Add Mode !!!"
'            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
'            mskPttRegNo.SetFocus
'            GoTo EndSub
'
'        ElseIf FormAddEditMode = cFORM_EDITMODE And mPttRegNo <> mPttRegNo_old And clsVGNOPT.mIpgPttRegNoEditModeChange_bln = False Then
'            AlertBox "Change Prohibited in Edit Mode !!!"
'            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
'            mskPttRegNo.SetFocus
'            GoTo EndSub
'
'        ElseIf mPttRegNo = 0 And clsVGNOPT.mIpgPttRegNoNotZero_bln = True Then
'            AlertBox "Prohibited Zero Patient Reg.No. !!!"
'            If FormAddEditMode = cFORM_ADDMODE Then
'                If clsVGNOPT.mIpgPttRegNoAutoNumber_bln = True Then
...
'            If ChkIsDuplicate(dbVgnDatabase, "PatMast", "PttRegNo", mPttRegNo, "PttCode", mIpgPttCode) = True Then
...
'                mIpgVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrReg", mFieldName:="IpgVchNo", mVtmFieldName:="IpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="IpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IpgDate", mDateFieldValue:=Ctod(dtpIpgDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormIpgAcsPermFileName)
'            Else
...
'
'With tRecset
'.Open "Select * from IBedState where IbsIpgCode=" & CStr(mIbsIpgCode) & " order by IbsCode", dbVgnDatabase, adOpenKeyset, adLockOptimistic
'If .RecordCount > 0 Then
'    Do While .EOF = False
...
'                mskIpgVchNo(Index).Text = ToMyNumFmt(mIpgVchNo, mDecimals:=0)
'            Rem Else
'            Rem     GetNextVTypeNo dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrReg", mFieldName:="IpgVchNo", mVtmFieldName:="IpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="IpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IpgDate", mDateFieldValue:=Ctod(dtpIpgDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormIpgAcsPermFileName, mDeletedVchNo:=mIpgVchNo
'            End If
'        End If
...
'        End If
'    Else
'        If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrReg", mChkFieldName:="IpgVchNo", mChkFieldValue:=CStr(mIpgVchNo), mVtmFieldName:="IpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="IpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
Private Sub mskIpgVchNo1_GotFocus()
FlashActiveControl mskIpgVchNo1, True
End Sub

Private Sub mskIpgVchNo1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbAdmIpdByIpdNo.CallFromText_Change = False Then
        fcmbAdmIpdByIpdNo.CallFromText_Change = True
        If InterActiveChange(mskIpgVchNo1) = True Then
            fcmbAdmIpdByIpdNo.UserText = Array(UnMyNumFmt(mskIpgVchNo1.Text), mskIpgVchNo1.SelStart)
            fcmbAdmIpdByIpdNo.Show
            mskIpgVchNo1.Text = ToMyNumFmt(fcmbAdmIpdByIpdNo.Text, mDecimals:=0, mSuppressZero:=True)
            If fcmbAdmIpdByIpdNo.ListSelected = True Then
                SendKeys "{tab}"
            End If
        End If
        fcmbAdmIpdByIpdNo.CallFromText_Change = False
    End If
End If

End Sub

Private Sub mskIpgVchNo1_LostFocus()
FlashActiveControl mskIpgVchNo1, False
End Sub
...
Private Sub mskPttRegNo1_GotFocus()
FlashActiveControl mskPttRegNo1, True
End Sub

Private Sub mskPttRegNo1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbAdmIpdByPttRegNo.CallFromText_Change = False Then
        fcmbAdmIpdByPttRegNo.CallFromText_Change = True
        If InterActiveChange(mskPttRegNo1) = True Then
            fcmbAdmIpdByPttRegNo.UserText = Array(UnMyNumFmt(mskPttRegNo.Text), mskPttRegNo.SelStart)
            fcmbAdmIpdByPttRegNo.Show
            mskPttRegNo1.Text = ToMyNumFmt(fcmbAdmIpdByPttRegNo.Text, mDecimals:=0, mSuppressZero:=True)
            If fcmbAdmIpdByPttRegNo.ListSelected = True Then
                SendKeys "{tab}"
            End If
        End If
        fcmbAdmIpdByPttRegNo.CallFromText_Change = False
    End If
End If

End Sub

Private Sub mskPttRegNo1_LostFocus()
FlashActiveControl mskPttRegNo1, False
End Sub
...
Private Sub txtAraName1_GotFocus()
FlashActiveControl txtAraName1, True
End Sub

Private Sub txtAraName1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbAraName.CallFromText_Change = False Then
        fcmbAraName.CallFromText_Change = True
        If InterActiveChange(txtAraName1) = True Then
            fcmbAraName.UserText = Array(txtAraName1.Text, txtAraName1.SelStart)
            fcmbAraName.Show
            txtAraName1.Text = fcmbAraName.Text
            If fcmbAraName.ListSelected = True Then
                SendKeys "{tab}"
            End If
        End If
...
Private Sub txtAraName1_LostFocus()
FlashActiveControl txtAraName1, False
End Sub
...
Private Sub txtBdmName1_GotFocus()
FlashActiveControl txtBdmName1, True
End Sub

Private Sub txtBdmName1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbBdmName.CallFromText_Change = False Then
        fcmbBdmName.CallFromText_Change = True
        If InterActiveChange(txtBdmName1) = True Then
            fcmbBdmName.UserText = Array(txtBdmName1.Text, txtBdmName1.SelStart)
            fcmbBdmName.Show
            txtBdmName1.Text = fcmbBdmName.Text
...
Private Sub txtBdmName1_LostFocus()
FlashActiveControl txtBdmName1, False
End Sub
...
Private Sub txtFlrName1_GotFocus()
FlashActiveControl txtFlrName1, True
End Sub

Private Sub txtFlrName1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbFlrName.CallFromText_Change = False Then
        fcmbFlrName.CallFromText_Change = True
        If InterActiveChange(txtFlrName1) = True Then
            fcmbFlrName.UserText = Array(txtFlrName1.Text, txtFlrName1.SelStart)
            fcmbFlrName.Show
            txtFlrName1.Text = fcmbFlrName.Text
...
Private Sub txtFlrName1_LostFocus()
FlashActiveControl txtFlrName1, False
End Sub
...
Dim tRecset As New ADODB.Recordset, srow As Integer, mIbsCode As Long, mIbsDischTime As Date, mIbsDischTime_old As Date
...
If txtIbsDischTime.Text <> "" Then
    ...
    mIbsCode = Val(Mfgrd3.TextMatrix(srow, X3IBSCODE_COL))
    If dtlAddMode = False And Cancel = False Then
        If mIbsDischTime <> mIbsDischTime_old Then
            With tRecset
            .open "Select * from ((IndrBlIbsDtl" _
                & " inner join IBedState on IndrBlIbsDtl.IbbsIbsCode = IBedState.IbsCode)" _
                & " inner join IndrBill on IndrBlIbsDtl.IbbsCode = IndrBill.IblCode and IndrBlIbsDtl.IbbsIblICode = IndrBill.IblICode)" _
                ...
                & " order by IblDate Desc" & "", dbVgnDatabase, adOpenKeyset, adLockReadOnly
            If .RecordCount > 0 Then
                If .fields("IbbsToTime") > mIbsDischTime Then
                    If IsNull(.fields("IbhCode")) = False Then
...
'fcmbAraName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgAraCode")): txtAraName.Text = fcmbAraName.Text
'fcmbStnName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgStnCode")): txtStnName.Text = fcmbStnName.Text
'fcmbPcgName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgPcgCode")): txtPcgName.Text = fcmbPcgName.Text
'fcmbAhName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgPttAhCode")): txtAhName.Text = fcmbAhName.Text
'
'Call ShowPatAge
'Call ShowPatBal
...
'
'fcmbVtmName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgVtmCode")): txtVtmName(0).Text = fcmbVtmName.Text
'fcmbCDctName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgCDctCode")): txtCDctName.Text = fcmbCDctName.Text
'fcmbRByName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgRByCode")): txtRByName.Text = fcmbRByName.Text
'fcmbFDigName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgFDigCode")): txtFDigName.Text = fcmbFDigName.Text
'fcmbBdmName.BoundText = CStr(clsVGNOPT.GetIndrRegOptBoundCode("IpgBdmCode")): txtBdmName(0).Text = fcmbBdmName.Text
'
'mIpgVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrReg", mFieldName:="IpgVchNo", mVtmFieldName:="IpgVtmCode", mVtmFieldValue:=Val(fcmbVtmName.BoundText), mFinYrFieldName:="IpgCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IpgDate", mDateFieldValue:=Ctod(dtpIpgDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormIpgAcsPermFileName)
'mskIpgVchNo(0).Text = ToMyNumFmt(mIpgVchNo, mDecimals:=0)
'
...
'        clsVGNOPT.mIpgAraCodeOld_lng = clsPAT.mPttAraCode_lng
'        clsVGNOPT.mIpgStnCodeOld_lng = clsPAT.mPttStnCode_lng
'        clsVGNOPT.mIpgPcgCodeOld_lng = clsPAT.mPttPcgCode_lng
'        clsVGNOPT.mIpgPttAhCodeOld_lng = clsPAT.mPttAhCode_lng
'        clsVGNOPT.mIpgVtmCodeOld_lng = .mVtmCode_lng
'        clsVGNOPT.mIpgCDctCodeOld_lng = .mIpgCDctCode_lng
'        clsVGNOPT.mIpgRByCodeOld_lng = .mIpgRByCode_lng
'        clsVGNOPT.mIpgFDigCodeOld_lng = .mIpgFDigCode_lng
'        clsVGNOPT.mIpgBdmCodeOld_lng = .mIpgBdmCode_lng
'        clsVGNOPT.UpdateIndrRegOld
'
...
'            If FormAddEditMode = cFORM_ADDMODE Or mMissingIBedState = True Then
'                dbVgnDatabase.Execute "Update IndrReg set IpgIbsCode=" & CStr(mIpgIbsCode) & " where IpgCode=" & CStr(mIpgCode)
'            End If
'
'            mEntrySaved = True
'        Else
...
Private Sub txtPttName1_GotFocus()
FlashActiveControl txtPttName1, True
End Sub

Private Sub txtPttName1_KeyDown(KeyCode As Integer, Shift As Integer)

If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbAdmIpdByPttName.CallFromText_Change = False Then
        fcmbAdmIpdByPttName.CallFromText_Change = True
        If InterActiveChange(txtPttName1) = True Then
            fcmbAdmIpdByPttName.UserText = Array(txtPttName1.Text, txtPttName1.SelStart)
            fcmbAdmIpdByPttName.Show
            txtPttName1.Text = fcmbAdmIpdByPttName.Text
            If fcmbAdmIpdByPttName.ListSelected = True Then
                SendKeys "{tab}"
            End If
        End If
        fcmbAdmIpdByPttName.CallFromText_Change = False
    End If
...
Private Sub txtPttName1_LostFocus()
FlashActiveControl txtPttName1, False
End Sub
...
Private Sub txtPttRefName1_GotFocus()
FlashActiveControl txtPttRefName1, True
End Sub

Private Sub txtPttRefName1_LostFocus()
FlashActiveControl txtPttRefName1, False
End Sub
...
clsListStru.AddFields mExpr:="tBdmSmryId", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="BdmCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="IbsCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="IpgCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="PttCode", mAttrib:=cFAT_SYSTEM

clsListStru.AddFields mExpr:="BdmName", mTitle:="Bed No", mAlign:=1, mWidth:=1500, mShowItem:=True
clsListStru.AddFields mExpr:="WrdName", mTitle:="Ward Name", mAlign:=1, mWidth:=1250, mShowItem:=True
clsListStru.AddFields mExpr:="FlrName", mTitle:="Floor Name", mAlign:=1, mWidth:=1250, mShowItem:=False
clsListStru.AddFields mExpr:="PttName", mTitle:="Patient Name", mAlign:=1, mWidth:=3000, mShowItem:=True
...
clsListStru.AddFields mExpr:="AraName", mTitle:="Area", mAlign:=1, mWidth:=1750, mShowItem:=True
clsListStru.AddFields mExpr:="StnName", mTitle:="Station", mAlign:=1, mWidth:=1750, mShowItem:=True
clsListStru.AddFields mExpr:="VtmName", mTitle:="Voucher", mAlign:=1, mWidth:=1500, mShowItem:=True
clsListStru.AddFields mExpr:="IpgVchNo", mTitle:="Ipd No", mAlign:=7, mWidth:=1500, mShowItem:=True
clsListStru.AddFields mExpr:="IpgDate", mTitle:="Date", mAlign:=1, mWidth:=1500, mShowItem:=True
clsListStru.AddFields mExpr:="IpgTime", mTitle:="Time", mAlign:=1, mWidth:=1500, mShowItem:=True

datRecset.open "Select * from ((BedMast" _
    ...
    & " Left Join" _
    & " (Select * from (Select * from IBedState where IbsDischState = " & CStr(cIBS_STATE_OCCUPIED) & ") as IBedState" _
    & " inner join ((IndrReg inner join ((PatMast inner join AreaMast on PatMast.PttAraCode = AreaMast.AraCode)" _
    ...
    & " on IBedState.IbsIpgCode = IndrReg.IpgCode)" _
    & " as IBedState" _
    & " on BedMast.BdmCode = IBedState.IbsBdmCode" _
    & " Where BedMast.BdmShowInList = True" _
    & IIf(txtPttName1.Text <> "", " and PttName like '%" & ToMyString(txtPttName1.Text) & "%'", "") _
    & IIf(mskPttRegNo1.Text <> "", " and PttRegNo = " & CStr(Val(UnMyNumFmt(mskPttRegNo1.Text))) & "", "") _
    & IIf(txtPttRefName1.Text <> "", " and PttRefName like '%" & ToMyString(txtPttRefName1.Text) & "%'", "") _
    & IIf(txtStnName1.Text <> "", " and StnName like '%" & ToMyString(txtStnName1.Text) & "%'", "") _
    & IIf(txtAraName1.Text <> "", " and AraName like '%" & ToMyString(txtAraName1.Text) & "%'", "") _
    & IIf(mskIpgVchNo1.Text <> "", " and IpgVchNo = " & CStr(Val(UnMyNumFmt(mskIpgVchNo1.Text))) & "", "") _
    & IIf(txtBdmName1.Text <> "", " and BdmName like '%" & ToMyString(txtBdmName1.Text) & "%'", "") _
    & IIf(txtFlrName1.Text <> "", " and FlrName like '%" & ToMyString(txtFlrName1.Text) & "%'", "") _
    & IIf(txtWrdName1.Text <> "", " and WrdName like '%" & ToMyString(txtWrdName1.Text) & "%'", "") _
    & IIf(chkAdmittedOnly.Value = vbChecked, " and IsNull(IpgCode) = False", "") _
    & " order by BdmName" _
...
If .RecordCount > 0 Then
    Do While .EOF = False
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("tBdmSmryId")) = Format(.fields("BdmCode"), cLONGCODESTRFMT) & Format(NoneZero1(.fields("IbsCode"), 0), cLONGCODESTRFMT) & Format(NoneZero1(.fields("IpgCode"), 0), cLONGCODESTRFMT) & Format(NoneZero1(.fields("PttCode"), 0), cLONGCODESTRFMT)
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("BdmCode")) = .fields("BdmCode")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IbsCode")) = NoneZero1(.fields("IbsCode"), 0)
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgCode")) = NoneZero1(.fields("IpgCode"), 0)
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PttCode")) = NoneZero1(.fields("PttCode"), 0)
        
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("BdmName")) = .fields("BdmName")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("WrdName")) = .fields("WrdName")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("FlrName")) = .fields("FlrName")
        If IsNull(.fields("IbsCode")) = False Then
            ...
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AraName")) = .fields("AraName")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("StnName")) = .fields("StnName")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("VtmName")) = .fields("VtmName")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgVchNo")) = ToMyNumFmt(.fields("IpgVchNo"), mDecimals:=0)
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgDate")) = Dtoc(.fields("IpgDate"))
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgTime")) = MinToTime(.fields("IpgTime"))
        Else
            ...
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AraName")) = ""
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("StnName")) = ""
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("VtmName")) = ""
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgVchNo")) = ""
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgDate")) = ""
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgTime")) = ""
        End If
...
mQryStr = "Select *" _
 & ",format(TrnDate,'yyyymmdd')+format(TrnVtmCode,'" & CStr(cLONGCODESTRFMT) & "')+format(TrnCode,'" & CStr(cLONGCODESTRFMT) & "') as tTrnSmryId"
...
mQryStr = mQryStr & "Select " _
 ...
 & " from OutdReg" _
 & " where OpgCode = -1 and True = False "
Rem indoor advance ---------------------
...
If .EOF = False Then
    Do While .EOF = False
        ...
        Mfgrd2.TextMatrix(srow, X2TRNAMTAFTDISC_COL) = ToMyNumFmt(.fields("TrnAmtAftDisc") * IIf(InList(.fields("VtmSysCode"), Array(cVGN_VTYPE_INDOOR_REFD, cVGN_VTYPE_INDOOR_BLREFD, cVGN_VTYPE_INDOOR_RGREFD)) = True, -1, 1))
...
If Index = 0 And Cancel = False Then
    If Val(fcmbBdmName.BoundText) <> mIpgBdmCode_old Then
        ...
        With tRecset
        .open "Select count(*) as MyCount from IBedState where IbsIpgCode=" & CStr(mIpgCode) & " and IbsCode>" & CStr(clsIPG.mIpgIbsCode_lng), dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
Private Sub txtStnName1_GotFocus()
FlashActiveControl txtStnName1, True
End Sub

Private Sub txtStnName1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbStnName.CallFromText_Change = False Then
        fcmbStnName.CallFromText_Change = True
        If InterActiveChange(txtStnName1) = True Then
            fcmbStnName.UserText = Array(txtStnName1.Text, txtStnName1.SelStart)
            fcmbStnName.Show
            txtStnName1.Text = fcmbStnName.Text
            If fcmbStnName.ListSelected = True Then
                SendKeys "{tab}"
            End If
        End If
...
Private Sub txtStnName1_LostFocus()
FlashActiveControl txtStnName1, False
End Sub
...
With tRecset
.open "Select * from IndrReg where IpgCode=" & CStr(mOldIpgCode) & " order by IpgDate,IpgVchNo,IpgCode", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
If fcmbAdmIpdByIpdNo.CallFromText_Change = False Then
    ...
    If InterActiveChange(txtVtmName(0)) = True Then
        fcmbAdmIpdByIpdNo.UserText = Array("", 0)
        fcmbAdmIpdByIpdNo.Show
        If fcmbAdmIpdByIpdNo.ListSelected = True And Val(fcmbAdmIpdByIpdNo.BoundText) > 0 Then
            mOldIpgCode = Val(fcmbAdmIpdByIpdNo.BoundText)
            tRecset.open "Select * from IndrReg where IpgCode=" & CStr(mOldIpgCode) & " order by IpgDate,IpgVchNo,IpgCode", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If fcmbAdmIpdByPttRegNo.CallFromText_Change = False Then
    fcmbAdmIpdByPttRegNo.CallFromText_Change = True
    If InterActiveChange(mskPttRegNo) = True Then
        fcmbAdmIpdByPttRegNo.UserText = Array("", 0)
        fcmbAdmIpdByPttRegNo.Show
        If fcmbAdmIpdByPttRegNo.ListSelected = True And Val(fcmbAdmIpdByPttRegNo.BoundText) > 0 Then
            mOldIpgCode = fcmbAdmIpdByPttRegNo.LFieldValue("IpgCode")
            tRecset.open "Select * from IndrReg where IpgCode=" & CStr(mOldIpgCode) & " order by IpgDate,IpgVchNo,IpgCode", dbVgnDatabase, adOpenKeyset, adLockOptimistic
            If tRecset.RecordCount > 0 Then
                mOldPttCode = tRecset.fields("IpgPttCode")
            End If
...
Private Sub txtWrdName1_GotFocus()
FlashActiveControl txtWrdName1, True
End Sub

Private Sub txtWrdName1_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    If fcmbWrdName.CallFromText_Change = False Then
        fcmbWrdName.CallFromText_Change = True
        If InterActiveChange(txtWrdName1) = True Then
            fcmbWrdName.UserText = Array(txtWrdName1.Text, txtWrdName1.SelStart)
            fcmbWrdName.Show
            txtWrdName1.Text = fcmbWrdName.Text
...
Private Sub txtWrdName1_LostFocus()
FlashActiveControl txtWrdName1, False
End Sub
...
Dim srow As Integer, mTrnCode As Long, mTrnSysType As String
