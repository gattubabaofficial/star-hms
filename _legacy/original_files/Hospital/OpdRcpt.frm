...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsHmsSnglTranEntry, clsORC As clsHmsOpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsHOPT As clsHmsOpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If dtlAddMode = True Then
        ...
        If .AddNewDtl(mOrcCode) = True Then
            ...
            Rem dbHmsDatabase.Execute "Update OutdRcDctDtl set OrdOrcCode=" & CStr(.mTrnDtlICode_lng) & " where OrdCode=" & CStr(mOrcCode) & " and OrdOrcCode=" & CStr(mOrcICode) & ""
    ...
    With tRecset
    .open "Select * from OutdRcpt where OrcCode=" & CStr(mOrcCode) & " and OrcICode=" & CStr(mOrcICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    ...
    If GetServDctDtlCount(cnn:=dbHmsDatabase, mTableName:="OutdRcDctDtl", mSdcCodeField:="OrdCode", mSdcCode:=.fields("OrcCode"), mSdcSubCodeField:="OrdOrcCode", mSdcSubCode:=.fields("OrcICode")) > 0 Then
...
If cmdOK.Caption = "&OK" And dtlSdcEditCanceled = False Then
    ...
    RefreshDatabase dbHmsDatabase
    With tRecset
    .open "Select * from OutdRcDctDtl where OrdCode=" & CStr(mOrcCode) & " and OrdOrcCode=" & CStr(mOrdOrcCode) & " and OrdICode=" & CStr(mOrdICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
fcmbOrcVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_OUTDOOR_RCPT) & " order by VtmName"
fcmbOrcVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdHdr inner join PatMast on OutdHdr.OhrPttCode = PatMast.PttCode where OhrCode=" & CStr(mOrcCode) & " order by OhrDate,OhrTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("OhrDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("OhrDate"))
    AddSmryParaLayer mClear:=True
    
Else
    datRecset.open "Select * from OutdHdr inner join PatMast on OutdHdr.OhrPttCode=PatMast.PttCode order by OhrDate,OhrTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mOrcPttRegNoNotZero_bln = True Then
    ...
    If mOrcVchNo <> mOrcVchNo_old Or Val(fcmbOrcVtmName.BoundText) <> mOrcVtmCode_old Or Ctod(dtpOrcDate.Text) <> mOrcDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbHmsDatabase, dbAcDatabase, mTableName:="OutdHdr", mChkFieldName:="OhrVchNo", mChkFieldValue:=mOrcVchNo, mKeyFieldName:="OhrCode", mKeyFieldValue:=mOrcCode, mVtmFieldName:="OhrVtmCode", mVtmFieldValue:=Val(fcmbOrcVtmName.BoundText), mFinYrFieldName:="OhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OhrDate", mDateFieldValue:=Ctod(dtpOrcDate.Text)) = True Then
            ...
            If FormAddEditMode = cFORM_ADDMODE Then
                mOrcVchNo = GetNextVTypeNo(dbHmsDatabase, dbAcDatabase, dbComDatabase, mTableName:="OutdHdr", mFieldName:="OhrVchNo", mVtmFieldName:="OhrVtmCode", mVtmFieldValue:=Val(fcmbOrcVtmName.BoundText), mFinYrFieldName:="OhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="OhrDate", mDateFieldValue:=Ctod(dtpOrcDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormOrcAcsPermFileName)
            Else
...
ElseIf mOrcVchNo > 0 Then
    If Index = 0 Then
        ...
        End If
    Else
        If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="OutdHdr", mChkFieldName:="OhrVchNo", mChkFieldValue:=CStr(mOrcVchNo), mVtmFieldName:="OhrVtmCode", mVtmFieldValue:=Val(fcmbOrcVtmName.BoundText), mFinYrFieldName:="OhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
fcmbOrcVtmName.BoundText = CStr(clsHOPT.GetOutdRcptOptBoundCode("OrcVtmCode")): txtOrcVtmName(0).Text = fcmbOrcVtmName.Text
...
With tRecset
.open "Select * from OutdRcpt where OrcCode=" & CStr(mOrcCode) & " order by OrcSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If FormAddEditMode = cFORM_ADDMODE Then
    If .AddNewHdr() = True Then
        ...
        dbHmsDatabase.Execute "Delete from OutdRcDctDtl where OrdOrcCode IN (Select OrcICode from OutdRcpt as orc inner join ServMast as srv on orc.OrcSrvCode=srv.SrvCode where srv.SrvMultiDctIncl=False and orc.OrcCode=" & CStr(mOrcCode) & ") and OrdCode=" & CStr(mOrcCode) & ""
        dbHmsDatabase.CommitTrans
        
        Rem updating linked autonumber
        Rem dbHmsDatabase.BeginTrans
        Rem dbHmsDatabase.Execute "Update OutdRcDctDtl set OrdCode=" & CStr(.mTrnCode_lng) & " where OrdCode=" & CStr(mOrcCode) & ""
        Rem dbHmsDatabase.CommitTrans
    End If
Else
    .UpdateHdr mOrcCode
...
ElseIf optDetailed.Value = True Then
    ...
    datRecset.open "Select OutdHdr.*,PatMast.*,PatCatgMst.PcgName,AreaMast.AraName,StsnMast.StnName,VTypMast.VtmName" _
     & ",format(OhrDate,'yyyymmdd')+format(OhrCode,'0000000') as tOrcSmryId" _
     & " from (OutdHdr inner join (((PatMast" _
     ...
     & " on OutdHdr.OhrPttCode = PatMast.PttCode)" _
...
If Val(fcmbSrvName.BoundText) <> mOrcSrvCode_old Or mOpgCDctCode_old <> clsOPG.mOpgCDctCode_lng Or mPttPcgCode_old <> clsPAT.mPttPcgCode_lng Or mOpgRByCode_old <> clsOPG.mOpgRByCode_lng Or mOpgRToCode_old <> clsOPG.mOpgRToCode_lng Or mOrcDate_old <> Ctod(dtpOrcDate.Text) Or mOrcTime_old <> TimeToMin(txtOrcTime_str) Or FormAddEditMode = cFORM_ADDMODE Then
    GetServRateValues cnn:=dbHmsDatabase, mSrvCode:=Val(fcmbSrvName.BoundText), mTranDate:=Ctod(dtpOrcDate.Text), mTranTime:=TimeToMin(txtOrcTime_str.Text), mSrmPttCode:=mOrcPttCode, mSrmDctCode:=clsOPG.mOpgCDctCode_lng, mSrmPcgCode:=clsPAT.mPttPcgCode_lng, mSrmRByCode:=clsOPG.mOpgRByCode_lng, mSrmRToCode:=0, mSrmRate:=mOrcRate, mSrmDiscPer:=mOrcDiscPer, mSrmInfByPer:=mOrcInfByPer, mSrmDefByPer:=mOrcDefByPer
...
With tRecset
.open "Select * from OutdHdr where OhrCode=" & CStr(mOldOrcCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
With tRecset
.open "Select top 1 * from OutdHdr where OhrPttCode=" & CStr(mPttCode) & " order by OhrDate desc,OhrVchNo desc,OhrCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    ...
    If InterActiveChange(txtPttName) = True Then
        ...
        If fcmbOldOpgByPttName.ListSelected = True And Val(fcmbOldOpgByPttName.BoundText) > 0 Then
            If FormAddEditMode = cFORM_ADDMODE And Mfgrd2.Rows = 2 Then
                ...
                Set tRecset = dbHmsDatabase.Execute("Select * from OutdReg where OpgCode = " & CStr(mOldOpgCode) & "")
...
If dtlAddEditMode = True And dtlAddMode = False And mDataOk = True And InterActiveChange(cmdOK) = True Then
    ...
    If clsSRV.mSrvMultiDctIncl_bln = False Then
        If GetServDctDtlCount(cnn:=dbHmsDatabase, mTableName:="OutdRcDctDtl", mSdcCodeField:="OrdCode", mSdcCode:=mOrcCode, mSdcSubCodeField:="OrdOrcCode", mSdcSubCode:=Val(mskOrcICode.Text)) > 0 Then
...
With tRecset
.open "Select OutdPymtDtl.*,OutdPymtHdr.OphVtmCode,OutdPymtHdr.OphVchNo from OutdPymtDtl inner join OutdPymtHdr on OutdPymtDtl.OpyCode=OutdPymtHdr.OphCode where OpyOhrCode=" & CStr(mOrcCode) & " order by OpyDate", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select OutdRefdDtl.*,OutdRefdHdr.OfhVtmCode,OutdRefdHdr.OfhVchNo from OutdRefdDtl inner join OutdRefdHdr on OutdRefdDtl.OfdCode=OutdRefdHdr.OfhCode where OfdOhrCode=" & CStr(mOrcCode) & " order by OfdDate", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select * from OutdRcDctDtl where OrdCode=" & CStr(mOrdCode) & " and OrdOrcCode=" & CStr(mOrdOrcCode) & " order by OrdSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
