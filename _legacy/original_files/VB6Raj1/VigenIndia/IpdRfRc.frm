...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsVgnSnglTranEntry, clsIRC As clsVgnIpdMultiTranEntry, clsIFD As clsVgnIpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnIpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from IndrRefdDtl where IfdCode=" & CStr(mIfdCode) & " and IfdICode=" & CStr(mIfdICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
clsVGNOPT.blnIndrRefdOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrRefdOptData
...
Set clsIFD = New clsVgnIpdMultiTranEntry
Set clsIFD.dbAcDatabase = dbAcDatabase
Set clsIFD.dbVgnDatabase = dbVgnDatabase
...
fcmbIfdVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_REFD) & " order by VtmName"
fcmbIfdVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrRefdHdr inner join PatMast on IndrRefdHdr.IfhPttCode=PatMast.PttCode where IfhCode=" & CStr(mIfdCode) & " order by IfhDate,IfhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrRefdHdr inner join PatMast on IndrRefdHdr.IfhPttCode=PatMast.PttCode order by IfhDate,IfhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mIfdPttRegNoNotZero_bln = True Then
    ...
    If mIfdVchNo <> mIfdVchNo_old Or Val(fcmbIfdVtmName.BoundText) <> mIfdVtmCode_old Or Ctod(dtpIfdDate.Text) <> mIfdDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="IndrRefdHdr", mChkFieldName:="IfhVchNo", mChkFieldValue:=mIfdVchNo, mKeyFieldName:="IfhCode", mKeyFieldValue:=mIfdCode, mVtmFieldName:="IfhVtmCode", mVtmFieldValue:=Val(fcmbIfdVtmName.BoundText), mFinYrFieldName:="IfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IfhDate", mDateFieldValue:=Ctod(dtpIfdDate.Text)) = True Then
            ...
            If FormAddEditMode = True Then
                mIfdVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRefdHdr", mFieldName:="IfhVchNo", mVtmFieldName:="IfhVtmCode", mVtmFieldValue:=Val(fcmbIfdVtmName.BoundText), mFinYrFieldName:="IfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IfhDate", mDateFieldValue:=Ctod(dtpIfdDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
...
fcmbIfdVtmName.BoundText = CStr(clsVGNOPT.GetIndrRefdOptBoundCode("IfdVtmCode")): txtIfdVtmName(0).Text = fcmbIfdVtmName.Text
...
With tRecset
.open "Select * from IndrRefdDtl where IfdCode=" & CStr(mIfdCode) & " order by IfdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mIfdVtmCodeOld_lng = .mTrnVtmCode_lng
    clsVGNOPT.UpdateIndrRefdOld
...
With tRecset
.open "Select * from IndrRefdHdr where IfhCode=" & CStr(mOldIrcCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IfhVtmCode")
...
With tRecset
.open "Select top 1 * from IndrRefdHdr where IfhPttCode=" & CStr(mPttCode) & " order by IfhDate desc,IfhVchNo desc,IfhCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrRefdHdr", mChkFieldName:="IfhVchNo", mChkFieldValue:=CStr(mIfdVchNo), mVtmFieldName:="IfhVtmCode", mVtmFieldValue:=Val(fcmbIfdVtmName.BoundText), mFinYrFieldName:="IfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
