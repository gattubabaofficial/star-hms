...
Begin VB.Form frmIndoorRgPayment 
   ...
   Begin VB.Label lblFormHeading 
      ...
      BackColor       =   &H00E0E0E0&
      BorderStyle     =   1  'Fixed Single
      Caption         =   "Indoor On Account Payments"
      BeginProperty Font 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   0
...
''    With tRecset
''    .Open "Select * from IndrRgPymtDtl where IgtCode=" & CStr(mIgtCode) & " and IgtICode=" & CStr(mIgtICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''
...
clsVGNOPT.blnIndrRgPymtOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrRgPymtOptData
...
fcmbIgtVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_RGPYMT) & " order by VtmName"
fcmbIgtVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrRgPymt inner join PatMast on IndrRgPymt.IgtPttCode=PatMast.PttCode where IgtCode=" & CStr(mIgtCode) & " order by IgtDate,IgtTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrRgPymt inner join PatMast on IndrRgPymt.IgtPttCode=PatMast.PttCode order by IgtDate,IgtTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
''Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mIgtPttRegNoNotZero_bln = True Then
    ...
    If mIgtVchNo <> mIgtVchNo_old Or Val(fcmbIgtVtmName.BoundText) <> mIgtVtmCode_old Or Ctod(dtpIgtDate.Text) <> mIgtDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="IndrRgPymt", mChkFieldName:="IgtVchNo", mChkFieldValue:=mIgtVchNo, mKeyFieldName:="IgtCode", mKeyFieldValue:=mIgtCode, mVtmFieldName:="IgtVtmCode", mVtmFieldValue:=Val(fcmbIgtVtmName.BoundText), mFinYrFieldName:="IgtCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgtDate", mDateFieldValue:=Ctod(dtpIgtDate.Text)) = True Then
            ...
            If FormAddEditMode = True Then
                mIgtVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRgPymt", mFieldName:="IgtVchNo", mVtmFieldName:="IgtVtmCode", mVtmFieldValue:=Val(fcmbIgtVtmName.BoundText), mFinYrFieldName:="IgtCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgtDate", mDateFieldValue:=Ctod(dtpIgtDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
...
fcmbIgtVtmName.BoundText = CStr(clsVGNOPT.GetIndrRgPymtOptBoundCode("IgtVtmCode")): txtIgtVtmName(0).Text = fcmbIgtVtmName.Text
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mIgtVtmCodeOld_lng = .mVtmCode_lng
    clsVGNOPT.UpdateIndrRgPymtOld
...
With tRecset
.open "Select * from IndrRgPymt where IgtCode=" & CStr(mOldIgtCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IgtVtmCode")
...
With tRecset
.open "Select top 1 * from IndrRgPymt where IgtPttCode=" & CStr(mPttCode) & " order by IgtDate desc,IgtVchNo desc,IgtCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrRgPymt", mChkFieldName:="IgtVchNo", mChkFieldValue:=CStr(mIgtVchNo), mVtmFieldName:="IgtVtmCode", mVtmFieldValue:=Val(fcmbIgtVtmName.BoundText), mFinYrFieldName:="IgtCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
If Dir(IndrRgPymtCRLFileFullPath) = "" Then
    ...
    Write #fhand, "Indoor on A/c.Payment", "IndrRgPymt.Rpt", True
    Close #fhand
End If
