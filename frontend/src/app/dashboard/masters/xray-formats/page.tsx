...
export default function XrayFormatsPage() {
  ...
  const loadData = async () => {
    try {
      const res = await fetch(`${API}/api/masters/pathology/test-formats?style=X-Ray`, {
        headers: authHdr(),
      });
      if (res.ok) {
        const data = await res.json();
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>X-Ray Report Formats</h1>
          <p className={styles.subtitle}>Configure templates and formatting layout style for X-Ray diagnostics [XrayRptFmt.frm]</p>
        </div>
      </div>
      ...
      {!entryMode ? (
        <div className={styles.listPanel} style={{ width: '100%' }}>
          <div className={styles.searchBar}>
            ...
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Search X-Ray formats..."
              value={search}
        ...
        <div className={styles.formContainer}>
          <h2 className={styles.formTitle}>
            {editing ? `Edit Format #${editing.TfmCode}: ${editing.TfmName}` : 'New X-Ray Format'}
          </h2>
          <form onSubmit={handleSave} className={styles.gridForm}>
            <div className={styles.formGroup}>
              ...
              <input
                ...
                type="text"
                required
                placeholder="e.g. Chest X-Ray Format"
                value={form.TfmName}
