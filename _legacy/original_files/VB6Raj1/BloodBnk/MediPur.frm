...
Dim dbAcDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbMdsDatabase As New ADODB.Connection, dbBdkDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsPUR As clsMdsMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
Dim clsPRT As Object, clsSIM As clsBdkMasterEntry
Dim mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbVtmName As clsComFlexSearch
Dim fcmbPrtName As Object, fcmbSimName As clsBdkFlexSearch
Dim mPartyMastTblName As String, mPrtNameTitle As String, mPrtNameField As String, mPrtCodeField As String, mPuhPrtCodeField As String, mPrtAraCodeField As String, mPrtStnCodeField As String, mPrtAddrField As String, mPrtTelNoField As String, mPrtSmsNoField As String
        ...
        If .EOF = False And .BOF = False Then
            ...
            Else
                ...
                ElseIf optPartywise.Value = True Then
                    mQryPrtCode = .fields(mPuhPrtCodeField)
                    fcmbPrtName.BoundText = CStr(mQryPrtCode)
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If mFormSysVchType = cCOM_VTYPE_RETNINW Then
        .mPurPttCode_lng = Val(fcmbPrtName.BoundText)
        .mPurPrtCode_lng = 0
    Else
    ...
    With tRecset
    .open "Select * from MedPurchDtl where PurCode=" & CStr(mPurCode) & " and PurICode=" & CStr(mPurICode) & "", dbMdsDatabase, adOpenKeyset, adLockOptimistic
...
Set clsPUR = New clsMdsMultiTranEntry
Set clsPUR.dbAcDatabase = dbAcDatabase
Set clsPUR.dbStDatabase = dbStDatabase
Set clsPUR.dbBdkDatabase = dbBdkDatabase
Set clsPUR.dbMdsDatabase = dbMdsDatabase
Set clsPUR.dbComDatabase = dbComDatabase
clsPUR.blnPurchTran = True
clsPUR.Init

Set clsSIM = New clsBdkMasterEntry
Set clsSIM.dbBdkDatabase = dbBdkDatabase
...
Set fcmbSimName = New clsBdkFlexSearch
Set fcmbSimName.dbBdkDatabase = dbBdkDatabase
...
If mFormSysVchType = cCOM_VTYPE_RETNINW Then
    ...
    mPrtCodeField = "PttCode"
    mPrtNameField = "PttName"
    mPuhPrtCodeField = "PuhPttCode"
    ...
    Set fcmbPrtName = New clsBdkFlexSearch
    Set fcmbPrtName.dbBdkDatabase = dbBdkDatabase
    Set fcmbPrtName.dbComDatabase = dbComDatabase
    fcmbPrtName.blnPatMastListByName = True
    fcmbPrtName.Init
    ' -----------------------
    Set clsPRT = New clsBdkMasterEntry
    Set clsPRT.dbAcDatabase = dbAcDatabase
    Set clsPRT.dbBdkDatabase = dbBdkDatabase
    clsPRT.blnPatMast = True
    clsPRT.Init
    
Else
    mPartyMastTblName = "PartyMast"
    mPrtNameTitle = "Party"
    mPrtCodeField = "PrtCode"
    mPrtNameField = "PrtName"
    mPuhPrtCodeField = "PuhPrtCode"
...
If mRemoteAccess = True Then
    datRecset.open "Select * from MedPurchHdr puh" _
        & " inner join " & mPartyMastTblName & " prt on puh." & mPuhPrtCodeField & " = prt." & mPrtCodeField & "" _
...
CloseDataSource dbMdsDatabase, mCheckOpen:=True
mFormSysVchType = 0
End Sub
...
If Val(fcmbPrtName.BoundText) < 1 Then
    ErrorBox "Invalid " & mPrtNameTitle & " Selection !!!"
    Cancel = True
    txtPrtName.SetFocus
...
With tRecset
.open "Select * from (" & mPartyMastTblName & " prt inner join AreaMast ara on prt." & mPrtAraCodeField & " = ara.AraCode) inner join StsnMast stn on prt." & mPrtStnCodeField & " = stn.StnCode where prt." & mPrtCodeField & " = " & CStr(mPurPrtCode) & "", dbComDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
    txtPrtAddr.Text = .fields(mPrtAddrField)
    txtAraName.Text = .fields("AraName")
    txtStnName.Text = .fields("StnName")
    txtPrtTelNo.Text = .fields(mPrtTelNoField)
    txtPrtSMSNo.Text = .fields(mPrtSmsNoField)
End If
End With
CloseTable tRecset
...
If mFormSysVchType = cCOM_VTYPE_RETNINW Then
    fcmbPrtName.BoundText = CStr(.mPuhPttCode_lng)
Else
    fcmbPrtName.BoundText = CStr(.mPuhPrtCode_lng)
End If
txtPrtName.Text = fcmbPrtName.Text
...
With tRecset
.open "Select top 1 * from MedPurchHdr where PuhVtmCode IN (" & mFormVchTypeList & ") and PuhCmpCode = " & CStr(sFinYrCmpCode) & " order by PuhCode desc", dbMdsDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
With tRecset
.open "Select * from MedPurchDtl where PurCode=" & CStr(mPurCode) & " order by PurSno", dbMdsDatabase, adOpenKeyset, adLockOptimistic
...
If mFormSysVchType = cCOM_VTYPE_RETNINW Then
    .mPuhPttCode_lng = Val(fcmbPrtName.BoundText)
    .mPuhPrtCode_lng = 0
Else
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbMdsDatabase.Execute "Update MedPurchHdr set PuhStkCode=" & CStr(mPurStkCode) & " where PuhCode=" & CStr(mPurCode)
            dbMdsDatabase.Execute "Update MedPurchDtl set PurStkCode=" & CStr(mPurStkCode) & " where PurCode=" & CStr(mPurCode)
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPartywise.Value = True Then
    If optMonthwise.Value = True Then
        ...
        datRecset.open "Select format(PuhDate,'yyyy/MM') as PurYrMonth" _
         ...
         & IIf(mQryPrtCode > 0, " and " & mPuhPrtCodeField & " = " & CStr(mQryPrtCode), "") _
    ...
    ElseIf optPartywise.Value = True Then
        clsListStru.AddFields mExpr:="PrtName", mTitle:=mPrtNameTitle & " Name", mShowItem:=True, mAlign:=flexAlignLeftCenter, mWidth:=3000
        clsListStru.AddFields mExpr:="PurCount", mTitle:="Count", mShowItem:=True, mAlign:=flexAlignRightCenter, mWidth:=1500
        clsListStru.AddFields mExpr:="PurQty_sum", mTitle:="Qty", mShowItem:=True, mAlign:=flexAlignRightCenter, mWidth:=1500
        clsListStru.AddFields mExpr:="PurAmount_sum", mTitle:="Amount", mShowItem:=True, mAlign:=flexAlignRightCenter, mWidth:=1500
        
        datRecset.open "Select prt." & mPrtNameField & ",hdr." & mPuhPrtCodeField & "" _
         & ",Count(PuhCode) as PurCount" _
         & ",Sum(PuhQty) as PurQty_sum" _
         & ",Sum(PuhAmount) as PurAmount_sum" _
         & " from MedPurchHdr hdr inner join " & mPartyMastTblName & " prt on hdr." & mPuhPrtCodeField & " = prt." & mPrtCodeField & "" _
         & " where PuhDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and PuhCode > 0 and PuhVtmCode in (" & mFormVchTypeList & ")" _
         & IIf(mQryPrtCode > 0, " and hdr." & mPuhPrtCodeField & " = " & CStr(mQryPrtCode), "") _
         & " group by prt." & mPrtNameField & ",hdr." & mPuhPrtCodeField & "" _
         & " order by prt." & mPrtNameField & ",hdr." & mPuhPrtCodeField & "" _
         , dbComDatabase, adOpenKeyset, adLockReadOnly
    End If
...
ElseIf optDetailed.Value = True Then
    ...
    clsListStru.AddFields mExpr:="PrtName", mTitle:=mPrtNameTitle & " Name", mAlign:=flexAlignLeftCenter, mWidth:=2500, mShowItem:=True
    ' --------------------------------------
    ...
    datRecset.open "Select hdr.*,prt.*" _
...
If .RecordCount > 0 Then
    Do While .EOF = False
        If optMonthwise.Value = True Or optDatewise.Value = True Or optPartywise.Value = True Then
            ...
            ElseIf optPartywise.Value = True Then
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PrtName")) = .fields(mPrtNameField)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PurCount")) = ToMyNumFmt(.fields("PurCount"), mDecimals:=0)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PurQty_sum")) = ToMyNumFmt(.fields("PurQty_sum"), mDecimals:=-1)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PurAmount_sum")) = ToMyNumFmt(.fields("PurAmount_sum"))
        ...
        ElseIf optDetailed.Value = True Then
            ...
            ' ------------------------------------
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PrtCode")) = .fields(mPrtCodeField)
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PrtName")) = .fields(mPrtNameField)
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PrtAddr")) = .fields(mPrtAddrField)
...
If Val(fcmbSimName.BoundText) <= 0 Then
    ...
    Cancel = True
Else
    If FormAddEditMode = cFORM_ADDMODE Or dtlAddMode = True Or Val(fcmbSimName.BoundText) <> mPurSimCode_old Then
...
With tRecset
'If FormAddEditMode = cFORM_ADDMODE Then
'    .Open "Select top 1 puh.*,vtm.VtmName from MedPurchHdr puh inner join VTypMast vtm on puh.PuhVtmCode=vtm.VtmCode where puh.PuhPrtCode=" & CStr(mPurPrtCode) & " order by puh.PuhDate Desc", dbComDatabase, adOpenKeyset, adLockOptimistic
'Else
    .open "Select top 1 puh.*,vtm.VtmName from MedPurchHdr puh" _
        & " inner join VTypMast vtm on puh.PuhVtmCode = vtm.VtmCode" _
        & " where puh." & mPuhPrtCodeField & " = " & CStr(mPurPrtCode) _
...
.open "Select top 1 * from (" _
    ...
    & "Select PurICode,PurSimCode as SimCode,PurItmCode as ItmCode,PurItrCode as ItrCode,PurSimRefNo as SimRefNo,PurQty as Qty from MedPurchDtl" _
    & " inner join MedPurchHdr on MedPurchDtl.PurCode = MedPurchHdr.PuhCode" _
    & " where PurCode=" & CStr(mPurCode) & IIf(IsMissing(mPurICode) = False, " and PurICode=" & CStr(mPurICode), "") _
    & " and PurRecState IN (" & CStr(cREC_ADDED) & "," & CStr(cREC_EDITED) & "," & CStr(cREC_UNCHANGED) & ")" _
    & " Union All " _
    & "Select SalPurICode as PurICode,SalSimCode as SimCode,SalItmCode as ItmCode,SalItrCode as ItrCode,SalSimRefNo as SimRefNo,(SalQty*-1) as Qty from (MedSaleDtl" _
