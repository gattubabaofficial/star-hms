...
Dim clsRFG As clsBdkMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
...
If mRemoteAccess = True Then
    datRecset.open "Select * from RefCatgMst where RfgCode=" & CStr(mRfgCode) & " order by RfgName", dbBdkDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from RefCatgMst order by RfgName", dbBdkDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtRfgName.Text <> mRfgName_old Then
    If ChkIsDuplicate(dbBdkDatabase, "RefCatgMst", "RfgName", txtRfgName.Text, "RfgCode", Val(mskFormBoundField.Text)) = True Then
