...
Dim fcmbSecurity As clsPmtFlexSearch
...
Set fcmbSecurity = New clsPmtFlexSearch
Set fcmbSecurity.dbCmpDatabase = dbCmpDatabase
fcmbSecurity.blnSecuMastList = True
fcmbSecurity.Init

If mRemoteAccess = True Then
    datRecset.Open "Select * from UserMast where UsrCode=" & CStr(mUsrCode) & " order by UsrName", dbCmpDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from UserMast order by UsrName", dbCmpDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If Trim(txtUsrName.Text) = "" Then
    ...
    Exit Function
End If
If txtSecName.Text = "" Or Val(fcmbSecurity.BoundText) = 0 Then
    ErrorBox "Invalid Security Group Selection !!!"
    txtSecName.SetFocus
    Exit Function
End If
...
Private Sub txtSecName_Change()
If fcmbSecurity.CallFromText_Change = False Then
    fcmbSecurity.CallFromText_Change = True
    If InterActiveChange(txtSecName) = True Then
        fcmbSecurity.UserText = Array(txtSecName.Text, txtSecName.SelStart)
        fcmbSecurity.Show
        txtSecName.Text = fcmbSecurity.Text
        If fcmbSecurity.ListSelected = True Then
            SendKeys "{tab}"
        End If
    End If
    fcmbSecurity.CallFromText_Change = False
End If
...
txtUsrName.Text = ""
fcmbSecurity.BoundText = "": txtSecName.Text = ""
...
fcmbSecurity.BoundText = .fields("UsrSecCode"): txtSecName.Text = fcmbSecurity.Text
...
.mUsrSecCode_lng = Val(fcmbSecurity.BoundText)
If FormAddEditMode = cFORM_ADDMODE Then
    .AddNew
...
clsListStru.AddFields mExpr:="UsrName", mTitle:="User Name", mAlign:=flexAlignLeftCenter, mWidth:=4000, mShowItem:=True
