...
EntryDate = Ctod(dtpObpyDate.Text)
End Property
...
EntryVtmCode = Val(fcmbObpyVtmName.BoundText)
End Property
...
If mRemoteAccess = True Then
    ...
    dtpFromDate.Text = Dtoc(datRecset.fields("ObphDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("ObphDate"))
    AddSmryParaLayer mClear:=True
...
fcmbObpyVtmName.BoundText = CStr(clsHOPT.GetOutdBlPymtOptBoundCode("ObpyVtmCode")): txtObpyVtmName(0).Text = fcmbObpyVtmName.Text
...
Else
    ...
    If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="OutdBlPymtHdr", mChkFieldName:="ObphVchNo", mChkFieldValue:=CStr(mObpyVchNo), mVtmFieldName:="ObphVtmCode", mVtmFieldValue:=Val(fcmbObpyVtmName.BoundText), mFinYrFieldName:="ObphCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
