...
Begin VB.Form frmHolidayAllowed 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   ClientHeight    =   9315
   ClientLeft      =   60
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      Enabled         =   0   'False
      Height          =   315
      ItemData        =   "HoliAlwd.frx":0000
      Left            =   0
      List            =   "HoliAlwd.frx":000D
   ...
   End
End
Attribute VB_Name = "frmHolidayAllowed"
...
Const X2DSGNAME_COL = 3
Const X2DPTNAME_COL = 4
Const X2SFTNAME_COL = 5
Const X2TIMESTR_COL = 6
Const X2STATUSNAME_COL = 7
Const X2ATDCODE_COL = 8
Const X2ATDSNO_COL = 9
Const X2ATDEMPCODE_COL = 10
Const X2ATDDSGCODE_COL = 11
Const X2ATDDPTCODE_COL = 12
Const X2ATDSFTCODE_COL = 13
Const X2ATDTIME_COL = 14
Const X2ATDSTATUS_COL = 15
Const X2ATDRECSTATE_COL = 16
...
Dim tRecset As New ADODB.Recordset, srow As Long, cnt As Integer, mAtdDate As Date, mAtdEmpCode As Long
...
Else
    ...
    If cmbSortingOrder.ListIndex = cLIST_DATEWISE Then
        For mAtdDate = mAtdDate1 To mAtdDate2
            .Open "Select * from EmpMast left join AchdMast on EmpMast.EmpCode=AchdMast.AhCode where EmpJoinDate<=#" & CStr(ToSysDate(mAtdDate)) & "# and EmpJobState=" & CStr(cATNDJSTAT_CONTINUED) & " order by AhName", dbComDatabase, adOpenKeyset, adLockOptimistic
            Do While .EOF = False
    ...
    ElseIf cmbSortingOrder.ListIndex = cLIST_EMPWISE Then
        .Open "Select * from EmpMast left join AchdMast where EmpMast.EmpCode=AchdMast.AhCode where EmpJobState=" & CStr(cATNDJSTAT_CONTINUED) & " order by AhName", dbComDatabase, adOpenKeyset, adLockOptimistic
        Do While .EOF = False
            For mAtdDate = mAtdDate1 To mAtdDate2
                If .Fields("EmpJoinDate") <= mAtdDate Then
                    ...
                    Mfgrd2.TextMatrix(srow, X2ATDCODE_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDSNO_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDEMPCODE_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDDSGCODE_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDDPTCODE_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDSFTCODE_COL) = cATNDSFT_DAY
                    Mfgrd2.TextMatrix(srow, X2ATDTIME_COL) = ""
                    Mfgrd2.TextMatrix(srow, X2ATDSTATUS_COL) = cATNDSTAT_ABSENT
                    Mfgrd2.TextMatrix(srow, X2ATDRECSTATE_COL) = cREC_NEW
...
mDataOk = (IsFinYrDate(Ctod(dtpAtdDate.Text)) = True And Val(fcmbEmpName.BoundText) > 0 And Val(fcmbDsgName.BoundText) > 0 And Val(fcmbDptName.BoundText) > 0 And cmbSftName.ListIndex > -1 And TimeToMin(txtTimeStr.Text) > 0 And cmbStatusName.ListIndex > -1)
...
If Val(fcmbEmpName.BoundText) <= 0 Then
    ...
    Cancel = True
Else
    If dtlAddMode = True Or mAtdEmpCode_old <> Val(fcmbEmpName.BoundText) Then
        clsEMP.GetData Val(fcmbEmpName.BoundText)
        
        fcmbDptName.BoundText = clsEMP.mEmpDptCode_lng: txtDptName.Text = fcmbDptName.Text
        fcmbDsgName.BoundText = clsEMP.mEmpDsgCode_lng: txtDsgName.Text = fcmbDsgName.Text
...
If Day(Ctod(dtpAtdDate.Text)) = clsEMP.mEmpWeeklyOff_int Then
    cmbStatusName.ListIndex = cATNDSTAT_WEEKLYOFF - 1
ElseIf TimeToMin(txtTimeStr.Text) > clsEMP.mEmpHalfDayTime_lng Then
    cmbStatusName.ListIndex = cATNDSTAT_PRESENT - 1
ElseIf TimeToMin(txtTimeStr.Text) > clsEMP.mEmpMinWrkTime_lng Then
