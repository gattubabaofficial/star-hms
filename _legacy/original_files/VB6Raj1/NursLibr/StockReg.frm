...
Dim fcmbPrtName As clsNcbFlexSearch, fcmbSimName As clsNcbFlexSearch
Dim frmTrn As Form
...
If mRemoteAccess = True Then
    'datRecset.Open "Select * from PurchDtl left join PartyMast on PurchDtl.PurPrtCode=PartyMast.PrtCode where PurCode=" & CStr(mPurCode) & " order by PurDate,OpgTime", dbNcbDatabase, adOpenKeyset, adLockOptimistic
Else
    'datRecset.Open "Select * from PurchDtl left join PartyMast on PurchDtl.PurPrtCode=PartyMast.PrtCode order by PurDate,OpgTime", dbNcbDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
