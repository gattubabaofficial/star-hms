...
Dim clsAH As clsAccountHead, clsPRT As clsKgtMasterEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbAhName As clsComFlexSearch, fcmbAraName As clsComFlexSearch, fcmbStnName As clsComFlexSearch
Dim fcmbPrtName As clsKgtFlexSearch
...
If mRemoteAccess = True Then
    datRecset.Open "Select * from PartyMast inner join AchdMast on PartyMast.PrtAhCode=AchdMast.AhCode where PrtCode=" & CStr(mPrtCode) & " order by PrtName", dbComDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from PartyMast inner join AchdMast on PartyMast.PrtAhCode=AchdMast.AhCode order by PrtName", dbComDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
ElseIf MyAddNewKey(Shift, KeyCode) = True Then
    If FormAddEditMode = cFORM_ADDMODE Then
        ...
        If clsPRT.BeginMast(0) = True Then
            mPrtCode = clsPRT.ActiveCode
        Else
            mPrtCode = 0
        End If
...
fcmbAhName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbKgtDatabase, mTableName:="PartyMast", mColumnName:="PrtAhCode", mOrderByColName:="PrtCode", mRtnDefValue:=0))
txtAhName.Text = fcmbAhName.Text

fcmbAraName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbKgtDatabase, mTableName:="PartyMast", mColumnName:="PrtAraCode", mOrderByColName:="PrtCode", mRtnDefValue:=0))
txtAraName.Text = fcmbAraName.Text

fcmbStnName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbKgtDatabase, mTableName:="PartyMast", mColumnName:="PrtStnCode", mOrderByColName:="PrtCode", mRtnDefValue:=0))
