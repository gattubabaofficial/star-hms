...
Begin VB.Form frmRefToMast 
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "RefToMst.frx":0000
      Left            =   0
      List            =   "RefToMst.frx":000D
   ...
   End
End
Attribute VB_Name = "frmRefToMast"
...
Dim mRToName_old As String, mRToAhCode_old As Long
Dim clsAH As clsAccountHead, clsRTO As clsVgnMasterEntry, clsRFG As clsVgnMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbAhName As clsComFlexSearch, fcmbRfgName As clsVgnFlexSearch
...
Public Property Get RToName() As String
RToName = txtRToName.Text
End Property
...
Call SmryList
txtSearch1Text_InterActiveChange txtRToName.Text

''datRecset.Find "RToName='" & txtRToName.Text & "'", Start:=1
''ShowRecActiveFlexRow datRecset, Mfgrd1
...
Set clsRFG = New clsVgnMasterEntry
Set clsRFG.dbVgnDatabase = dbVgnDatabase
...
Set fcmbRfgName = New clsVgnFlexSearch
Set fcmbRfgName.dbVgnDatabase = dbVgnDatabase
fcmbRfgName.blnRefCatgMstList = True
fcmbRfgName.Init

If mRemoteAccess = True Then
    datRecset.open "Select * from RefToMast left join AchdMast on RefToMast.RToAhCode=AchdMast.AhCode where RToCode=" & CStr(mRToCode) & " order by RToName", dbComDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from RefToMast left join AchdMast on RefToMast.RToAhCode=AchdMast.AhCode order by RToName", dbComDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If Val(mskFormBoundField.Text) < 1 And FormAddEditMode = cFORM_EDITMODE Then
    ...
    GoTo EndSub
End If
If Trim(txtRToName.Text) = "" Then
    ErrorBox "Invalid Account Head Name !!!"
    txtRToName.SetFocus
    GoTo EndSub
End If
If txtRToName.Text <> mRToName_old Then
    If ChkIsDuplicate(dbVgnDatabase, "RefToMast", "RToName", txtRToName.Text, "RToCode", Val(mskFormBoundField.Text)) = True Then
...
If Val(fcmbRfgName.BoundText) <= 0 Then
    ...
    GoTo EndSub
End If
If Val(UnMyPerFmt(mskRToShare.Text)) < 0 Then
    ErrorBox "Invalid Share Percentage !!!"
    mskRToShare.SetFocus
    GoTo EndSub
End If
...
Private Sub mskRToShare_GotFocus()
FlashActiveControl mskRToShare, True
End Sub

Private Sub mskRToShare_LostFocus()
FlashActiveControl mskRToShare, False
End Sub

Private Sub mskRToShare_Validate(Cancel As Boolean)
mskRToShare.Text = ToMyPerFmt(mskRToShare.Text)
If Val(UnMyPerFmt(mskRToShare.Text)) < 0 Then
    ErrorBox "Invalid Share Percentage !!!"
    mskRToShare.SetFocus
    Cancel = True
...
Private Sub txtRToAddr_GotFocus()
FlashActiveControl txtRToAddr, True
End Sub

Private Sub txtRToAddr_LostFocus()
FlashActiveControl txtRToAddr, False
End Sub

Private Sub txtRToAddr_Validate(Cancel As Boolean)
txtRToAddr.Text = ToMyWord(txtRToAddr.Text)
End Sub

Private Sub txtRToEmail_GotFocus()
FlashActiveControl txtRToEmail, True
End Sub

Private Sub txtRToEmail_LostFocus()
FlashActiveControl txtRToEmail, False
End Sub
...
txtRToName.Text = ToMyWord(txtRToName.Text)
If Trim(txtRToName.Text) = "" Then
...
If CanUserAddOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsRTO.BeginMast(0) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
...
clsRTO.Clear
mRToCode = clsRTO.ActiveCode
mskFormBoundField.Text = mRToCode
txtRToName.Text = ""
...
txtRToTelNo.Text = ""
txtRToEmail.Text = ""
mskRToShare.Text = ToMyPerFmt(0)
fcmbAhName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbVgnDatabase, mTableName:="RefToMast", mColumnName:="RToAhCode", mOrderByColName:="RToCode", mRtnDefValue:=0))
txtAhName.Text = fcmbAhName.Text
...
mRToName_old = txtRToName.Text
mRToAhCode_old = Val(fcmbAhName.BoundText)
...
If CanUserEditOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsRTO.BeginMast(datRecset.fields("RToCode")) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
FormAddEditMode = cFORM_EDITMODE
With datRecset
mRToCode = .fields("RToCode")
clsRTO.GetData mRToCode
mskFormBoundField.Text = CStr(.fields("RToCode"))
txtRToName.Text = .fields("RToName")
fcmbRfgName.BoundText = CStr(.fields("RToRfgCode")): txtRfgName.Text = fcmbRfgName.Text
txtRToSpeci.Text = .fields("RToSpeci")
txtRToAddr.Text = .fields("RToAddr")
txtRToTelNo.Text = .fields("RToTelNo")
txtRToEmail.Text = .fields("RToEmail")
mskRToShare.Text = ToMyPerFmt(.fields("RToShare"))
fcmbAhName.BoundText = CStr(.fields("RToAhCode")): txtAhName.Text = fcmbAhName.Text
End With
...
Else
    .GetData mRToCode
End If
.mName_str = txtRToName.Text
.mRToRfgCode_lng = Val(fcmbRfgName.BoundText)
.mRToSpeci_str = txtRToSpeci.Text
.mRToAddr_str = txtRToAddr.Text
.mRToTelNo_str = txtRToTelNo.Text
.mRToEmail_str = txtRToEmail.Text
.mRToShare_dbl = Val(UnMyPerFmt(mskRToShare.Text))
.mRToAhCode_lng = Val(fcmbAhName.BoundText)
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    Else
        If mRToAhCode_old <> Val(fcmbAhName.BoundText) Then
            clsAH.UpdateDependency mRToAhCode_old, False
            clsAH.UpdateDependency Val(fcmbAhName.BoundText), True
        End If
    End If
...
Dim mDeleteit As Boolean, mRecAbsPos As Long, mRToAhCode As Long
...
mRToCode = datRecset.fields("RToCode")
mRToAhCode = datRecset.fields("RToAhCode")
mRecAbsPos = datRecset.AbsolutePosition

If clsRTO.CanDeleteMast(mRToCode) = True Then
    txtVoid.SetFocus
    If clsRTO.BeginMast(mRToCode) = True Then
        clsRTO.DeleteMast mRToCode
        clsRTO.UpdateMast mRToCode
        clsRTO.EndMast mRToCode
        ...
        clsAH.UpdateDependency mRToAhCode, False
    End If
    ShowRecActiveFlexRow datRecset, Mfgrd1
    Mfgrd1.SetFocus
Else
...
Private Sub txtRToSpeci_GotFocus()
FlashActiveControl txtRToSpeci, True
End Sub

Private Sub txtRToSpeci_LostFocus()
FlashActiveControl txtRToSpeci, False
End Sub

Private Sub txtRToSpeci_Validate(Cancel As Boolean)
txtRToSpeci.Text = ToMyWord(txtRToSpeci.Text)
End Sub

Private Sub txtRToTelNo_GotFocus()
FlashActiveControl txtRToTelNo, True
End Sub

Private Sub txtRToTelNo_LostFocus()
FlashActiveControl txtRToTelNo, False
End Sub
...
clsListStru.AddFields mExpr:="RToCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="RToName", mTitle:="Referred To Name", mAlign:=1, mWidth:=4000, mShowItem:=True
...
If .RecordCount > 0 Then
    Do While .EOF = False
        clsRFG.GetData .fields("RToRfgCode")
        
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("RToCode")) = .fields("RToCode")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("RToName")) = .fields("RToName")
        ...
        If chkOpBal.Value = 1 Then
            If chkCombinedOpBal.Value = 1 Then
                aAcntBal = clsAH.GetAhOpnBal(.fields("RToAhCode"), mDrCrSeperated:=False)
                
                mBal = aAcntBal(0)
                
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AhOpBal")) = ToMyAcFmt(mBal)
            Else
                aAcntBal = clsAH.GetAhOpnBal(.fields("RToAhCode"), mDrCrSeperated:=True)
                Rem debit balance
                mBal = aAcntBal(0)
        ...
        If chkCurBal.Value = 1 Then
            If chkCombinedCurBal.Value = 1 Then
                aAcntBal = clsAH.GetAhCurBal(.fields("RToAhCode"), sFinYrEndDate, mDrCrSeperated:=False)
                
                mBal = aAcntBal(0)
                
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AhCurBal")) = ToMyAcFmt(mBal)
            Else
                aAcntBal = clsAH.GetAhCurBal(.fields("RToAhCode"), sFinYrEndDate, mDrCrSeperated:=True)
                Rem debit balance
                mBal = aAcntBal(0)
...
Do While datRecset.EOF = False
    Rem opening balance
    aAhOpBal = clsAH.GetAhOpnBal(datRecset.fields("RToAhCode"), mDrCrSeperated:=True)
    mOpnDrAmtTot = mOpnDrAmtTot + aAhOpBal(0)
    mOpnCrAmtTot = mOpnCrAmtTot + aAhOpBal(1)
    
    Rem current balance
    aAhCurBal = clsAH.GetAhCurBal(datRecset.fields("RToAhCode"), sFinYrEndDate, mDrCrSeperated:=True)
...
SearchFlex Mfgrd1, clsListStru.GetItemIndex("RToName"), mText
