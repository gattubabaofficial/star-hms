...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsVgnSnglTranEntry, clsIBL As clsVgnIpdMultiTranEntry, clsIBFD As clsVgnIpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnIpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from IndrBlRefdDtl where IbfdCode=" & CStr(mIbfdCode) & " and IbfdICode=" & CStr(mIbfdICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
clsVGNOPT.blnIndrBlRefdOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrBlRefdOptData
...
Set clsIBFD = New clsVgnIpdMultiTranEntry
Set clsIBFD.dbAcDatabase = dbAcDatabase
Set clsIBFD.dbVgnDatabase = dbVgnDatabase
...
fcmbIbfdVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_BLREFD) & " order by VtmName"
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrBlRefdHdr inner join PatMast on IndrBlRefdHdr.IbfhPttCode=PatMast.PttCode where IbfhCode=" & CStr(mIbfdCode) & " order by IbfhDate,IbfhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrBlRefdHdr inner join PatMast on IndrBlRefdHdr.IbfhPttCode=PatMast.PttCode order by IbfhDate,IbfhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mIbfdPttRegNoNotZero_bln = True Then
...
fcmbIbfdVtmName.BoundText = CStr(clsVGNOPT.GetIndrBlRefdOptBoundCode("IbfdVtmCode")): txtIbfdVtmName(0).Text = fcmbIbfdVtmName.Text
...
With tRecset
.open "Select * from IndrBlRefdDtl where IbfdCode=" & CStr(mIbfdCode) & " order by IbfdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mIbfdVtmCodeOld_lng = .mTrnVtmCode_lng
    clsVGNOPT.UpdateIndrBlRefdOld
...
With tRecset
.open "Select * from IndrBlRefdHdr where IbfhCode=" & CStr(mOldIblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IbfhVtmCode")
...
With tRecset
.open "Select top 1 * from IndrBlRefdHdr where IbfhPttCode=" & CStr(mPttCode) & " order by IbfhDate desc,IbfhVchNo desc,IbfhCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrBlRefdHdr", mChkFieldName:="IbfhVchNo", mChkFieldValue:=CStr(mIbfdVchNo), mVtmFieldName:="IbfhVtmCode", mVtmFieldValue:=Val(fcmbIbfdVtmName.BoundText), mFinYrFieldName:="IbfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
