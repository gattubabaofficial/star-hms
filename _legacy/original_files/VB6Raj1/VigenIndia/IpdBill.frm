...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsVgnSnglTranEntry, clsIGT As clsVgnSnglTranEntry, clsIGF As clsVgnSnglTranEntry, clsIBL As clsVgnIpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnIpdOptionsEntry
...
Dim fcmbIgtVchNo As clsVgnFlexSearch, fcmbIgfVchNo As clsVgnFlexSearch
...
Dim tRecset As New ADODB.Recordset, srow As Integer, mIpgDate As Date, mIpgTime As Long, mIblDate As Date, mIblTime As Long, mUnitCount As Integer, mUnitLocal As Integer
Dim t1Recset As New ADODB.Recordset, t2Recset As New ADODB.Recordset, t3Recset As New ADODB.Recordset, mIbbsFromDate As Date, mIbbsFromTime As Long, mIbbsToDate As Date, mIbbsToTime As Long, mAddBedCharges As Boolean
Dim mAutoInsertServ As Boolean
...
If mIblIpgCode > 0 And mIblPttCode > 0 And mIpdDischState = cIBS_STATE_OCCUPIED And IIf(ChkBillDate() = False, False, IIf(ChkBillTime() = False, False, True)) Then           ''' And Mfgrd2.Rows < (Mfgrd2.FixedRows + 2)
    ...
    If mAutoInsertServ = True Then
        ...
        With tRecset
        .open "Select * from ServMast" _
            & " where SrvAutoInsIpdBill = True" _
            & " and SrvCode NOT IN (Select distinct SrvCode from ServMast as srv inner join BedMast as bdm on srv.SrvCode=bdm.BdmSrvCode" _
            & " where SrvAutoInsIpdBill=True)" _
            & " order by SrvIndex", dbVgnDatabase, adOpenKeyset, adLockOptimistic
            Rem & " inner join IBedState as ibs on bdm.BdmCode=ibs.IbsBdmCode    and IbsIpgCode=" & CStr(mIblIpgCode)  "
        If .EOF = False Then
            txtVoid.SetFocus
            mAutoInsServInProcess = True
            Do While .EOF = False
                t2Recset.open "Select top 1 * from IndrBill" _
                ...
                If t2Recset.EOF = True Then ''' checking pre-existence in same bill
                    t3Recset.open "Select top 1 * from IndrBill" _
                        ...
                        & " and IblRecState IN (" & CStr(cREC_ADDED) & "," & CStr(cREC_EDITED) & "," & CStr(cREC_UNCHANGED) & ")" _
                        & " order by IblDate desc,IbhTime desc,IblCode desc,IblICode desc" _
                        , dbVgnDatabase, adOpenKeyset, adLockOptimistic
        ...
        With tRecset
        .open "Select * from" _
            & " (ServMast as srv inner join BedMast as bdm on srv.SrvCode = bdm.BdmSrvCode)" _
            & " inner join IBedState as ibs on bdm.BdmCode=ibs.IbsBdmCode where SrvAutoInsIpdBill = True and IbsIpgCode = " & CStr(mIblIpgCode) _
            & " order by IbsCode,SrvIndex", dbVgnDatabase, adOpenKeyset, adLockOptimistic
        If .EOF = False Then
            txtVoid.SetFocus
            mAutoInsServInProcess = True
            Do While .EOF = False
                ...
                If t2Recset.EOF = True Then ''' checking pre-existence of auto-inserted service in same bill
                    ...
                    If .fields("SrvAutoInsOnceIpdBill") = False Or t3Recset.EOF = True Then ''' checking pre-existence in other bills of same ipd no when applicable only once
                        ...
                        t1Recset.open "Select top 1 * from IndrBlIbsDtl" _
                            & " where IbbsIpgCode = " & CStr(mIblIpgCode) & " and IbbsIbsCode = " & CStr(.fields("IbsCode")) _
                            & " and IbbsRecState IN (" & CStr(cREC_ADDED) & "," & CStr(cREC_EDITED) & "," & CStr(cREC_UNCHANGED) & ")" _
                            & " order by IbbsToDate desc,IbbsToTime desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
        ...
        If Mfgrd2.Rows > (Mfgrd2.FixedRows + 1) Then
            If MsgBox("Auto-Arrange Services in Respective Index Order", vbYesNoCancel + vbDefaultButton1 + vbQuestion, "Automation") = vbYes Then
                Me.MousePointer = vbHourglass
                ResetIpdBillSrvIndexOrder cnn:=dbVgnDatabase, mIblCode:=mIblCode
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If GetServDctDtlCount(cnn:=dbVgnDatabase, mTableName:="IndrBlDctDtl", mSdcCodeField:="IbdCode", mSdcCode:=.fields("IblCode"), mSdcSubCodeField:="IbdIblCode", mSdcSubCode:=.fields("IblICode")) > 0 Then
...
If cmdSdcOK.Caption = "&OK" And dtlSdcEditCanceled = False Then
    ...
    RefreshDatabase dbVgnDatabase
    With tRecset
    .open "Select * from IndrBlDctDtl where IbdCode=" & CStr(mIblCode) & " and IbdIblCode=" & CStr(mIbdIblCode) & " and IbdICode=" & CStr(mIbdICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
With tRecset
.open "Select top 1 * from IndrBill" _
    & " left join IndrBlHdr on IndrBill.IblCode = IndrBlHdr.IbhCode" _
...
If .EOF = False Then
    If .fields("IblDate") > Ctod(dtpIblDate.Text) Then
        ...
        If IsNull(.fields("IbhCode")) = False Then
            ErrorBox "Prior Bill Exist vide Bill No." & CStr(.fields("IbhVchNo")) & " Dated " & Dtoc(.fields("IblDate")) & " !!!"
        Else
...
clsVGNOPT.blnIndrBillOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrBillOptData
...
Set clsIGF = New clsVgnSnglTranEntry
Set clsIGF.dbAcDatabase = dbAcDatabase
Set clsIGF.dbVgnDatabase = dbVgnDatabase
...
fcmbIblVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_BILL) & " order by VtmName"
fcmbIblVtmName.Init
...
Set fcmbIgtVchNo = New clsVgnFlexSearch
Set fcmbIgtVchNo.dbVgnDatabase = dbVgnDatabase
Set fcmbIgtVchNo.dbComDatabase = dbComDatabase
fcmbIgtVchNo.blnOldIgtListByIgtNo = True
fcmbIgtVchNo.Init

Set fcmbIgfVchNo = New clsVgnFlexSearch
Set fcmbIgfVchNo.dbVgnDatabase = dbVgnDatabase
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrBlHdr inner join PatMast on IndrBlHdr.IbhPttCode=PatMast.PttCode where IbhCode=" & CStr(mIblCode) & " order by IbhDate,IbhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrBlHdr inner join PatMast on IndrBlHdr.IbhPttCode=PatMast.PttCode order by IbhDate,IbhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mIblPttRegNoNotZero_bln = True Then
    ...
    If mIblVchNo <> mIblVchNo_old Or Val(fcmbIblVtmName.BoundText) <> mIblVtmCode_old Or Ctod(dtpIblDate.Text) <> mIblDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="IndrBlHdr", mChkFieldName:="IbhVchNo", mChkFieldValue:=mIblVchNo, mKeyFieldName:="IbhCode", mKeyFieldValue:=mIblCode, mVtmFieldName:="IbhVtmCode", mVtmFieldValue:=Val(fcmbIblVtmName.BoundText), mFinYrFieldName:="IbhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IbhDate", mDateFieldValue:=Ctod(dtpIblDate.Text)) = True Then
            ...
            If FormAddEditMode = cFORM_ADDMODE Then
                mIblVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrBlHdr", mFieldName:="IbhVchNo", mVtmFieldName:="IbhVtmCode", mVtmFieldValue:=Val(fcmbIblVtmName.BoundText), mFinYrFieldName:="IbhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IbhDate", mDateFieldValue:=Ctod(dtpIblDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormIblAcsPermFileName)
            Else
...
ElseIf mIblVchNo > 0 Then
    If Index = 0 Then
        ...
        End If
    Else
        If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrBlHdr", mChkFieldName:="IbhVchNo", mChkFieldValue:=CStr(mIblVchNo), mVtmFieldName:="IbhVtmCode", mVtmFieldValue:=Val(fcmbIblVtmName.BoundText), mFinYrFieldName:="IbhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
With tRecset
.open "Select IpgAdvAmt from IndrReg where IpgCode=" & CStr(mIpgCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
With tRecset
.open "Select sum(IbhAdvAmt) as IbhAdvAmt_sum from IndrBlHdr where IbhIpgCode=" & CStr(mIpgCode) & " and IbhCode<>" & CStr(mIblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
''mIpgDpogAmt = 0
''With tRecset
''.Open "Select sum(IgtDpogAmt) as IgtDpogAmt_sum from IndrRgPymt where IgtIpgCode=" & CStr(mIpgCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''If .EOF = False Then
...
''
''With tRecset
''.Open "Select sum(IbhDpogAmt) as IbhDpogAmt_sum from IndrBlHdr where IbhCode<>" & CStr(mIblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''If .EOF = False Then
...
''mIpgRfugAmt = 0
''With tRecset
''.Open "Select sum(IgfRfugAmt) as IgfRfugAmt_sum from IndrRgRefd where IgfIpgCode=" & CStr(mIpgCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''If .EOF = False Then
...
''
''With tRecset
''.Open "Select sum(IbhRfugAmt) as IbhRfugAmt_sum from IndrBlHdr where IbhCode<>" & CStr(mIblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''If .EOF = False Then
...
fcmbIblVtmName.BoundText = CStr(clsVGNOPT.GetIndrBillOptBoundCode("IblVtmCode")): txtIblVtmName(0).Text = fcmbIblVtmName.Text
...
.open "Select * from IndrBill" _
    ...
    & " order by IblSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If FormAddEditMode = cFORM_ADDMODE Then
    If .AddNewHdr() = True Then
        ...
        dbVgnDatabase.Execute "Delete from IndrBlDctDtl where IbdIblCode IN (Select IblICode from IndrBill as ibl inner join ServMast as srv on ibl.IblSrvCode=srv.SrvCode where srv.SrvMultiDctIncl=False and ibl.IblCode=" & CStr(mIblCode) & ") and IbdCode=" & CStr(mIblCode) & ""
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mIblVtmCodeOld_lng = .mTrnVtmCode_lng
    clsVGNOPT.UpdateIndrBillOld
...
ElseIf optDetailed.Value = True Then
    ...
    datRecset.open "Select IndrBlHdr.*,PatMast.*" _
...
If mIpgCDctCode_old <> clsIPG.mIpgCDctCode_lng Or mPttPcgCode_old <> clsPAT.mPttPcgCode_lng Or mIpgRByCode_old <> clsIPG.mIpgRByCode_lng Or mIpgRToCode_old <> clsIPG.mIpgRToCode_lng Or mIblDate_old <> Ctod(dtpIblDate.Text) Or mIblTime_old <> TimeToMin(txtIblTime_str) Or FormAddEditMode = cFORM_ADDMODE Or dtlAddMode = True Then
    GetBedRateValues cnn:=dbVgnDatabase, mBdmCode:=mBdmCode, mTranDate:=Ctod(dtpIblDate.Text), mTranTime:=TimeToMin(txtIblTime_str.Text), mSrmPttCode:=mIblPttCode, mSrmDctCode:=clsIPG.mIpgCDctCode_lng, mSrmPcgCode:=clsPAT.mPttPcgCode_lng, mSrmRByCode:=clsIPG.mIpgRByCode_lng, mSrmRToCode:=0, mSrmRate:=mIblRate, mSrmDiscPer:=mIblDiscPer, mSrmInfByPer:=mIblInfByPer, mSrmDefByPer:=mIblDefByPer
'    If mIblDiscPer = 0 And clsPAT.mPttDiscAllowed_bln = True Then
'        mIblDiscPer = clsPAT.mPttDiscPer_dbl
'    End If
    If mIblDiscPer = 0 And Val(UnMyPerFmt(mskIblPttDiscPer.Text)) <> 0 Then
...
With tRecset
.open "Select * from IndrBlHdr where IbhCode=" & CStr(mOldIblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IbhVtmCode")
...
With tRecset
.open "Select top 1 * from IndrBlHdr where IbhPttCode=" & CStr(mPttCode) & " order by IbhDate desc,IbhVchNo desc,IbhCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
If dtlAddEditMode = True And dtlAddMode = False And mDataOk = True And InterActiveChange(cmdOK) = True Then
    ...
    If clsSRV.mSrvMultiDctIncl_bln = False Then
        If GetServDctDtlCount(cnn:=dbVgnDatabase, mTableName:="IndrBlDctDtl", mSdcCodeField:="IbdCode", mSdcCode:=mIblCode, mSdcSubCodeField:="IbdIblCode", mSdcSubCode:=Val(mskIblICode.Text)) > 0 Then
...
With tRecset
.open "Select IndrBlPymtDtl.*,IndrBlPymtHdr.IbphVtmCode,IndrBlPymtHdr.IbphVchNo from IndrBlPymtDtl inner join IndrBlPymtHdr on IndrBlPymtDtl.IbpyCode=IndrBlPymtHdr.IbphCode where IbpyIbhCode=" & CStr(mIblCode) & " order by IbpyDate", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select IndrBlRefdDtl.*,IndrBlRefdHdr.IbfhVtmCode,IndrBlRefdHdr.IbfhVchNo from IndrBlRefdDtl inner join IndrBlRefdHdr on IndrBlRefdDtl.IbfdCode=IndrBlRefdHdr.IbfhCode where IbfdIbhCode=" & CStr(mIblCode) & " order by IbfdDate", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
mIpdDischState = GetIpdDischState(cnn:=dbVgnDatabase, mIpgCode:=mIblIpgCode)
If mIpdDischState <> cIBS_STATE_OCCUPIED Then
...
With tRecset
.open "Select * from IndrBlDctDtl where IbdCode=" & CStr(mIblCode) & " and IbdIblCode=" & CStr(mIbdIblCode) & " order by IbdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
If cmdDpogOK.Caption = "&OK" And dtlDpogEditCanceled = False Then
    ...
    RefreshDatabase dbVgnDatabase
    With tRecset
    .open "Select * from IndrBlDpogDtl where IbgdCode=" & CStr(mIblCode) & " and IbgdICode=" & CStr(mIbgdICode) & " order by IbgdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
With tRecset
.open "Select * from IndrBlDpogDtl where IbgdCode=" & CStr(mIblCode) & " order by IbgdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
If cmdRfugOk.Caption = "&OK" And dtlRfugEditCanceled = False Then
    ...
    RefreshDatabase dbVgnDatabase
    With tRecset
    .open "Select * from IndrBlRfugDtl where IbgfCode=" & CStr(mIblCode) & " and IbgfICode=" & CStr(mIbgfICode) & " order by IbgfSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
With tRecset
.open "Select * from IndrBlRfugDtl where IbgfCode=" & CStr(mIblCode) & " order by IbgfSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
