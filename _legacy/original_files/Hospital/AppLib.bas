...
Public Function ChkDuplPatByContNo(ByRef cnn As ADODB.Connection, ByVal mPttCode As Long, ByVal mContNo As String) As Boolean
Dim tRecset As ADODB.Recordset, mRtnval As Boolean, ary1 As Variant, mLoopCnt As Integer, mContNoStr As String

mRtnval = False
If Trim(mContNo) <> "" Then
    If InStr(mContNo, ";") > 0 Then
        ary1 = Split(mContNo, ";")
    Else
        ary1 = Split(mContNo, ",")
    End If
    For mLoopCnt = LBound(ary1) To UBound(ary1)
        mContNoStr = ary1(mLoopCnt)
        Set tRecset = cnn.Execute("Select * from PatMast" _
         & " where (PttTelNo LIKE '%" & Trim(mContNoStr) & "%' or PttSmsNo LIKE '%" & Trim(mContNoStr) & "%')" _
         & " and PttCode <> " & CStr(mPttCode) & "")
        With tRecset
        If .EOF = False Then
            AlertBox "Tel.No. or SMS No. already Exist for " _
            & vbCrLf & .fields("PttName") & " (" & CStr(.fields("PttRegNo")) & ")" _
            & vbCrLf & .fields("PttRefName") _
            & vbCrLf & .fields("PttAddr") _
            & vbCrLf & .fields("PttTelNo") & "; " & .fields("PttSmsNo")
        End If
        End With
        CloseTable tRecset
    Next mLoopCnt
