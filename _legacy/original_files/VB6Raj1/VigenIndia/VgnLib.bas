Attribute VB_Name = "VgnLibrary"

Option Explicit

Public sVgnDatabaseName As String
...
Public Const cVGNSALEPYMT_CRLFILENAME = "VSalPymt.Crl"
...
Public Const cVGN_VTYPE_OUTDOOR_REG = 101
Rem -------------- Store
Public Const cVGN_VTYPE_MEDPURCH = 201
Public Const cVGN_VTYPE_MEDSALE = 202
Public Const cVGN_VTYPE_MEDRETNINW = 203
Public Const cVGN_VTYPE_MEDRETNOUTW = 204
Public Const cVGN_VTYPE_MEDSALE_PYMT = 205


Rem Crystal Report List file names
Rem outdoor files
Public Const cOUTDREG_CRLFILENAME = "OutdReg.Crl"
Rem store report list files
Public Const cOPDREG_CRLFILENAME = "OpdRegRp.Crl"
...
Public Sub InitVgnVar()
' --------------------------------------
sVgnDatabaseName = "VigenInd.Mdb"
...
' ------------------------
clsVType.AddType cVGN_VTYPE_OUTDOOR_REG, "Outdoor Registration"
Rem ---------------- store ----------------------------------------------------------------
clsVType.AddType cVGN_VTYPE_MEDPURCH, "Store Purchase"
clsVType.AddType cVGN_VTYPE_MEDSALE, "Store Sales"
clsVType.AddType cVGN_VTYPE_MEDRETNINW, "Store Sales Return"
clsVType.AddType cVGN_VTYPE_MEDRETNOUTW, "Store Purchase Return"
clsVType.AddType cVGN_VTYPE_MEDSALE_PYMT, "Store Sales Payment"

Rem restoring Vigen system parameter options
Call InitVgnServRateSysParaOpt

End Sub

Public Sub InitVgnServRateSysParaOpt()
'Dim clsVGNOPT As New clsVgnOpdOptionsEntry
'
'clsVGNOPT.blnServRateSysOptMast = True
'clsVGNOPT.Init
'If sCmpDataPath = "" Then
'    clsVGNOPT.Clear
'Else
'    clsVGNOPT.GetServRateSysOptData
'End If
'
'sExtServRateSys = clsVGNOPT.mExtServRateSys_bln: sDctwseServRateSys = clsVGNOPT.mDctwseServRateSys_bln: sPcgwseServRateSys = clsVGNOPT.mPcgwseServRateSys_bln: sRBywseServRateSys = clsVGNOPT.mRBywseServRateSys_bln: sRTowseServRateSys = clsVGNOPT.mRTowseServRateSys_bln: sTimewseServRateSys = clsVGNOPT.mTimewseServRateSys_bln
'sExtServDiscSys = clsVGNOPT.mExtServDiscSys_bln: sDctwseServDiscSys = clsVGNOPT.mDctwseServDiscSys_bln: sPcgwseServDiscSys = clsVGNOPT.mPcgwseServDiscSys_bln: sRBywseServDiscSys = clsVGNOPT.mRBywseServDiscSys_bln: sRTowseServDiscSys = clsVGNOPT.mRTowseServDiscSys_bln: sTimewseServDiscSys = clsVGNOPT.mTimewseServDiscSys_bln
'sExtServBargSys = clsVGNOPT.mExtServBargSys_bln: sDctwseServBargSys = clsVGNOPT.mDctwseServBargSys_bln: sPcgwseServBargSys = clsVGNOPT.mPcgwseServBargSys_bln: sRBywseServBargSys = clsVGNOPT.mRBywseServBargSys_bln: sRTowseServBargSys = clsVGNOPT.mRTowseServBargSys_bln: sTimewseServBargSys = clsVGNOPT.mTimewseServBargSys_bln
'sExtServShareSys = clsVGNOPT.mExtServShareSys_bln: sDctwseServShareSys = clsVGNOPT.mDctwseServShareSys_bln: sPcgwseServShareSys = clsVGNOPT.mPcgwseServShareSys_bln: sRBywseServShareSys = clsVGNOPT.mRBywseServShareSys_bln: sRTowseServShareSys = clsVGNOPT.mRTowseServShareSys_bln: sTimewseServShareSys = clsVGNOPT.mTimewseServShareSys_bln
'Set clsVGNOPT = Nothing

End Sub

Public Sub RemoveVgnNullValue(Optional ByRef FormProgStatus As frmProgressStatus)
Dim dbVgnDatabase As New ADODB.Connection, dbVgnCatalog As New ADOX.Catalog
...
If Not FormProgStatus Is Nothing Then
    If FormProgStatus.Visible = True Then
        FormProgStatus.pbar1.Min = 0: FormProgStatus.pbar1.Max = dbVgnCatalog.Tables.count
    End If
End If

dbVgnDatabase.BeginTrans
For Each mTable In dbVgnCatalog.Tables
    If mTable.Type = "TABLE" Then
        If Not FormProgStatus Is Nothing Then
            If FormProgStatus.Visible = True Then
                FormProgStatus.ShowNextStatus dbVgnCatalog.ActiveConnection.Properties("Data Source Name") & vbCrLf & "Table: " & mTable.Name
            End If
        End If
        
        mTableName = mTable.Name
        With tRecset
        .open "Select * from " & mTableName & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
Public Property Get VgnSalePymtCRLFileFullPath(Optional ByVal mFilePath, Optional ByVal mFileName) As String
mFilePath = IIf(IsMissing(mFilePath) = False, mFilePath, sCmpDataPath)  ''' sAppPath
mFileName = IIf(IsMissing(mFileName) = False, mFileName, cVGNSALEPYMT_CRLFILENAME)
VgnSalePymtCRLFileFullPath = AddBS(mFilePath) & mFileName
...
Public Property Get OpdRegRepCRLFileFullPath(Optional ByVal mFilePath, Optional ByVal mFileName) As String
mFilePath = IIf(IsMissing(mFilePath) = False, mFilePath, sCmpDataPath)  ''' sAppPath
mFileName = IIf(IsMissing(mFileName) = False, mFileName, cOPDREG_CRLFILENAME)
OpdRegRepCRLFileFullPath = AddBS(mFilePath) & mFileName

End Property

Public Property Get dbVgnDatabaseFullPath(Optional ByVal mDbPath) As String
mDbPath = IIf(IsMissing(mDbPath) = False, mDbPath, sCmpDataPath)
dbVgnDatabaseFullPath = AddBS(mDbPath) & sVgnDatabaseName

End Property

Public Function OpenVgnDataSource(ByRef db As ADODB.Connection, Optional ByVal mDbPath, Optional ByVal mUserId As String, Optional ByVal mPwd As String, Optional ByVal mExclusiveMode As Boolean) As Variant
mDbPath = AddBS(IIf(IsMissing(mDbPath) = False, mDbPath, sCmpDataPath))
db.ConnectionString = "Provider=" & DbProvider & ";Data Source=" & dbVgnDatabaseFullPath(mDbPath)
...
Dim clsSRV As New clsVgnMasterEntry, clsSGP As New clsVgnMasterEntry, clsPCG As New clsVgnMasterEntry, clsPTT As New clsVgnMasterEntry
Dim tRecset As New ADODB.Recordset
...
Set clsSRV.dbVgnDatabase = cnn
clsSRV.Init

clsSGP.blnServGrpMast = True
Set clsSGP.dbVgnDatabase = cnn
clsSGP.Init

clsPCG.blnPatCatgMast = True
Set clsPCG.dbVgnDatabase = cnn
clsPCG.Init

clsPTT.blnPatMast = True
Set clsPTT.dbVgnDatabase = cnn
clsPTT.Init
...
'Dim clsBDM As New clsVgnMasterEntry, clsSRV As New clsVgnMasterEntry, clsSGP As New clsVgnMasterEntry, clsPCG As New clsVgnMasterEntry, clsPTT As New clsVgnMasterEntry
'Dim tRecset As New ADODB.Recordset
'
'Dim mDctwseRateSys As Boolean, mPcgwseRateSys As Boolean, mRBywseRateSys As Boolean, mRTowseRateSys As Boolean, mTimewseRateSys As Boolean
'Dim mDctwseDiscSys As Boolean, mPcgwseDiscSys As Boolean, mRBywseDiscSys As Boolean, mRTowseDiscSys As Boolean, mTimewseDiscSys As Boolean
