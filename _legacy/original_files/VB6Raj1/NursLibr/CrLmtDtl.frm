...
Dim dbNcbDatabase As New ADODB.Connection, dbComDatabase As New ADODB.Connection
...
Dim clsCLD As clsNcbMasterEntry
Dim mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbPgmName As clsNcbFlexSearch, fcmbPrtName As clsNcbFlexSearch, fcmbSigName As clsNcbFlexSearch, fcmbSimName As clsNcbFlexSearch
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    .Open "Select * from (((CrLimitDtl cld" _
        ...
        & " order by CldICode", dbNcbDatabase, adOpenKeyset, adLockOptimistic
...
Set clsCLD = New clsNcbMasterEntry
Set clsCLD.dbNcbDatabase = dbNcbDatabase
clsCLD.blnCrLimitDtlMast = True
clsCLD.Init

Set fcmbPgmName = New clsNcbFlexSearch
Set fcmbPgmName.dbNcbDatabase = dbNcbDatabase
...
Set fcmbSigName = New clsNcbFlexSearch
Set fcmbSigName.dbNcbDatabase = dbNcbDatabase
...
With tRecset
.Open "Select * from CrLimitDtl where CldPgmCode = " & CStr(Val(fcmbPgmName.BoundText)) & " and CldPrtCode = " & CStr(Val(fcmbPrtName.BoundText)) & " and CldSigCode = " & CStr(Val(fcmbSigName.BoundText)) & " and CldSimCode = " & CStr(Val(fcmbSimName.BoundText)) & " and CldICode <> " & Val(Mfgrd2.TextMatrix(Mfgrd2.Row, X2CLDICODE_COL)) & " order by CldICode", dbNcbDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
