...
Dim clsDIG As clsBdkMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
...
If mRemoteAccess = True Then
    datRecset.open "Select * from DiagMast where DigCode=" & CStr(mDigCode) & " order by DigName", dbBdkDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from DiagMast order by DigName", dbBdkDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtDigName.Text <> mDigName_old Then
    If ChkIsDuplicate(dbBdkDatabase, "DiagMast", "DigName", txtDigName.Text, "DigCode", Val(mskFormBoundField.Text)) = True Then
