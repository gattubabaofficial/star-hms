...
export default function UserRights() {
  ...
  useEffect(() => {
    fetch('http://localhost:3001/api/masters/user-roles')
      .then(res => res.json())
      .then(data => {
        setRoles(data);
        if (data.length > 0) handleRoleSelect(data[0].UrlCode);
      });
  }, []);
  ...
  const handleRoleSelect = (roleCode: number) => {
    ...
    fetch(`http://localhost:3001/api/masters/user-rights/${roleCode}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`http://localhost:3001/api/masters/user-rights/${selectedRoleCode}`, {
        method: 'POST',
