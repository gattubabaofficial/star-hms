...
export default function IPDOnAccountRefundPage() {
  ...
  const loadRefunds = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/refunds?from_date=${fromDate}&to_date=${toDate}`);
  ...
  const totRefunded = refunds.reduce((s, r) => s + (r.RfugAmt || 0), 0);
      ...
      {showModal && (
        <div className={styles.modalOverlay} style={{ zIndex: 1000 }}>
          <div className={styles.modalContent} style={{ maxWidth: 440 }}>
            <div className={styles.modalHeader}>
              <h3 style={{ fontSize: 16, fontWeight: 700 }}>Issue Deposit Refund</h3>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)" }}>
              No on-account refund records found.
            </div>
          ) : (
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1200, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredRefunds.map(r => {
                  const selected = selectedRow?.IgfCode === r.IgfCode;
                  return (
                    <tr
                      key={r.IgfCode}
