...
Begin VB.Form frmEmployeeMast 
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "EmpMast.frx":0000
      Left            =   0
      List            =   "EmpMast.frx":000D
...
Dim mEmpName_old As String, mEmpAhCode_old As Long
...
Dim fcmbAhName As clsComFlexSearch, fcmbDptName As clsPyrFlexSearch, fcmbDsgName As clsPyrFlexSearch
...
Call SmryList
txtSearch1Text_InterActiveChange txtEmpName.Text
...
With tRecset
.open "Select top 1 * from Attendance where AtdEmpCode=" & CStr(mEmpCode) & " and AtdDate<#" & CStr(ToSysDate(Ctod(dtpEmpJoinDate.Text))) & "#", dbPyrDatabase, adOpenKeyset, adLockOptimistic
...
fcmbDptName.blnDeptMastList = True
fcmbDptName.Init

Set fcmbDsgName = New clsPyrFlexSearch
Set fcmbDsgName.dbPyrDatabase = dbPyrDatabase
fcmbDsgName.blnDesgMastList = True
fcmbDsgName.Init

If mRemoteAccess = True Then
    datRecset.open "Select * from EmpMast left join AchdMast on EmpMast.EmpAhCode=AchdMast.AhCode where EmpCode=" & CStr(mEmpCode) & " order by EmpName", dbComDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from EmpMast left join AchdMast on EmpMast.EmpAhCode=AchdMast.AhCode order by EmpName", dbComDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtEmpName.Text <> mEmpName_old Then
    If ChkIsDuplicate(dbPyrDatabase, "EmpMast", "EmpName", txtEmpName.Text, "EmpCode", Val(mskFormBoundField.Text)) = True Then
...
Private Sub txtDptName_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    Call txtDptName_Change
End If
End Sub

Private Sub txtDsgName_KeyDown(KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    Call txtDsgName_Change
End If
...
txtEmpName.Text = ToMyWord(txtEmpName.Text)
If Trim(txtEmpName.Text) = "" Then
...
txtEmpName.Text = ""
fcmbAhName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbPyrDatabase, mTableName:="EmpMast", mColumnName:="EmpAhCode", mOrderByColName:="EmpCode", mRtnDefValue:=0))
txtAhName.Text = fcmbAhName.Text
...
mEmpAhCode_old = Val(fcmbAhName.BoundText)
...
fcmbAhName.BoundText = CStr(.fields("EmpAhCode")): txtAhName.Text = fcmbAhName.Text
...
.mEmpAhCode_lng = Val(fcmbAhName.BoundText)
.mEmpDptCode_lng = Val(fcmbDptName.BoundText)
.mEmpDsgCode_lng = Val(fcmbDsgName.BoundText)
.mEmpJoinDate_dt = Ctod(dtpEmpJoinDate.Text)
.mEmpBSalary_dbl = Val(UnMyNumFmt(mskEmpBSalary.Text))
.mEmpHRAPer_dbl = Val(UnMyPerFmt(mskEmpHRAPer.Text))
.mEmpHRAAmt_dbl = Val(UnMyNumFmt(mskEmpHRAAmt.Text))
.mEmpDAPer_dbl = Val(UnMyPerFmt(mskEmpDAPer.Text))
.mEmpDAAmt_dbl = Val(UnMyNumFmt(mskEmpDAAmt.Text))
.mEmpSelfPFPer_dbl = Val(UnMyPerFmt(mskEmpSelfPFPer.Text))
.mEmpSelfPFAmt_dbl = Val(UnMyNumFmt(mskEmpSelfPFAmt.Text))
.mEmpCoPFPer_dbl = Val(UnMyPerFmt(mskEmpCoPFPer.Text))
.mEmpCoPFAmt_dbl = Val(UnMyNumFmt(mskEmpCoPFAmt.Text))
.mEmpSelfESICPer_dbl = Val(UnMyPerFmt(mskEmpSelfESICPer.Text))
.mEmpSelfESICAmt_dbl = Val(UnMyNumFmt(mskEmpSelfESICAmt.Text))
.mEmpNSalary_dbl = Val(UnMyNumFmt(mskEmpNSalary.Text))
.mEmpWrkTime_lng = TimeToMin(txtWrkTimeStr.Text)
.mEmpHalfDayTime_lng = TimeToMin(txtHalfDayTimeStr.Text)
.mEmpMinWrkTime_lng = TimeToMin(txtMinWrkTimeStr.Text)
.mEmpAddOT_bln = (chkEmpAddOT.Value = vbChecked)
.mEmpCutUT_bln = (chkEmpCutUT.Value = vbChecked)
.mEmpMnLeaves_dbl = Val(UnMyNumFmt(mskEmpMnLeaves.Text))
.mEmpWeeklyOff_int = Val(Right(cmbWeeklyOff.List(cmbWeeklyOff.ListIndex), 1))
.mEmpJobState_int = Val(Right(cmbJobStateName.List(cmbJobStateName.ListIndex), 1))
.mEmpSftCode_int = Val(Right(cmbSftName.List(cmbSftName.ListIndex), 1))
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    Rem refreshing required if new account created done at form_activate
    ...
    Else
        If mEmpAhCode_old <> Val(fcmbAhName.BoundText) Then
            clsAH.UpdateDependency mEmpAhCode_old, False
            clsAH.UpdateDependency Val(fcmbAhName.BoundText), True
        End If
    End If
...
Dim mDeleteit As Boolean, mRecAbsPos As Long, mEmpAhCode As Long
...
mEmpAhCode = datRecset.fields("EmpAhCode")
mRecAbsPos = datRecset.AbsolutePosition

If clsEMP.CanDeleteMast(mEmpCode) = True Then
    txtVoid.SetFocus
    If clsEMP.BeginMast(mEmpCode) = True Then
        clsEMP.DeleteMast mEmpCode
        clsEMP.UpdateMast mEmpCode
        ...
        clsAH.UpdateDependency mEmpAhCode, False
    End If
    ShowRecActiveFlexRow datRecset, Mfgrd1
    Mfgrd1.SetFocus
Else
...
clsListStru.AddFields mExpr:="EmpName", mTitle:="Employee Name", mAlign:=1, mWidth:=4000, mShowItem:=True
...
clsListStru.AddFields mExpr:="DptName", mTitle:="Department", mAlign:=1, mWidth:=2000, mShowItem:=True
clsListStru.AddFields mExpr:="DsgName", mTitle:="Designation", mAlign:=1, mWidth:=2000, mShowItem:=True
clsListStru.AddFields mExpr:="AhName", mTitle:="Account Name", mAlign:=1, mWidth:=2000, mShowItem:=True
clsListStru.AddFields mExpr:="BSalary", mTitle:="Basic Salary", mAlign:=7, mWidth:=2000, mShowItem:=True
...
If .RecordCount > 0 Then
    Do While .EOF = False
        ...
        If chkOpBal.Value = 1 Then
            If chkCombinedOpBal.Value = 1 Then
                aAcntBal = clsAH.GetAhOpnBal(.fields("EmpAhCode"), mDrCrSeperated:=False)
                
                mBal = aAcntBal(0)
                
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AhOpBal")) = ToMyAcFmt(mBal)
            Else
                aAcntBal = clsAH.GetAhOpnBal(.fields("EmpAhCode"), mDrCrSeperated:=True)
                Rem debit balance
                mBal = aAcntBal(0)
        ...
        If chkCurBal.Value = 1 Then
            If chkCombinedCurBal.Value = 1 Then
                aAcntBal = clsAH.GetAhCurBal(.fields("EmpAhCode"), sFinYrEndDate, mDrCrSeperated:=False)
                
                mBal = aAcntBal(0)
                
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AhCurBal")) = ToMyAcFmt(mBal)
            Else
                aAcntBal = clsAH.GetAhCurBal(.fields("EmpAhCode"), sFinYrEndDate, mDrCrSeperated:=True)
                Rem debit balance
                mBal = aAcntBal(0)
...
Do While datRecset.EOF = False
    Rem opening balance
    aAhOpBal = clsAH.GetAhOpnBal(datRecset.fields("EmpAhCode"), mDrCrSeperated:=True)
    mOpnDrAmtTot = mOpnDrAmtTot + aAhOpBal(0)
    mOpnCrAmtTot = mOpnCrAmtTot + aAhOpBal(1)
    
    Rem current balance
    aAhCurBal = clsAH.GetAhCurBal(datRecset.fields("EmpAhCode"), sFinYrEndDate, mDrCrSeperated:=True)
...
SearchFlex Mfgrd1, clsListStru.GetItemIndex("EmpName"), mText
