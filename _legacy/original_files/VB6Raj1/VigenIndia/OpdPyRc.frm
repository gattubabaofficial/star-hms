...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsVgnSnglTranEntry, clsORC As clsVgnOpdMultiTranEntry, clsOPY As clsVgnOpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnOpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from OutdPymtDtl where OpyCode=" & CStr(mOpyCode) & " and OpyICode=" & CStr(mOpyICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
Set clsOPY = New clsVgnOpdMultiTranEntry
Set clsOPY.dbAcDatabase = dbAcDatabase
Set clsOPY.dbVgnDatabase = dbVgnDatabase
...
fcmbOpyVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_OUTDOOR_PYMT) & " order by VtmName"
fcmbOpyVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdPymtHdr inner join PatMast on OutdPymtHdr.OphPttCode=PatMast.PttCode where OphCode=" & CStr(mOpyCode) & " order by OphDate,OphTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from OutdPymtHdr inner join PatMast on OutdPymtHdr.OphPttCode=PatMast.PttCode order by OphDate,OphTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mOpyPttRegNoNotZero_bln = True Then
...
fcmbOpyVtmName.BoundText = CStr(clsVGNOPT.GetOutdPymtOptBoundCode("OpyVtmCode")): txtOpyVtmName(0).Text = fcmbOpyVtmName.Text
...
With tRecset
.open "Select * from OutdPymtDtl where OpyCode=" & CStr(mOpyCode) & " order by OpySno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mOpyVtmCodeOld_lng = .mTrnVtmCode_lng
...
With tRecset
.open "Select * from OutdPymtHdr where OphCode=" & CStr(mOldOrcCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("OphVtmCode")
...
With tRecset
.open "Select top 1 * from OutdPymtHdr where OphPttCode=" & CStr(mPttCode) & " order by OphDate desc,OphVchNo desc,OphCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="OutdPymtHdr", mChkFieldName:="OphVchNo", mChkFieldValue:=CStr(mOpyVchNo), mVtmFieldName:="OphVtmCode", mVtmFieldValue:=Val(fcmbOpyVtmName.BoundText), mFinYrFieldName:="OphCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
