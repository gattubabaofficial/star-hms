...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsHmsSnglTranEntry, clsORC As clsHmsOpdMultiTranEntry, clsOFD As clsHmsOpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsHOPT As clsHmsOpdOptionsEntry
...
EntryDate = Ctod(dtpOfdDate.Text)
End Property
...
EntryVtmCode = Val(fcmbOfdVtmName.BoundText)
End Property
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from OutdRefdDtl where OfdCode=" & CStr(mOfdCode) & " and OfdICode=" & CStr(mOfdICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
clsHOPT.blnOutdRefdOptMast = True
clsHOPT.Init
clsHOPT.GetOutdRefdOptData
...
Set clsOFD = New clsHmsOpdMultiTranEntry
Set clsOFD.dbAcDatabase = dbAcDatabase
Set clsOFD.dbHmsDatabase = dbHmsDatabase
...
fcmbOfdVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_OUTDOOR_REFD) & " order by VtmName"
fcmbOfdVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdRefdHdr inner join PatMast on OutdRefdHdr.OfhPttCode = PatMast.PttCode where OfhCode = " & CStr(mOfdCode) & " order by OfhDate,OfhTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("OfhDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("OfhDate"))
    AddSmryParaLayer mClear:=True
    
Else
    datRecset.open "Select * from OutdRefdHdr inner join PatMast on OutdRefdHdr.OfhPttCode=PatMast.PttCode order by OfhDate,OfhTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mOfdPttRegNoNotZero_bln = True Then
...
fcmbOfdVtmName.BoundText = CStr(clsHOPT.GetOutdRefdOptBoundCode("OfdVtmCode")): txtOfdVtmName(0).Text = fcmbOfdVtmName.Text
...
With tRecset
.open "Select * from OutdRefdDtl where OfdCode=" & CStr(mOfdCode) & " order by OfdSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mOfdVtmCodeOld_lng = .mTrnVtmCode_lng
    clsHOPT.UpdateOutdRefdOld
...
With tRecset
.open "Select * from OutdRefdHdr where OfhCode=" & CStr(mOldOrcCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("OfhVtmCode")
...
With tRecset
.open "Select top 1 * from OutdRefdHdr where OfhPttCode=" & CStr(mPttCode) & " order by OfhDate desc,OfhVchNo desc,OfhCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="OutdRefdHdr", mChkFieldName:="OfhVchNo", mChkFieldValue:=CStr(mOfdVchNo), mVtmFieldName:="OfhVtmCode", mVtmFieldValue:=Val(fcmbOfdVtmName.BoundText), mFinYrFieldName:="OfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
