...
export default function IPDBillPaymentPage() {
  ...
  const loadPayments = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/bill-payments?from_date=${fromDate}&to_date=${toDate}`);
  ...
  const totPaid = payments.reduce((s, r) => s + (r.DepoAmt || 0), 0);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 110px)", gap: 10 }}>
      ...
      {showModal && (
        <div className={styles.modalOverlay} style={{ zIndex: 1000 }}>
          <div className={styles.modalContent} style={{ maxWidth: 440 }}>
            <div className={styles.modalHeader}>
              <h3 style={{ fontSize: 16, fontWeight: 700 }}>Collect Bill Payment</h3>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)" }}>
          IPD Bill Payments List (Settlement)
        </span>
      </div>
      ...
      <div className={styles.sectionBox} style={{ padding: 0, flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1200, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredPayments.map(p => {
                  const selected = selectedRow?.IbphCode === p.IbphCode;
                  return (
                    <tr
                      key={p.IbphCode}
        ...
        {!loading && filteredPayments.length > 0 && (
          <div style={{
            ...
            <div>GRAND TOTAL</div>
            <div style={{ textAlign: "right" }}>₹{totPaid.toFixed(2)}</div>
            <div></div>
          </div>
        )}
      </div>
