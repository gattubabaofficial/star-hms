...
Begin VB.Form frmIndoorRgRefund 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   ClientHeight    =   10860
   ClientLeft      =   60
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      Enabled         =   0   'False
      Height          =   315
      ItemData        =   "IpdRgRfd.frx":0000
      Left            =   0
      List            =   "IpdRgRfd.frx":000D
   ...
   Begin VB.Frame frFormDtl 
      ...
      Begin VB.Frame frFooter 
         ...
         Begin VB.Label Label2 
            ...
            Width           =   1815
         End
         Begin VB.Label lblOldIgfCodeDesc 
            Appearance      =   0  'Flat
            BackColor       =   &H80000005&
            BeginProperty Font 
               Name            =   "Verdana"
      ...
      Begin VB.Frame frHeader 
         ...
         Begin VB.TextBox txtPttSex 
            ...
            Width           =   1215
         End
         Begin VB.TextBox mskIgfIpgCode 
            Alignment       =   1  'Right Justify
            Appearance      =   0  'Flat
            BeginProperty DataFormat 
               Type            =   1
            ...
            Width           =   1455
         End
         Begin VB.TextBox txtIgfTime_str 
            Appearance      =   0  'Flat
            BeginProperty DataFormat 
               Type            =   0
               Format          =   "0"
         ...
         Begin VB.TextBox txtFDigName 
            ...
            Width           =   3975
         End
         Begin VB.TextBox mskIgfPttCode 
            Alignment       =   1  'Right Justify
            Appearance      =   0  'Flat
            BeginProperty DataFormat 
               Type            =   1
   ...
   Begin VB.Label lblFormHeading 
      ...
      BackColor       =   &H00E0E0E0&
      BorderStyle     =   1  'Fixed Single
      Caption         =   "Indoor On Account Refunds"
      BeginProperty Font 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmIndoorRgRefund"
...
Dim mIgfCode As Long, mIgfIpgCode As Long, mIgfPttCode As Long, mQryPttCode As Long, mIgfOldIgfCode As Long, dtlAddEditMode As Boolean, dtlAddMode As Boolean, dtlEditCanceled As Boolean
...
Dim mIgfCode_old As Long, mIgfDate_old As Date, mIgfVtmCode_old As Long, mIgfVchNo_old As Long
Dim mIblCode_old As Long, mIgfRfugAmt_old As Double, mIgfAdjAmt_old As Double
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsVgnSnglTranEntry, clsIGT As clsVgnSnglTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnIpdOptionsEntry          ' clsIBL As clsVgnIpdMultiTranEntry,
...
Dim fcmbIgfVtmName As clsComFlexSearch
...
EntryDate = Ctod(dtpIgfDate.Text)
End Property
...
EntryVtmCode = Val(fcmbIgfVtmName.BoundText)
End Property
        ...
        If .EOF = False And .BOF = False Then
            ...
            Else
                ...
                If optMonthwise.Value = True Then
                    dtpFromDate.Text = Dtoc(MonthStartDate(DateSerial(Val(Left(.fields("IgfYrMonth"), 4)), Val(Right(.fields("IgfYrMonth"), 2)), 1)))
                    dtpToDate.Text = Dtoc(MonthEndDate(DateSerial(Val(Left(.fields("IgfYrMonth"), 4)), Val(Right(.fields("IgfYrMonth"), 2)), 1)))
                    optMonthwise.Value = False
                    optDatewise.Value = True
                ElseIf optDatewise.Value = True Then
                    dtpFromDate.Text = Dtoc(.fields("IgfDate"))
                    dtpToDate.Text = Dtoc(.fields("IgfDate"))
                    optDatewise.Value = False
                    optDetailed.Value = True
                ElseIf optPatientwise.Value = True Then
                    mQryPttCode = .fields("IgfPttCode")
                    fcmbPttRegNo.BoundText = CStr(mQryPttCode)
                    fcmbPttName.BoundText = CStr(mQryPttCode)
...
''Dim tRecset As New ADODB.Recordset, srow As Integer, mIgfSno As Integer, mIgfICode As Long
''
''srow = Mfgrd2.Row
''mIgfSno = Val(Mfgrd2.TextMatrix(srow, X2IBPYSNO_COL))
''mIgfICode = Val(Mfgrd2.TextMatrix(srow, X2IBPYICODE_COL))
''
...
''        .ClearDtl
''    Else
''        .GetDtlData mIgfCode, mIgfICode
''    End If
''    .mTrnCode_lng = mIgfCode
''    .mTrnDtlSno_int = Val(UnMyNumFmt(mskIgfSno.Text))
''    .mTrnDate_dt = Ctod(dtpIgfDate.Text)
''    .mTrnCmpCode_int = sFinYrCmpCode
''
''    .mIgfPttCode_lng = mIgfPttCode
''    .mIgfIbhCode_lng = Val(fcmbIblVchNo.BoundText)
''    .mIgfRfugAmt_dbl = Val(UnMyNumFmt(mskIgfRfugAmt.Text))
''    .mIgfRemark_str = ""
''
''    If dtlAddMode = True Then
''        .AddNewDtl mIgfCode
''        mIgfICode = .mTrnDtlICode_lng
''    Else
''        .UpdateDtl mIgfCode, mIgfICode
''    End If
''    End With
''
''    RefreshDatabase dbVgnDatabase
''    With tRecset
''    .Open "Select * from IndrRgRefdDtl where IgfCode=" & CStr(mIgfCode) & " and IgfICode=" & CStr(mIgfICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''
''    clsIBL.GetHdrData .fields("IgfIbhCode")
''    clsVTM.GetData clsIBL.mTrnVtmCode_lng
''
''    Mfgrd2.TextMatrix(srow, X2IBPYICODE_COL) = .fields("IgfICode")
''    Mfgrd2.TextMatrix(srow, X2IBPYSNO_COL) = ToMyNumFmt(.fields("IgfSno"), mDecimals:=0)
...
''    Mfgrd2.TextMatrix(srow, X2IBLDEPOAMTEXCL_COL) = ToMyNumFmt(clsIBL.mIbhDepoAmt_dbl - .fields("IgfRfugAmt"))
''    Mfgrd2.TextMatrix(srow, X2IBPYDEPOAMT_COL) = ToMyNumFmt(.fields("IgfRfugAmt"))
...
''    Mfgrd2.TextMatrix(srow, X2IBLCODE_COL) = .fields("IgfIbhCode")
''    Mfgrd2.TextMatrix(srow, X2IBPYRECSTATE_COL) = .fields("IgfRecState")
''
''    End With
''    CloseTable tRecset
''
...
If optDetailed.Value = True Then
    txtSearch1Text_InterActiveChange Format(Ctod(dtpIgfDate.Text), "yyyymmdd") + Format(mIgfCode, "0000000")
ElseIf optMonthwise.Value = True Then
    txtSearch1Text_InterActiveChange Format(Ctod(dtpIgfDate.Text), "yyyy") & "/" & Format(Ctod(dtpIgfDate.Text), "mm")
ElseIf optDatewise.Value = True Then
    txtSearch1Text_InterActiveChange Format(Ctod(dtpIgfDate.Text), "yyyymmdd")
ElseIf optPatientwise.Value = True Then
...
Private Sub dtpIgfDate_Validate(Cancel As Boolean)
dtpIgfDate.Text = ToMyDate(dtpIgfDate.Text)
If IsFinYrDate(Ctod(dtpIgfDate.Text)) = False Then
    ErrorBox "Invalid Input !!!"
    dtpIgfDate.SetFocus
    Cancel = True
End If
...
mFormUIdFileName = "IndrRgRf.Uid"
mFormAcsPermFileName = "IndrRgRf.Acp"
...
ChkCustomForm Me, CfsFileFullPath("IndrRgFd.Cfs")
...
clsVGNOPT.blnIndrRgRefdOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetIndrRgRefdOptData
...
Set clsIGT = New clsVgnSnglTranEntry
Set clsIGT.dbAcDatabase = dbAcDatabase
Set clsIGT.dbVgnDatabase = dbVgnDatabase
clsIGT.blnIndrRgRefdTran = True
clsIGT.Init
...
Set fcmbIgfVtmName = New clsComFlexSearch
Set fcmbIgfVtmName.dbAcDatabase = dbAcDatabase
fcmbIgfVtmName.blnVTypeMastList = True
fcmbIgfVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_INDOOR_RGREFD) & " order by VtmName"
fcmbIgfVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode where IgfCode=" & CStr(mIgfCode) & " order by IgfDate,IgfTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode order by IgfDate,IgfTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
''Mfgrd1.FixedCols = 0
Call SmryList
...
''colcnt = X2IBPYSNO_COL: Mfgrd2.ColWidth(colcnt) = mskIgfSno.Width: Mfgrd2.TextMatrix(0, colcnt) = "Sno": Mfgrd2.ColAlignment(colcnt) = 7
...
''colcnt = X2IBPYDEPOAMT_COL: Mfgrd2.ColWidth(colcnt) = mskIgfRfugAmt.Width: Mfgrd2.TextMatrix(0, colcnt) = "Depo.Amt": Mfgrd2.ColAlignment(colcnt) = 7
...
''
''colcnt = X2IBPYSNO_COL: mskIgfSno.Left = Mfgrd2.Left + Mfgrd2.ColPos(colcnt)
...
''colcnt = X2IBPYDEPOAMT_COL: mskIgfRfugAmt.Left = Mfgrd2.Left + Mfgrd2.ColPos(colcnt)
...
''lblIpgAdvAmtTot.Left = mskIblAdvAmt.Left - frFooter.Left: lblIpgAdvAmtTot.Width = mskIblAdvAmt.Width
''lblIblDepoAmtExclTot.Left = mskIblDepoAmtExcl.Left - frFooter.Left: lblIblDepoAmtExclTot.Width = mskIblDepoAmtExcl.Width
''lblIgfRfugAmtTot.Left = mskIgfRfugAmt.Left - frFooter.Left: lblIgfRfugAmtTot.Width = mskIgfRfugAmt.Width
...
Dim mPttRegNo As Long, mIgfPttCode As Long, mIpgVchNo As Long, mIgfVchNo As Long
Dim mIblAmtAftDiscAmtTot As Double, mIblAdvAmtTot As Double, mIblDepoAmtExclTot As Double, mIgfRfugAmtTot As Double, mIblRefuAmtTot As Double
Dim mIgfRfugAmt As Double, mIgfAdjAmt As Double, mIgfBalAmt As Double
...
mIgfPttCode = Val(mskIgfPttCode.Text)
If mIgfPttCode < 1 Then
    ErrorBox "Invalid Key Value !!!"
    txtPttName.SetFocus
    GoTo EndSub
End If
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mIgfPttRegNoNotZero_bln = True Then
...
If Val(mskIgfIpgCode.Text) < 1 Then
    ErrorBox "Invalid Key Value !!!"
    txtPttName.SetFocus
    GoTo EndSub
End If
...
mIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(0).Text))
If mIgfVchNo > 0 Then
    If mIgfVchNo <> mIgfVchNo_old Or Val(fcmbIgfVtmName.BoundText) <> mIgfVtmCode_old Or Ctod(dtpIgfDate.Text) <> mIgfDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=mIgfVchNo, mKeyFieldName:="IgfCode", mKeyFieldValue:=mIgfCode, mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text)) = True Then
            ErrorBox "Duplicate Voucher No. !!!"
            mskIgfVchNo(0).SetFocus
            ' ---------------
            If FormAddEditMode = True Then
                mIgfVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRgRefd", mFieldName:="IgfVchNo", mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
                mIgfVchNo = mIgfVchNo_old
            End If
            mskIgfVchNo(0).Text = ToMyNumFmt(mIgfVchNo, mDecimals:=0)
            GoTo EndSub
        End If
    End If
Else
    ...
    GoTo EndSub
End If
If IsDate(dtpIgfDate.Text) = False Then
    ErrorBox "Invalid Input !!!"
    dtpIgfDate.SetFocus
    GoTo EndSub
Else
    ...
    End If
End If
If TimeToMin(txtIgfTime_str.Text) > cDAYMINUTES Then
    ErrorBox "Invalid Input !!!"
    txtIgfTime_str.SetFocus
    GoTo EndSub
End If
mIgfRfugAmt = Val(UnMyNumFmt(mskIgfRfugAmt.Text))
If mIgfRfugAmt <= 0 Then
    ErrorBox "Invalid Amount !!!"
    mskIgfRfugAmt.SetFocus
    GoTo EndSub
End If
mIgfAdjAmt = Val(UnMyNumFmt(mskIgfAdjAmt.Text))
If mIgfAdjAmt < 0 Then
    ErrorBox "Invalid Adjusted Amount !!!"
    Rem mskIgfAdjAmt.SetFocus
    Rem GoTo EndSub
ElseIf mIgfAdjAmt > mIgfRfugAmt Then
    ErrorBox "Excess Amount Adjusted !!!"
    Rem mskIgfAdjAmt.SetFocus
    Rem GoTo EndSub
End If
mIgfBalAmt = Val(UnMyNumFmt(mskIgfBalAmt.Text))
If mIgfBalAmt < 0 Then
    ErrorBox "Invalid Balance Amount !!!"
    Rem mskIgfBalAmt.SetFocus
    Rem GoTo EndSub
End If

''mIgfRfugAmtTot = Val(UnMyNumFmt(lblIgfRfugAmtTot.Caption))
''If mIgfRfugAmtTot < 0 Then
''    ErrorBox "Invalid Input !!!"
''    Mfgrd2.SetFocus
''    GoTo EndSub
''End If
...
''If (mIblAdvAmtTot + mIblDepoAmtExclTot + mIgfRfugAmtTot) > mIblAmtAftDiscAmtTot Then
...
Private Sub mskIgfAdjAmt_GotFocus()
FlashActiveControl mskIgfAdjAmt, True
End Sub

Private Sub mskIgfAdjAmt_LostFocus()
FlashActiveControl mskIgfAdjAmt, False
End Sub
...
Private Sub mskIgfRfugAmt_Validate(Cancel As Boolean)
mskIgfRfugAmt.Text = ToMyNumFmt(mskIgfRfugAmt.Text)
If Val(UnMyNumFmt(mskIgfRfugAmt.Text)) <= 0 Then
    ErrorBox "Invalid Amount !!!"
    mskIgfRfugAmt.SetFocus
    Cancel = True
Else
    If Val(UnMyNumFmt(mskIgfRfugAmt.Text)) <> mIgfRfugAmt_old Or FormAddEditMode = cFORM_ADDMODE Then
        ShowIpgLinkTranAmt Val(UnMyNumFmt(mskIgfIpgCode.Text))
        mskIgfBalAmt.Text = Val(UnMyNumFmt(mskIgfRfugAmt.Text)) - Val(UnMyNumFmt(mskIgfAdjAmt.Text))
    End If
End If
...
Private Sub mskIgfVchNo_GotFocus(Index As Integer)
FlashActiveControl mskIgfVchNo(Index), True
End Sub

Private Sub mskIgfVchNo_LostFocus(Index As Integer)
FlashActiveControl mskIgfVchNo(Index), False
End Sub

Private Sub mskIgfVchNo_Validate(Index As Integer, Cancel As Boolean)
Dim mIgfVchNo As Long

mIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(Index).Text))
If mIgfVchNo < 0 And Index = 0 Then
    mIgfVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRgRefd", mFieldName:="IgfVchNo", mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
    mskIgfVchNo(Index).Text = ToMyNumFmt(mIgfVchNo, mDecimals:=0)
ElseIf mIgfVchNo > 0 And Index = 0 Then
    If mIgfVchNo <> mIgfVchNo_old Or Val(fcmbIgfVtmName.BoundText) <> mIgfVtmCode_old Or Ctod(dtpIgfDate.Text) <> mIgfDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=mIgfVchNo, mKeyFieldName:="IgfCode", mKeyFieldValue:=mIgfCode, mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text)) = True Then
            ErrorBox "Duplicate Voucher No. !!!"
            Cancel = True
            mskIgfVchNo(Index).SetFocus
            ' ----------
...
Private Sub txtIgfRemark_GotFocus()
FlashActiveControl txtIgfRemark, True
End Sub

Private Sub txtIgfRemark_LostFocus()
FlashActiveControl txtIgfRemark, False
End Sub

Private Sub txtIgfRemark_Validate(Cancel As Boolean)
txtIgfRemark.Text = ToMyWord(txtIgfRemark.Text)
End Sub
...
Private Sub txtIgfTime_str_GotFocus()
FlashActiveControl txtIgfTime_str, True
End Sub

Private Sub txtIgfTime_str_LostFocus()
FlashActiveControl txtIgfTime_str, False
End Sub

Private Sub txtIgfTime_str_Validate(Cancel As Boolean)
txtIgfTime_str.Text = MinToTime(TimeToMin(txtIgfTime_str.Text))
If TimeToMin(txtIgfTime_str.Text) > cDAYMINUTES Then
    ErrorBox "Invalid Input !!!"
    txtIgfTime_str.SetFocus
    Cancel = True
...
Private Sub txtIgfVtmName_Change(Index As Integer)
If fcmbIgfVtmName.CallFromText_Change = False Then
    fcmbIgfVtmName.CallFromText_Change = True
    If InterActiveChange(txtIgfVtmName(Index)) = True Then
        fcmbIgfVtmName.UserText = Array(txtIgfVtmName(Index).Text, txtIgfVtmName(Index).SelStart)
        fcmbIgfVtmName.Show
        txtIgfVtmName(Index).Text = fcmbIgfVtmName.Text
        If fcmbIgfVtmName.ListSelected = True Then
            SendKeys "{tab}"
        End If
    End If
    fcmbIgfVtmName.CallFromText_Change = False
End If

End Sub

Private Sub txtIgfVtmName_GotFocus(Index As Integer)
FlashActiveControl txtIgfVtmName(Index), True
End Sub

Private Sub txtIgfVtmName_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
If MyDropDownKey(Shift, KeyCode) = True Then
    Call txtIgfVtmName_Change(Index)
ElseIf MyAddNewKey(Shift, KeyCode) = True Then
...
Private Sub txtIgfVtmName_LostFocus(Index As Integer)
FlashActiveControl txtIgfVtmName(Index), False
End Sub

Private Sub txtIgfVtmName_Validate(Index As Integer, Cancel As Boolean)
Dim mIgfVchNo As Long

If Val(fcmbIgfVtmName.BoundText) < 1 Then
    ErrorBox "Invalid Selection !!!"
    txtIgfVtmName(Index).SetFocus
    Cancel = True
ElseIf Index = 0 Then
    If Val(fcmbIgfVtmName.BoundText) <> mIgfVtmCode_old Or Ctod(dtpIgfDate.Text) <> mIgfDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        mIgfVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRgRefd", mFieldName:="IgfVchNo", mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
        mskIgfVchNo(Index).Text = ToMyNumFmt(mIgfVchNo, mDecimals:=0)
    End If
End If
...
mIgfCode = clsIGT.ActiveCode
mIgfIpgCode = 0
mIgfPttCode = 0

ShowIgfData mIgfCode
ShowIpgData mIgfIpgCode
ShowPatData mIgfPttCode
...
If CanUserEditOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsIGT.BeginTran(datRecset.fields("IgfCode")) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
...
With datRecset
mIgfCode = .fields("IgfCode")
mIgfIpgCode = .fields("IgfIpgCode")
mIgfPttCode = .fields("IgfPttCode")

clsIGT.GetData mIgfCode
clsIPG.GetData mIgfIpgCode
clsPAT.GetData mIgfPttCode
End With
...
Private Sub ShowIgfData(ByVal mIgfCode As Long)

With clsIGT
.GetData mIgfCode

mskFormBoundField.Text = mIgfCode
fcmbIgfVtmName.BoundText = CStr(.mVtmCode_lng): txtIgfVtmName(0).Text = fcmbIgfVtmName.Text
txtIgfPrefix.Text = .mIgfPrefix_str
mskIgfVchNo(0).Text = ToMyNumFmt(.mVchNo_lng, mDecimals:=0)
dtpIgfDate.Text = Dtoc(.mVchDate_dt)
txtIgfTime_str.Text = MinToTime(.mIgfTime_lng)
txtIgfRemark.Text = .mIgfRemark_str
mskIgfRfugAmt.Text = ToMyNumFmt(.mIgfRfugAmt_dbl)
mskIgfAdjAmt.Text = ToMyNumFmt(.mIgfAdjAmt_dbl)
mskIgfBalAmt.Text = ToMyNumFmt(.mIgfBalAmt_dbl)
Rem old indoor bill
mIgfOldIgfCode = .mIgfOldIgfCode_lng
ShowOldIgfCodeDesc mIgfOldIgfCode
End With

''Call CalcIblChg
''ShowDtlData mIgfCode

End Sub

Private Sub ShowIgfDefData()
Dim mIgfVchNo As Long

dtpIgfDate.Text = Dtoc(DefaultEntryDate)
txtIgfTime_str.Text = MinToTime(TimeToMin(DateTime.Time$))

fcmbIgfVtmName.BoundText = CStr(clsVGNOPT.GetIndrRgRefdOptBoundCode("IgfVtmCode")): txtIgfVtmName(0).Text = fcmbIgfVtmName.Text
...
Private Sub StoreIgfOldData()
mIgfCode_old = mIgfCode
mIgfDate_old = Ctod(dtpIgfDate.Text)
mIgfVtmCode_old = Val(fcmbIgfVtmName.BoundText)
mIgfVchNo_old = Val(UnMyNumFmt(mskIgfVchNo(0).Text))
mIgfRfugAmt_old = Val(UnMyNumFmt(mskIgfRfugAmt.Text))
mIgfAdjAmt_old = Val(UnMyNumFmt(mskIgfAdjAmt.Text))

End Sub

''Private Sub ShowDtlData(ByVal mIgfCode As Long)
...
''
''With tRecset
''.Open "Select * from IndrRgRefdDtl where IgfCode=" & CStr(mIgfCode) & " order by IgfSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
''If .RecordCount > 0 Then
''    Do While .EOF = False
''        If InList(.fields("IgfRecState"), Array(cREC_ADDED, cREC_EDITED, cREC_UNCHANGED)) = True Then
''            clsIBL.GetHdrData .fields("IgfIbhCode")
''            clsVTM.GetData clsIBL.mTrnVtmCode_lng
''
''            Mfgrd2.TextMatrix(srow, X2IBPYICODE_COL) = .fields("IgfICode")
...
''            srow = srow + 1
''        Else
''            If InList(.fields("IgfRecState"), Array(cREC_CANCELED, cREC_DELETED)) = False Then
...
Else
    .GetData mIgfCode
End If
.mVchDate_dt = Ctod(dtpIgfDate.Text)
.mVtmCode_lng = Val(fcmbIgfVtmName.BoundText)
.mIgfPrefix_str = txtIgfPrefix.Text
.mVchNo_lng = Val(UnMyNumFmt(mskIgfVchNo(0).Text))
.mIgfPostfix_str = ""
.mCmpCode_int = sFinYrCmpCode

.mIgfTime_lng = TimeToMin(txtIgfTime_str.Text)
.mIgfIpgCode_lng = mIgfIpgCode
.mIgfPttCode_lng = mIgfPttCode
.mIgfOldIgfCode_lng = mIgfOldIgfCode
.mIgfRfugAmt_dbl = Val(UnMyNumFmt(mskIgfRfugAmt.Text))
.mIgfAdjAmt_dbl = .mIgfAdjAmt_dbl
.mIgfBalAmt_dbl = .mIgfRfugAmt_dbl - .mIgfAdjAmt_dbl
.mIgfRemark_str = txtIgfRemark.Text
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mIgfVtmCodeOld_lng = .mVtmCode_lng
    clsVGNOPT.UpdateIndrRgRefdOld
...
mIgfCode = datRecset.fields("IgfCode")
mIgfPttCode = datRecset.fields("IgfPttCode")
mRecAbsPos = datRecset.AbsolutePosition

If clsIGT.CanDeleteTran(mIgfCode) = True Then
    txtVoid.SetFocus
    If clsIGT.BeginTran(mIgfCode) = True Then
        clsIGT.DeleteTran mIgfCode
        clsIGT.UpdateTran mIgfCode
        clsIGT.EndTran mIgfCode
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPatientwise.Value = True Then
    If optMonthwise.Value = True Then
        clsListStru.AddFields mExpr:="IgfYrMonth", mAttrib:=cFAT_SYSTEM
        clsListStru.AddFields mExpr:="IgfMnYear", mTitle:="Month", mShowItem:=True, mAlign:=1, mWidth:=2000
        clsListStru.AddFields mExpr:="IgfCount", mTitle:="Count", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="IgfRfugAmt_sum", mTitle:="Depo.Amt", mShowItem:=True, mAlign:=7, mWidth:=1500
        
        datRecset.open "Select format(IgfDate,'yyyy/MM') as IgfYrMonth" _
         & ",Count(IgfCode) as IgfCount" _
         & ",Sum(IgfRfugAmt) as IgfRfugAmt_sum" _
         & " from IndrRgRefd" _
         ...
         & " group by format(IgfDate,'yyyy/MM')" _
         & " order by format(IgfDate,'yyyy/MM')" _
         , dbVgnDatabase, adOpenKeyset, adLockReadOnly

    ElseIf optDatewise.Value = True Then
        clsListStru.AddFields mExpr:="IgfDateYMD", mAttrib:=cFAT_SYSTEM
        clsListStru.AddFields mExpr:="IgfDate", mTitle:="Date", mShowItem:=True, mAlign:=1, mWidth:=2000
        clsListStru.AddFields mExpr:="IgfCount", mTitle:="Count", mShowItem:=True, mAlign:=7, mWidth:=1500
        clsListStru.AddFields mExpr:="IgfRfugAmt_sum", mTitle:="Depo.Amt", mShowItem:=True, mAlign:=7, mWidth:=1500
        
        datRecset.open "Select IgfDate" _
         & ",Count(IgfCode) as IgfCount" _
         & ",Sum(IgfRfugAmt) as IgfRfugAmt_sum" _
         & " from IndrRgRefd" _
         & " where IgfDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
    ...
    ElseIf optPatientwise.Value = True Then
        ...
        datRecset.open "Select PatMast.PttName,PatMast.PttRegNo,IndrRgRefd.IgfPttCode" _
         & ",Count(IgfCode) as IgfCount" _
         & ",Sum(IgfRfugAmt) as IgfRfugAmt_sum" _
         & " from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode" _
         & " where IgfDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
         & " and IgfCode>0" _
         & IIf(mQryPttCode > 0, " and IgfPttCode=" & CStr(mQryPttCode), "") _
         & " group by PatMast.PttName,PatMast.PttRegNo,IndrRgRefd.IgfPttCode" _
         & " order by PatMast.PttName,PatMast.PttRegNo,IndrRgRefd.IgfPttCode" _
         , dbVgnDatabase, adOpenKeyset, adLockReadOnly
    End If
    
ElseIf optDetailed.Value = True Then
    clsListStru.AddFields mExpr:="tIgfSmryId", mAttrib:=cFAT_SYSTEM
    clsListStru.AddFields mExpr:="IgfCode", mAttrib:=cFAT_SYSTEM
    ...
    clsListStru.AddFields mExpr:="IgfDate", mTitle:="Date", mAlign:=1, mWidth:=1300, mShowItem:=True
    ' -------------------------------------
    ...
    clsListStru.AddFields mExpr:="IgfRfugAmt", mTitle:="Depo.Amt", mAlign:=7, mWidth:=1250, mShowItem:=True
    ' --------------------------------------
    ...
    datRecset.open "Select IndrRgRefd.*,PatMast.*" _
     & ",format(IgfDate,'yyyymmdd')+format(IgfCode,'0000000') as tIgfSmryId" _
     & " from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode" _
     & " where IgfDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
     & IIf(mRemoteAccess = True, " and IgfCode=" & CStr(mIgfCode) & "", "") _
     & " and IgfCode>0" _
     & IIf(mQryPttCode > 0, " and IgfPttCode=" & CStr(mQryPttCode), "") _
     & " order by IgfDate,IgfCode" _
     , dbVgnDatabase, adOpenKeyset, adLockReadOnly
...
If .RecordCount > 0 Then
    Do While .EOF = False
        If optMonthwise.Value = True Or optDatewise.Value = True Or optPatientwise.Value = True Then
            If optMonthwise.Value = True Then
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfYrMonth")) = .fields("IgfYrMonth")
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfMnYear")) = MonthName(Val(Right(.fields("IgfYrMonth"), 2))) & "-" & Left(.fields("IgfYrMonth"), 4)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfCount")) = ToMyNumFmt(.fields("IgfCount"), mDecimals:=0)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfRfugAmt_sum")) = ToMyNumFmt(.fields("IgfRfugAmt_sum"))
                
            ElseIf optDatewise.Value = True Then
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfDateYMD")) = Format(.fields("IgfDate"), "yyyymmdd")
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfDate")) = Dtoc(.fields("IgfDate"))
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfCount")) = ToMyNumFmt(.fields("IgfCount"), mDecimals:=0)
                Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfRfugAmt_sum")) = ToMyNumFmt(.fields("IgfRfugAmt_sum"))
                
            ElseIf optPatientwise.Value = True Then
                clsPAT.GetData .fields("IgfPttCode")
        ...
        ElseIf optDetailed.Value = True Then
            ...
            clsVTM.GetData .fields("IgfVtmCode")
            
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("tIgfSmryId")) = Format(.fields("IgfDate"), "yyyymmdd") & Format(.fields("IgfCode"), "0000000")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfCode")) = .fields("IgfCode")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("VtmName")) = clsVTM.mName_str
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IblVchNo")) = .fields("IgfVchNo")
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfDate")) = Dtoc(.fields("IgfDate"))
            Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IgfRfugAmt")) = ToMyNumFmt(.fields("IgfRfugAmt"))
            ' ------------------------------------
...
Else    ' system field search has been requested by programmer
    If optDetailed.Value = True Then
        SearchFlex Mfgrd1, clsListStru.GetItemIndex("tIgfSmryId"), mText
    ElseIf optMonthwise.Value = True Then
        SearchFlex Mfgrd1, clsListStru.GetItemIndex("IgfYrMonth"), mText
    ElseIf optDatewise.Value = True Then
        SearchFlex Mfgrd1, clsListStru.GetItemIndex("IgfDateYMD"), mText
    ElseIf optPatientwise.Value = True Then
...
Private Sub ShowOldIgfCodeDesc(ByVal mOldIgfCode As Long)
Dim tRecset As New ADODB.Recordset

With tRecset
.open "Select * from IndrRgRefd where IgfCode=" & CStr(mOldIgfCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IgfVtmCode")
    lblOldIgfCodeDesc.Caption = RTrim(clsVTM.mName_str) & " No.:" & CStr(.fields("IgfVchNo")) & " Dated:" & Dtoc(.fields("IgfDate"))
Else
    lblOldIgfCodeDesc.Caption = "#" & CStr(mOldIgfCode) & "#"
End If
End With
CloseTable tRecset
...
Private Sub ShowIpgLinkTranAmt(ByVal mIgfIpgCode As Long)
...
mAdjTranStr = GetIndrIgfAdjTranVchr(cnn:=dbComDatabase, mIpgCode:=mIgfIpgCode, mIgfCode:=mIgfCode)
txtIpgAdjTranVchr.Text = mAdjTranStr   ''' Replace(mLinkTranStr, vbCrLf, "; ")

mLinkTranStr = GetIndrLinkTranVchr(cnn:=dbComDatabase, mIpgCode:=mIgfIpgCode)
txtIpgLinkTranVchr.Text = mLinkTranStr   ''' Replace(mLinkTranStr, vbCrLf, "; ")

End Sub

Private Function GetOldIgfCodeFromPttCode(ByVal mPttCode As Long) As Long
Dim tRecset As New ADODB.Recordset

With tRecset
.open "Select top 1 * from IndrRgRefd where IgfPttCode=" & CStr(mPttCode) & " order by IgfDate desc,IgfVchNo desc,IgfCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    GetOldIgfCodeFromPttCode = .fields("IgfCode")
Else
    GetOldIgfCodeFromPttCode = 0
End If
End With
CloseTable tRecset
    ...
    If InterActiveChange(txtIpgVtmName) = True Then
        ...
        If FormAddEditMode = cFORM_ADDMODE And fcmbAdmIpgByIpgNo.ListSelected = True And Val(fcmbAdmIpgByIpgNo.BoundText) > 0 Then
            ...
            If mOldIpgCode > 0 And mOldIpgPttCode > 0 Then
                mIgfPttCode = mOldIpgPttCode
                ShowPatData mIgfPttCode
                Call StorePatOldData
                'ShowPttDiscPer mIgfPttCode
                
                mIgfIpgCode = mOldIpgCode
                ShowIpgData mIgfIpgCode
                Call StoreIpgOldData
                
                mIgfOldIgfCode = GetOldIgfCodeFromPttCode(mIgfPttCode)
                ShowOldIgfCodeDesc mIgfOldIgfCode
            End If
        Else
...
If Val(fcmbIgfVtmName.BoundText) < 1 Then
    ...
    txtIgfVtmName(1).SetFocus
    Exit Sub
End If
If Val(UnMyNumFmt(mskIgfVchNo(1).Text)) < 1 Then
    ErrorBox "Invalid Voucher No. !!!"
    mskIgfVchNo(1).SetFocus
    Exit Sub
Else
    mIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(1).Text))
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=CStr(mIgfVchNo), mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
        ErrorBox "Voucher No. does not Exist !!!"
        mskIgfVchNo(1).SetFocus
        Exit Sub
    End If
End If
If Val(UnMyNumFmt(mskIgfVchNo(2).Text)) < 1 Then
    ErrorBox "Invalid Voucher No. !!!"
    mskIgfVchNo(2).SetFocus
    Exit Sub
Else
    mIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(2).Text))
    If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=CStr(mIgfVchNo), mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
        ErrorBox "Voucher No. does not Exist !!!"
        mskIgfVchNo(2).SetFocus
        Exit Sub
...
Dim mFromIgfVchNo As Long, mToIgfVchNo As Long, mIgfVtmCode As Long
Dim fCRView As Form

mFromIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(1).Text))
mToIgfVchNo = Val(UnMyNumFmt(mskIgfVchNo(2).Text))
mIgfVtmCode = Val(fcmbIgfVtmName.BoundText)
...
frmMain.Cryst.SelectionFormula = "{vewIndrRgRefd.IgfVchNo}>=" & CStr(mFromIgfVchNo) & " and {vewIndrRgRefd.IgfVchNo}<=" & CStr(mToIgfVchNo) & " and {vewIndrRgRefd.IgfVtmCode}=" & CStr(mIgfVtmCode) & " and {vewIndrRgRefd.IgfCmpCode}=" & CStr(sFinYrCmpCode)
...
If mCallSource = "Keydown" Then
    If Mfgrd1.Row < Mfgrd1.Rows - 1 And optDetailed.Value = True And datRecset.BOF = False And datRecset.EOF = False Then
        fcmbIgfVtmName.BoundText = CStr(datRecset.fields("IgfVtmCode"))
        txtIgfVtmName(1).Text = fcmbIgfVtmName.Text
        mskIgfVchNo(1).Text = datRecset.fields("IgfVchNo")
        mskIgfVchNo(2).Text = datRecset.fields("IgfVchNo")
        ' ----------------
...
ElseIf mCallSource = "SaveEvent" Then
    txtIgfVtmName(1).Text = fcmbIgfVtmName.Text
    mskIgfVchNo(1).Text = CStr(Val(UnMyNumFmt(mskIgfVchNo(0).Text)))
    mskIgfVchNo(2).Text = CStr(Val(UnMyNumFmt(mskIgfVchNo(0).Text)))
    ' ----------------
    Call AssignPrintPara
    frPrintCRList.ZOrder
    frPrintCRList.Visible = True
...
If Dir(IndrRgRefdCRLFileFullPath) = "" Then
    fhand = FreeFile
    Open IndrRgRefdCRLFileFullPath For Output As #fhand
    Write #fhand, "Indoor on A/c.Refund", "IndrRgRefd.Rpt", True
    Close #fhand
End If

cmbCRList.Clear
fhand = FreeFile
Open IndrRgRefdCRLFileFullPath For Input As #fhand
Do While EOF(fhand) = False
    Input #fhand, mCRLTitle, mCRLFileName, mCRLDefault
    If Dir(IndrRgRefdCRLFileFullPath(mFileName:=mCRLFileName)) <> "" Then
        cmbCRList.AddItem mCRLTitle & Space(100) & IndrRgRefdCRLFileFullPath(mFileName:=mCRLFileName)
