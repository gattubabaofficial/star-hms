...
Dim dbStDatabase As New ADODB.Connection, dbTlgDatabase As New ADODB.Connection, dbPyrDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection
...
If dtlAddMode = True Then
    mPdnCode = GetNextSeqno(dbTlgDatabase, "Production", "PdnCode")
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With PDNRecset
    If dtlAddMode = True Then
        mPdnSno = GetNextSeqno(dbTlgDatabase, "Production", "PdnSno", "PdnCode" = CStr(mPdnCode))
        
        PDNRecset.Open "Select * from Production where PdnCode=" & CStr(mPdnCode) & "", dbTlgDatabase, adOpenKeyset, adLockOptimistic
        .AddNew
        Rem .Fields("PdnICode") = AutoNumber
        .Fields("PdnCode") = mPdnCode
        .Fields("PdnSno") = mPdnSno
    Else
        PDNRecset.Open "Select * from Production where PdnCode=" & CStr(mPdnCode) & " and PdnICode=" & CStr(mPdnICode) & "", dbTlgDatabase, adOpenKeyset, adLockOptimistic
    End If
    ...
    If dtlAddMode = True Then
        ...
        clsSTK.mStkNarr_str = "Production Entry"
        clsSTK.AddNew mPdnStkCode
    Else
        Rem issue stock
    ...
    dbTlgDatabase.BeginTrans
    PDNRecset.Open "Select * from Production where PdnCode=" & CStr(mPdnCode) & " order by PdnICode", dbTlgDatabase, adOpenKeyset, adLockOptimistic
    With PDNRecset
    ...
    Mfgrd2.TextMatrix(srow, X2EMPNAME_COL) = clsEMP.mName_str
    ...
    Mfgrd2.TextMatrix(srow, X2PDNRAWINQTY_COL) = ToMyNumFmt(.Fields("PdnRawInQty"))
    Mfgrd2.TextMatrix(srow, X2PDNFINOUTQTY_COL) = ToMyNumFmt(.Fields("PdnFinOutQty"))
    Mfgrd2.TextMatrix(srow, X2PDNRWKOUTQTY_COL) = ToMyNumFmt(.Fields("PdnRwkOutQty"))
    Mfgrd2.TextMatrix(srow, X2PDNRGHOUTQTY_COL) = ToMyNumFmt(.Fields("PdnRghOutQty"))
    Mfgrd2.TextMatrix(srow, X2PDNREJOUTQTY_COL) = ToMyNumFmt(.Fields("PdnRejOutQty"))
    Mfgrd2.TextMatrix(srow, X2PDNTDFOUTQTY_COL) = ToMyNumFmt(.Fields("PdnTdfOutQty"))
    ...
    Mfgrd2.TextMatrix(srow, X2PDNREQDQTY_COL) = ToMyNumFmt(.Fields("PdnReqdQty"))
    Mfgrd2.TextMatrix(srow, X2PDNCOMPQTY_COL) = ToMyNumFmt(.Fields("PdnCompQty"))
...
Else
    ...
    Rem Production
    PDNRecset.Open "Select * from Production where PdnCode=" & CStr(mPdnCode) & " order by PdnICode", dbTlgDatabase, adOpenKeyset, adLockOptimistic
    PDN_LogRS.Open "Select * from Production_Log where PdnCode=" & CStr(mPdnCode) & " order by PdnICode", dbTlgDatabase, adOpenKeyset, adLockOptimistic
...
tRecset.Open "Select * from Production" _
...
Set fcmbEmpName.dbPyrDatabase = dbPyrDatabase
Set fcmbEmpName.frmAccessMaster = New frmEmployeeMaster
...
If mDeleteit = True Then
    ...
    dbTlgDatabase.BeginTrans
    With PDNRecset
    .Open "Select * from Production where PdnCode=" & CStr(mPdnCode) & " order by PdnICode", dbTlgDatabase, adOpenKeyset, adLockOptimistic
    If .RecordCount > 0 Then
        .MoveFirst
        Do While .EOF = False
            .Fields("PdnRecState") = ChkDeleteRecStatus(.Fields("PdnRecState"))
...
mskPdnRawInQty.Text = ToMyNumFmt(0)
mskPdnFinOutQty.Text = ToMyNumFmt(0)
mskPdnRwkOutQty.Text = ToMyNumFmt(0)
mskPdnRghOutQty.Text = ToMyNumFmt(0)
mskPdnRejOutQty.Text = ToMyNumFmt(0)
mskPdnTdfOutQty.Text = ToMyNumFmt(0)
txtCycleTimeStr.Text = SecToTime(0)
txtTimeStr.Text = MinToTime(0)
mskPdnReqdQty.Text = ToMyNumFmt(0)
mskPdnCompQty.Text = ToMyNumFmt(0)
txtCompTimeStr.Text = SecToTime(0)
mskBalQtyForProdn.Text = ToMyNumFmt(0)
mskBalQtyForPack.Text = ToMyNumFmt(0)
...
If mDataOk = True Then
    mskBalQtyForProdn.Text = ToMyNumFmt(GetBalQtyForProdn(mAddEditMode:=True, srow:=Mfgrd2.Row))
    mskBalQtyForPack.Text = ToMyNumFmt(GetBalQtyForPack(mAddEditMode:=True, srow:=Mfgrd2.Row))
    Call ChkCTComp
Else
    mskBalQtyForProdn.Text = ToMyNumFmt(0)
    mskBalQtyForPack.Text = ToMyNumFmt(0)
...
mskPdnFinOutQty.Text = ToMyNumFmt(mskPdnFinOutQty.Text)
...
If mPdnItmCode_old <> mPdnItmCode Then
    ...
    For mLoopPdnDate = mPdnDate_old To mMaxChkDate
        ...
        tRecset.Open "Select sum(PdnFinOutQty) as PdnFinOutQty_sum from Production where PdnItmCode=" & CStr(mPdnItmCode_old) & " and PdnDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
        If tRecset.RecordCount > 0 Then
            mPdnFinOutQty_sum = NoneZero(tRecset.Fields("PdnFinOutQty_sum"), 0)
        End If
        CloseTable tRecset
        
        tRecset.Open "Select sum(MpkItmQty) as MpkItmQty_sum from MatPacking where MpkItmCode=" & CStr(mPdnItmCode_old) & " and MpkDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#", dbTlgDatabase, adOpenKeyset, adLockOptimistic
        If tRecset.RecordCount > 0 Then
            mMpkItmQty_sum = NoneZero(tRecset.Fields("MpkItmQty_sum"), 0)
        End If
        CloseTable tRecset
        ...
        If mBalQty < 0 Then
            ErrorBox "Excess Packing Qty over Production Qty is " & ToMyNumFmt(mBalQty, mUseAbs:=True) & " on dated " & Dtoc(mLoopPdnDate) & " !!!"
...
ElseIf mPdnDate < mPdnDate_old Then
    ...
    For mLoopPdnDate = mPdnDate To mMaxChkDate
        ...
        tRecset.Open "Select sum(PdnFinOutQty) as PdnFinOutQty_sum from Production where PdnItmCode=" & CStr(mPdnItmCode) & " and PdnDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
        If tRecset.RecordCount > 0 Then
            mPdnFinOutQty_sum = NoneZero(tRecset.Fields("PdnFinOutQty_sum"), 0)
        End If
        CloseTable tRecset
        
        tRecset.Open "Select sum(MpkItmQty) as MpkItmQty_sum from MatPacking where MpkItmCode=" & CStr(mPdnItmCode) & " and MpkDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#", dbTlgDatabase, adOpenKeyset, adLockOptimistic
        If tRecset.RecordCount > 0 Then
            mMpkItmQty_sum = NoneZero(tRecset.Fields("MpkItmQty_sum"), 0)
        End If
        CloseTable tRecset
...
tRecset.Open "Select sum(PdnFinOutQty) as PdnFinOutQty_sum from Production where PdnItmCode=" & CStr(mPdnItmCode) & " and PdnDate<=#" & CStr(ToSysDate(mPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
If tRecset.RecordCount > 0 Then
    mPdnFinOutQty_sum = NoneZero(tRecset.Fields("PdnFinOutQty_sum"), 0)
End If
CloseTable tRecset

tRecset.Open "Select sum(MpkItmQty) as MpkItmQty_sum from MatPacking where MpkItmCode=" & CStr(mPdnItmCode) & " and MpkDate<=#" & CStr(ToSysDate(mPdnDate)) & "#", dbTlgDatabase, adOpenKeyset, adLockOptimistic
If tRecset.RecordCount > 0 Then
    mMpkItmQty_sum = NoneZero(tRecset.Fields("MpkItmQty_sum"), 0)
End If
CloseTable tRecset
...
If mPdnOutQtyTot <> Val(UnMyNumFmt(mskPdnRawInQty.Text)) Then
    ErrorBox "Mismatch Total Qty " & ToMyNumFmt(mPdnOutQtyTot) & " !!!"
...
mskPdnRawInQty.Text = ToMyNumFmt(mskPdnRawInQty.Text)
...
ChkPdnInQty = True
With tRecset
.Open "Select top 1 * from Production order by PdnDate desc", dbTlgDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
For mLoopPdnDate = mPdnDate To mMaxChkDate
    ...
    tRecset.Open "Select sum(MisQty) as MisQty_sum from MatIssue where MisEmpCode=" & CStr(mPdnEmpCode) & " and MisItmCode=" & CStr(mPdnItmCode) & " and MisMacCode=" & CStr(mPdnMacCode) & " and MisDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#", dbTlgDatabase, adOpenKeyset, adLockOptimistic
    If tRecset.RecordCount > 0 Then
        mMisQty_sum = NoneZero(tRecset.Fields("MisQty_sum"), 0)
    End If
    CloseTable tRecset
    
    tRecset.Open "Select sum(PdnConsQty) as PdnConsQty_sum from Production where PdnEmpCode=" & CStr(mPdnEmpCode) & " and PdnItmCode=" & CStr(mPdnItmCode) & " and PdnMacCode=" & CStr(mPdnMacCode) & " and PdnDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
    If tRecset.RecordCount > 0 Then
        mPdnConsQty_sum = NoneZero(tRecset.Fields("PdnConsQty_sum"), 0)
    End If
    CloseTable tRecset
    
    tRecset.Open "Select sum(PdnWipOutQty) as PdnWipOutQty_sum from Production where PdnEmpCode=" & CStr(mPdnEmpCode) & " and PdnItmCode=" & CStr(mPdnItmCode) & " and PdnMacCode=" & CStr(mPdnMacCode) & " and PdnDate<=#" & CStr(ToSysDate(mLoopPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
    If tRecset.RecordCount > 0 Then
        mPdnWipOutQty_sum = NoneZero(tRecset.Fields("PdnWipOutQty_sum"), 0)
    End If
    CloseTable tRecset
    ...
    If mBalQty < 0 Then
        ErrorBox "Excess Produciton Qty over Material Issued to Operator is " & ToMyNumFmt(mBalQty, mUseAbs:=True) & " on dated " & Dtoc(mLoopPdnDate) & " !!!"
        ChkPdnInQty = False
        Exit For
    End If
Next mLoopPdnDate
...
tRecset.Open "Select sum(MisQty) as MisQty_sum from MatIssue where MisEmpCode=" & CStr(mPdnEmpCode) & " and MisItmCode=" & CStr(mPdnItmCode) & " and MisMacCode=" & CStr(mPdnMacCode) & " and MisDate<=#" & CStr(ToSysDate(mPdnDate)) & "#", dbTlgDatabase, adOpenKeyset, adLockOptimistic
If tRecset.RecordCount > 0 Then
    mMisQty_sum = NoneZero(tRecset.Fields("MisQty_sum"), 0)
End If
CloseTable tRecset

tRecset.Open "Select sum(PdnConsQty) as PdnConsQty_sum from Production where PdnEmpCode=" & CStr(mPdnEmpCode) & " and PdnItmCode=" & CStr(mPdnItmCode) & " and PdnMacCode=" & CStr(mPdnMacCode) & " and PdnDate<=#" & CStr(ToSysDate(mPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
If tRecset.RecordCount > 0 Then
    mPdnConsQty_sum = NoneZero(tRecset.Fields("PdnConsQty_sum"), 0)
End If
CloseTable tRecset

tRecset.Open "Select sum(PdnWipOutQty) as PdnWipOutQty_sum from Production where PdnEmpCode=" & CStr(mPdnEmpCode) & " and PdnItmCode=" & CStr(mPdnItmCode) & " and PdnMacCode=" & CStr(mPdnMacCode) & " and PdnDate<=#" & CStr(ToSysDate(mPdnDate)) & "#" & IIf(dtlAddMode = False, " and PdnICode<>" & CStr(mPdnICode), ""), dbTlgDatabase, adOpenKeyset, adLockOptimistic
If tRecset.RecordCount > 0 Then
    mPdnWipOutQty_sum = NoneZero(tRecset.Fields("PdnWipOutQty_sum"), 0)
End If
CloseTable tRecset
...
For srow = 1 To Mfgrd2.Rows - 2
    Mfgrd2.TextMatrix(srow, X2BALQTYFORPRODN_COL) = ToMyNumFmt(GetBalQtyForProdn(mAddEditMode:=False, srow:=srow))
    Mfgrd2.TextMatrix(srow, X2BALQTYFORPACK_COL) = ToMyNumFmt(GetBalQtyForPack(mAddEditMode:=False, srow:=srow))
Next srow
...
mskPdnRejOutQty.Text = ToMyNumFmt(mskPdnRejOutQty.Text)
If ChkPdnInQty(mAddEditMode:=True) = False Then
    Rem only message because other input qty change might be required
End If
...
mskPdnRghOutQty.Text = ToMyNumFmt(mskPdnRghOutQty.Text)
If ChkPdnInQty(mAddEditMode:=True) = False Then
    Rem only message because other input qty change might be required
End If
...
mskPdnRwkOutQty.Text = ToMyNumFmt(mskPdnRwkOutQty.Text)
If ChkPdnInQty(mAddEditMode:=True) = False Then
    Rem only message because other input qty change might be required
End If
...
mskPdnTdfOutQty.Text = ToMyNumFmt(mskPdnTdfOutQty.Text)
If ChkPdnInQty(mAddEditMode:=True) = False Then
    Rem only message because other input qty change might be required
Else
...
PDN_LogRS.Open "Select * from Production_Log where PdnCode=" & CStr(mPdnCode) & "", dbTlgDatabase, adOpenKeyset, adLockOptimistic
With PDN_LogRS
If .RecordCount = 0 Then
    .Close
    ...
    dbTlgDatabase.BeginTrans
    dbTlgDatabase.Execute "Insert into Production_Log Select * from Production where PdnCode=" & CStr(mPdnCode) & " and PdnRecState=" & CStr(cREC_UNCHANGED) & ""
    dbTlgDatabase.CommitTrans
Else
    .Close
...
dbTlgDatabase.BeginTrans
dbTlgDatabase.Execute "Delete from Production_Log where PdnCode=" & CStr(mPdnCode)
dbTlgDatabase.CommitTrans
...
mskPdnReqdQty.Text = ToMyNumFmt(mPdnReqdQty)
mskPdnCompQty.Text = ToMyNumFmt(mPdnCompQty)
