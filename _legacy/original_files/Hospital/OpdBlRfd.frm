...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsHmsSnglTranEntry, clsOBL As clsHmsOpdMultiTranEntry, clsOBFD As clsHmsOpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsHOPT As clsHmsOpdOptionsEntry
...
EntryDate = Ctod(dtpObfdDate.Text)
End Property
...
EntryVtmCode = Val(fcmbObfdVtmName.BoundText)
End Property
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from OutdBlRefdDtl where ObfdCode=" & CStr(mObfdCode) & " and ObfdICode=" & CStr(mObfdICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
clsHOPT.blnOutdBlRefdOptMast = True
clsHOPT.Init
clsHOPT.GetOutdBlRefdOptData
...
Set clsOBFD = New clsHmsOpdMultiTranEntry
Set clsOBFD.dbAcDatabase = dbAcDatabase
Set clsOBFD.dbHmsDatabase = dbHmsDatabase
...
fcmbObfdVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_OUTDOOR_BLREFD) & " order by VtmName"
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdBlRefdHdr inner join PatMast on OutdBlRefdHdr.ObfhPttCode = PatMast.PttCode where ObfhCode=" & CStr(mObfdCode) & " order by ObfhDate,ObfhTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("ObfhDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("ObfhDate"))
    AddSmryParaLayer mClear:=True
    
Else
    datRecset.open "Select * from OutdBlRefdHdr inner join PatMast on OutdBlRefdHdr.ObfhPttCode=PatMast.PttCode order by ObfhDate,ObfhTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mObfdPttRegNoNotZero_bln = True Then
    ...
    If mObfdVchNo <> mObfdVchNo_old Or Val(fcmbObfdVtmName.BoundText) <> mObfdVtmCode_old Or Ctod(dtpObfdDate.Text) <> mObfdDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbHmsDatabase, dbAcDatabase, mTableName:="OutdBlRefdHdr", mChkFieldName:="ObfhVchNo", mChkFieldValue:=mObfdVchNo, mKeyFieldName:="ObfhCode", mKeyFieldValue:=mObfdCode, mVtmFieldName:="ObfhVtmCode", mVtmFieldValue:=Val(fcmbObfdVtmName.BoundText), mFinYrFieldName:="ObfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="ObfhDate", mDateFieldValue:=Ctod(dtpObfdDate.Text)) = True Then
            ...
            If FormAddEditMode = True Then
                mObfdVchNo = GetNextVTypeNo(dbHmsDatabase, dbAcDatabase, dbComDatabase, mTableName:="OutdBlRefdHdr", mFieldName:="ObfhVchNo", mVtmFieldName:="ObfhVtmCode", mVtmFieldValue:=Val(fcmbObfdVtmName.BoundText), mFinYrFieldName:="ObfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="ObfhDate", mDateFieldValue:=Ctod(dtpObfdDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
...
fcmbObfdVtmName.BoundText = CStr(clsHOPT.GetOutdBlRefdOptBoundCode("ObfdVtmCode")): txtObfdVtmName(0).Text = fcmbObfdVtmName.Text
...
With tRecset
.open "Select * from OutdBlRefdDtl where ObfdCode=" & CStr(mObfdCode) & " order by ObfdSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mObfdVtmCodeOld_lng = .mTrnVtmCode_lng
    clsHOPT.UpdateOutdBlRefdOld
...
With tRecset
.open "Select * from OutdBlRefdHdr where ObfhCode=" & CStr(mOldOblCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("ObfhVtmCode")
...
With tRecset
.open "Select top 1 * from OutdBlRefdHdr where ObfhPttCode=" & CStr(mPttCode) & " order by ObfhDate desc,ObfhVchNo desc,ObfhCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="OutdBlRefdHdr", mChkFieldName:="ObfhVchNo", mChkFieldValue:=CStr(mObfdVchNo), mVtmFieldName:="ObfhVtmCode", mVtmFieldValue:=Val(fcmbObfdVtmName.BoundText), mFinYrFieldName:="ObfhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
