...
export default function IPDAdvancePage() {
  ...
  const [selectedRow, setSelectedRow] = useState<DepositRecord | null>(null);
  ...
  // Inputs
  const [depDate, setDepDate] = useState(todayDate);
  ...
  const loadDeposits = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/deposits?from_date=${fromDate}&to_date=${toDate}`);
  ...
  const totDpog = deposits.reduce((s, r) => s + (r.DpogAmt || 0), 0);
  const totAdj  = deposits.reduce((s, r) => s + (r.AdjAmt || 0), 0);
  const totBal  = deposits.reduce((s, r) => s + (r.BalAmt || 0), 0);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 110px)", gap: 10 }}>
      ...
      <div className={styles.sectionBox} style={{ padding: 0, flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: "center" }}>
              ...
              <span style={{ color: "var(--text-secondary)" }}>Loading deposits...</span>
            </div>
            ...
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1200, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredDeposits.map(d => {
                  const selected = selectedRow?.IgtCode === d.IgtCode;
                  return (
                    <tr
                      key={d.IgtCode}
                    ...
                    >
                      ...
                      <td style={{ textAlign: "right" }}>₹{d.AdjAmt.toFixed(2)}</td>
                      <td style={{ textAlign: "right", fontWeight: 700 }}>₹{d.BalAmt.toFixed(2)}</td>
                      <td>{d.CareOf || "—"}</td>
                      <td>{d.Remark || "—"}</td>
                      <td>{d.Address || "—"}</td>
                    </tr>
        ...
        {!loading && filteredDeposits.length > 0 && (
          <div style={{
            ...
            display: "grid", gridTemplateColumns: "1fr repeat(3, 130px) 350px",
            alignItems: "center", fontWeight: 700, fontSize: 13, borderTop: "2px solid #990000"
          }}>
            <div>GRAND TOTAL</div>
            <div style={{ textAlign: "right" }}>₹{totDpog.toFixed(2)}</div>
            <div style={{ textAlign: "right" }}>₹{totAdj.toFixed(2)}</div>
      ...
      <div style={{
        ...
        {[
          ...
          { text: "Refresh List (F5)", color: "#f9e6ff", onClick: loadDeposits },
