...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsOPG As clsVgnSnglTranEntry, clsOBL As clsVgnOpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsVGNOPT As clsVgnOpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If dtlAddMode = True Then
        ...
        If .AddNewDtl(mOblCode) = True Then
            ...
            Rem dbVgnDatabase.Execute "Update OutdBlDctDtl set ObdOblCode=" & CStr(.mTrnDtlICode_lng) & " where ObdCode=" & CStr(mOblCode) & " and ObdOblCode=" & CStr(mOblICode) & ""
    ...
    With tRecset
    .open "Select * from OutdBill where OblCode=" & CStr(mOblCode) & " and OblICode=" & CStr(mOblICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
    ...
    If GetServDctDtlCount(cnn:=dbVgnDatabase, mTableName:="OutdBlDctDtl", mSdcCodeField:="ObdCode", mSdcCode:=.fields("OblCode"), mSdcSubCodeField:="ObdOblCode", mSdcSubCode:=.fields("OblICode")) > 0 Then
...
If cmdOK.Caption = "&OK" And dtlSdcEditCanceled = False Then
    ...
    RefreshDatabase dbVgnDatabase
    With tRecset
    .open "Select * from OutdBlDctDtl where ObdCode=" & CStr(mOblCode) & " and ObdOblCode=" & CStr(mObdOblCode) & " and ObdICode=" & CStr(mObdICode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
clsVGNOPT.blnOutdBillOptMast = True
clsVGNOPT.Init
clsVGNOPT.GetOutdBillOptData
...
fcmbOblVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cVGN_VTYPE_OUTDOOR_BILL) & " order by VtmName"
fcmbOblVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from OutdBlHdr inner join PatMast on OutdBlHdr.ObhPttCode=PatMast.PttCode where ObhCode=" & CStr(mOblCode) & " order by ObhDate,ObhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from OutdBlHdr inner join PatMast on OutdBlHdr.ObhPttCode=PatMast.PttCode order by ObhDate,ObhTime", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsVGNOPT.mOblPttRegNoNotZero_bln = True Then
    ...
    If mOblVchNo <> mOblVchNo_old Or Val(fcmbOblVtmName.BoundText) <> mOblVtmCode_old Or Ctod(dtpOblDate.Text) <> mOblDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbVgnDatabase, dbAcDatabase, mTableName:="OutdBlHdr", mChkFieldName:="ObhVchNo", mChkFieldValue:=mOblVchNo, mKeyFieldName:="ObhCode", mKeyFieldValue:=mOblCode, mVtmFieldName:="ObhVtmCode", mVtmFieldValue:=Val(fcmbOblVtmName.BoundText), mFinYrFieldName:="ObhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="ObhDate", mDateFieldValue:=Ctod(dtpOblDate.Text)) = True Then
            ...
            If FormAddEditMode = cFORM_ADDMODE Then
                mOblVchNo = GetNextVTypeNo(dbVgnDatabase, dbAcDatabase, dbComDatabase, mTableName:="OutdBlHdr", mFieldName:="ObhVchNo", mVtmFieldName:="ObhVtmCode", mVtmFieldValue:=Val(fcmbOblVtmName.BoundText), mFinYrFieldName:="ObhCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="ObhDate", mDateFieldValue:=Ctod(dtpOblDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormOblAcsPermFileName)
            Else
...
ElseIf mOblVchNo > 0 Then
    If Index = 0 Then
        ...
        End If
    Else
        If ChkIsExistVTypeNo(dbVgnDatabase, mTableName:="OutdBlHdr", mChkFieldName:="ObhVchNo", mChkFieldValue:=CStr(mOblVchNo), mVtmFieldName:="ObhVtmCode", mVtmFieldValue:=Val(fcmbOblVtmName.BoundText), mFinYrFieldName:="ObhCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
fcmbOblVtmName.BoundText = CStr(clsVGNOPT.GetOutdBillOptBoundCode("OblVtmCode")): txtOblVtmName(0).Text = fcmbOblVtmName.Text
...
With tRecset
.open "Select * from OutdBill where OblCode=" & CStr(mOblCode) & " order by OblSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
...
If FormAddEditMode = cFORM_ADDMODE Then
    If .AddNewHdr() = True Then
        ...
        dbVgnDatabase.Execute "Delete from OutdBlDctDtl where ObdOblCode IN (Select OblICode from OutdBill as obl inner join ServMast as srv on obl.OblSrvCode=srv.SrvCode where srv.SrvMultiDctIncl=False and obl.OblCode=" & CStr(mOblCode) & ") and ObdCode=" & CStr(mOblCode) & ""
        dbVgnDatabase.CommitTrans
        
        Rem updating linked autonumber
        Rem dbVgnDatabase.BeginTrans
        Rem dbVgnDatabase.Execute "Update OutdBlDctDtl set OblCode=" & CStr(.mTrnCode_lng) & " where OblCode=" & CStr(mOblCode) & ""
        Rem dbVgnDatabase.CommitTrans
    End If
Else
    .UpdateHdr mOblCode
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsVGNOPT.mOblVtmCodeOld_lng = .mTrnVtmCode_lng
    clsVGNOPT.UpdateOutdBillOld
...
If Val(fcmbSrvName.BoundText) <> mOblSrvCode_old Or mOpgCDctCode_old <> clsOPG.mOpgCDctCode_lng Or mPttPcgCode_old <> clsPAT.mPttPcgCode_lng Or mOpgRByCode_old <> clsOPG.mOpgRByCode_lng Or mOpgRToCode_old <> clsOPG.mOpgRToCode_lng Or mOblDate_old <> Ctod(dtpOblDate.Text) Or mOblTime_old <> TimeToMin(txtOblTime_str) Or FormAddEditMode = cFORM_ADDMODE Then
    GetServRateValues cnn:=dbVgnDatabase, mSrvCode:=Val(fcmbSrvName.BoundText), mTranDate:=Ctod(dtpOblDate.Text), mTranTime:=TimeToMin(txtOblTime_str.Text), mSrmPttCode:=mOblPttCode, mSrmDctCode:=clsOPG.mOpgCDctCode_lng, mSrmPcgCode:=clsPAT.mPttPcgCode_lng, mSrmRByCode:=clsOPG.mOpgRByCode_lng, mSrmRToCode:=0, mSrmRate:=mOblRate, mSrmDiscPer:=mOblDiscPer, mSrmInfByPer:=mOblInfByPer, mSrmDefByPer:=mOblDefByPer
...
With tRecset
.open "Select * from OutdBlHdr where ObhCode=" & CStr(mOldOblCode) & "", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("ObhVtmCode")
...
With tRecset
.open "Select top 1 * from OutdBlHdr where ObhPttCode=" & CStr(mPttCode) & " order by ObhDate desc,ObhVchNo desc,ObhCode desc", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
If dtlAddEditMode = True And dtlAddMode = False And mDataOk = True And InterActiveChange(cmdOK) = True Then
    ...
    If clsSRV.mSrvMultiDctIncl_bln = False Then
        If GetServDctDtlCount(cnn:=dbVgnDatabase, mTableName:="OutdBlDctDtl", mSdcCodeField:="ObdCode", mSdcCode:=mOblCode, mSdcSubCodeField:="ObdOblCode", mSdcSubCode:=Val(mskOblICode.Text)) > 0 Then
...
With tRecset
.open "Select OutdBlPymtDtl.*,OutdBlPymtHdr.ObphVtmCode,OutdBlPymtHdr.ObphVchNo from OutdBlPymtDtl inner join OutdBlPymtHdr on OutdBlPymtDtl.ObpyCode=OutdBlPymtHdr.ObphCode where ObpyObhCode=" & CStr(mOblCode) & " order by ObpyDate", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select OutdBlRefdDtl.*,OutdBlRefdHdr.ObfhVtmCode,OutdBlRefdHdr.ObfhVchNo from OutdBlRefdDtl inner join OutdBlRefdHdr on OutdBlRefdDtl.ObfdCode=OutdBlRefdHdr.ObfhCode where ObfdObhCode=" & CStr(mOblCode) & " order by ObfdDate", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select * from OutdBlDctDtl where ObdCode=" & CStr(mOblCode) & " and ObdOblCode=" & CStr(mObdOblCode) & " order by ObdSno", dbVgnDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
