...
Dim clsSGP As clsHmsMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
...
If mRemoteAccess = True Then
    datRecset.open "Select * from ServGrpMst where SgpCode=" & CStr(mSgpCode) & " order by SgpName", dbHmsDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from ServGrpMst order by SgpName", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtSgpName.Text <> mSgpName_old Then
    If ChkIsDuplicate(dbHmsDatabase, "ServGrpMst", "SgpName", txtSgpName.Text, "SgpCode", Val(mskFormBoundField.Text)) = True Then
...
Else
    With tRecset
    .open "Select SgpCode from ServGrpMst order by SgpCode", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    tRecset.open "Select * from ServGrpMst where SgpCode<>" & CStr(.mCode_lng) & " and SgpIndex=" & .mSgpIndex_int & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If clsSGP.CanDeleteMast(mSgpCode) = True Then
    ...
    If clsSGP.BeginMast(mSgpCode) = True Then
        ...
        dbHmsDatabase.BeginTrans
        dbHmsDatabase.Execute "Update ServGrpMst set SgpIndex=SgpIndex-1 where SgpIndex>=" & CStr(clsSGP.mSgpIndex_int) & " and SgpCode<>" & CStr(mSgpCode) & ""
        dbHmsDatabase.CommitTrans
