...
Begin VB.Form frmAreaMast 
   ...
   Begin VB.ComboBox cmbFormEntryMode 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "AreaMast.frx":0000
      Left            =   0
      List            =   "AreaMast.frx":000D
...
Dim mAraCode As Long
Dim mRemoteAccess As Boolean, mEntrySaved As Boolean, mEntryAborted As Boolean
Dim dbAcDatabase As New ADODB.Connection
Dim datRecset As New ADODB.Recordset
Dim mAraName_old As String
Dim clsARA As clsComMastEntry, clsSTN As clsComMastEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim fcmbStnName As clsComFlexSearch

Public Property Let RemoteAccessCode(ByVal mAccessCode As Long)
mRemoteAccess = True
mAraCode = mAccessCode
End Property
...
Public Property Get AraName() As String
AraName = txtAraName.Text
End Property
...
Call SmryList
txtSearch1Text_InterActiveChange txtAraName.Text

''datRecset.Find "AraName='" & txtAraName.Text & "'", Start:=1
''ShowRecActiveFlexRow datRecset, Mfgrd1
...
If mRemoteAccess = True Then
    datRecset.open "Select * from AreaMast where AraCode=" & CStr(mAraCode) & " order by AraName", dbAcDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from AreaMast order by AraName", dbAcDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If Val(mskFormBoundField.Text) < 1 Then
    ...
    Exit Function
End If
If Trim(txtAraName.Text) = "" Then
    ErrorBox "Invalid Operator Name !!!"
    txtAraName.SetFocus
    Exit Function
End If
If txtAraName.Text <> mAraName_old Then
    If ChkIsDuplicate(dbAcDatabase, "AreaMast", "AraName", txtAraName.Text, "AraCode", Val(mskFormBoundField.Text)) = True Then
...
txtAraName.Text = ToMyWord(txtAraName.Text)
If Trim(txtAraName.Text) = "" Then
...
If CanUserAddOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsARA.BeginMast(0) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
...
clsARA.Clear
mAraCode = clsARA.ActiveCode
mskFormBoundField.Text = mAraCode
txtAraName.Text = ""
fcmbStnName.BoundText = "": txtStnName.Text = ""

mAraName_old = txtAraName.Text
...
If CanUserEditOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsARA.BeginMast(datRecset.fields("AraCode")) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
FormAddEditMode = cFORM_EDITMODE
With datRecset
mAraCode = .fields("AraCode")
clsARA.GetData mAraCode
mskFormBoundField.Text = CStr(.fields("AraCode"))
txtAraName.Text = .fields("AraName")
fcmbStnName.BoundText = CStr(.fields("AraStnCode")): txtStnName.Text = fcmbStnName.Text
End With
...
Else
    .GetData mAraCode
End If
.mName_str = txtAraName.Text
.mAraStnCode_lng = Val(fcmbStnName.BoundText)
If FormAddEditMode = cFORM_ADDMODE Then
    .AddNew
...
mAraCode = datRecset.fields("AraCode")
mRecAbsPos = datRecset.AbsolutePosition

If clsARA.CanDeleteMast(mAraCode) = True Then
    txtVoid.SetFocus
    If clsARA.BeginMast(mAraCode) = True Then
        clsARA.DeleteMast mAraCode
        clsARA.UpdateMast mAraCode
        clsARA.EndMast mAraCode
...
clsListStru.AddFields mExpr:="AraCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="AraName", mTitle:="Area Name", mAlign:=1, mWidth:=3000, mShowItem:=True
clsListStru.AddFields mExpr:="AraStnName", mTitle:="Station", mAlign:=1, mWidth:=1500, mShowItem:=True
...
If .RecordCount > 0 Then
    Do While .EOF = False
        clsSTN.GetData .fields("AraStnCode")
        
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AraCode")) = .fields("AraCode")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AraName")) = .fields("AraName")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("AraStnName")) = clsSTN.mName_str
...
SearchFlex Mfgrd1, clsListStru.GetItemIndex("AraName"), mText
...
If MyDropDownKey(mShift:=Shift, mKeyCode:=KeyCode) = True Then
