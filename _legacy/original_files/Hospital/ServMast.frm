...
Dim clsDCT As clsHmsMasterEntry, clsPCG As clsHmsMasterEntry, clsRBY As clsHmsMasterEntry, clsRTO As clsHmsMasterEntry
Dim fcmbAhName As clsComFlexSearch, fcmbSgpName As clsHmsFlexSearch, fcmbDctName As clsHmsFlexSearch, fcmbPcgName As clsHmsFlexSearch, fcmbRByName As clsHmsFlexSearch, fcmbRToName As clsHmsFlexSearch
...
Set fcmbDctName = New clsHmsFlexSearch
Set fcmbDctName.dbHmsDatabase = dbHmsDatabase
...
Set fcmbRToName = New clsHmsFlexSearch
Set fcmbRToName.dbHmsDatabase = dbHmsDatabase
...
Set clsRBY = New clsHmsMasterEntry
Set clsRBY.dbHmsDatabase = dbHmsDatabase
...
txtSrvName.Text = ""
fcmbSgpName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbHmsDatabase, mTableName:="ServMast", mColumnName:="SrvSgpCode", mOrderByColName:="SrvCode", mRtnDefValue:=0))
txtSgpName.Text = fcmbSgpName.Text
fcmbAhName.BoundText = CStr(GetLastValueFromTableCol(cnn:=dbHmsDatabase, mTableName:="ServMast", mColumnName:="SrvAhCode", mOrderByColName:="SrvCode", mRtnDefValue:=0))
txtAhName.Text = fcmbAhName.Text
mskSrvIndex.Text = 0
...
With tRecset
.open "Select * from ServRateMst where SrmCode=" & CStr(mSrmCode) & " order by SrmSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    dbHmsDatabase.BeginTrans
    dbHmsDatabase.Execute "Update ServRateMst set SrmRecState=" & cREC_DELETED & " where SrmCode=" & CStr(mSrvCode)
    
    For srow = 1 To Mfgrd2.Rows - 2
        mSrmICode = Val(Mfgrd2.TextMatrix(srow, X2SRMICODE_COL))
        With tRecset
        .open "Select * from ServRateMst where SrmICode=" & CStr(mSrmICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
        If mSrmICode < 1 Then
            .AddNew
        ...
        CloseTable tRecset
    Next srow
    dbHmsDatabase.Execute "Delete from ServRateMst where SrmCode=" & CStr(mSrvCode) & " and SrmRecState=" & cREC_DELETED & ""
    dbHmsDatabase.CommitTrans
...
If clsSRV.CanDeleteMast(mSrvCode) = True Then
    ...
    If clsSRV.BeginMast(mSrvCode) = True Then
        ...
        dbHmsDatabase.BeginTrans
        dbHmsDatabase.Execute "Delete from ServRateMst where SrmCode=" & CStr(mSrvCode) & ""
        dbHmsDatabase.CommitTrans
