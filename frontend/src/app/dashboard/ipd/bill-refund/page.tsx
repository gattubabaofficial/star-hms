...
export default function IPDBillRefundPage() {
  ...
  const loadRefunds = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/bill-refunds?from_date=${fromDate}&to_date=${toDate}`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API}/bill-refunds`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          IbfhIpgCode: selectedPat.IhdCode,
          IbfhPttCode: selectedPat.PttCode,
          IbfhRefuAmt: rfdAmt,
          IbfhRemark:  `Mode: ${payMode}. ${remark}`.trim(),
        }),
      });

      if (!res.ok) throw new Error("Saving refund failed");
      showToast("Bill refund issued successfully!");
      setShowModal(false);
      loadRefunds();
    } catch {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 110px)", gap: 10 }}>
      ...
      {showModal && (
        <div className={styles.modalOverlay} style={{ zIndex: 1000 }}>
          <div className={styles.modalContent} style={{ maxWidth: 440 }}>
            <div className={styles.modalHeader}>
              <h3 style={{ fontSize: 16, fontWeight: 700 }}>Issue Bill Post-Refund</h3>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)" }}>
          IPD Bill Refunds List (Settlement)
        </span>
      </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)" }}>
              No settlement refund records found.
            </div>
          ) : (
            <table className={styles.table} style={{ fontSize: 12, minWidth: 1200, borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredRefunds.map(r => {
                  const selected = selectedRow?.IbfhCode === r.IbfhCode;
                  return (
                    <tr
                      key={r.IbfhCode}
