...
''    With tRecset
''    .Open "Select * from IndrRgRefdDtl where IgfCode=" & CStr(mIgfCode) & " and IgfICode=" & CStr(mIgfICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
''
...
clsHOPT.blnIndrRgRefdOptMast = True
clsHOPT.Init
clsHOPT.GetIndrRgRefdOptData
...
fcmbIgfVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_INDOOR_RGREFD) & " order by VtmName"
fcmbIgfVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode where IgfCode=" & CStr(mIgfCode) & " order by IgfDate,IgfTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from IndrRgRefd inner join PatMast on IndrRgRefd.IgfPttCode=PatMast.PttCode order by IgfDate,IgfTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
''Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mIgfPttRegNoNotZero_bln = True Then
    ...
    If mIgfVchNo <> mIgfVchNo_old Or Val(fcmbIgfVtmName.BoundText) <> mIgfVtmCode_old Or Ctod(dtpIgfDate.Text) <> mIgfDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbHmsDatabase, dbAcDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=mIgfVchNo, mKeyFieldName:="IgfCode", mKeyFieldValue:=mIgfCode, mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text)) = True Then
            ...
            If FormAddEditMode = True Then
                mIgfVchNo = GetNextVTypeNo(dbHmsDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrRgRefd", mFieldName:="IgfVchNo", mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IgfDate", mDateFieldValue:=Ctod(dtpIgfDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormAcsPermFileName)
            Else
...
fcmbIgfVtmName.BoundText = CStr(clsHOPT.GetIndrRgRefdOptBoundCode("IgfVtmCode")): txtIgfVtmName(0).Text = fcmbIgfVtmName.Text
...
''
''With tRecset
''.Open "Select * from IndrRgRefdDtl where IgfCode=" & CStr(mIgfCode) & " order by IgfSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mIgfVtmCodeOld_lng = .mVtmCode_lng
    clsHOPT.UpdateIndrRgRefdOld
...
With tRecset
.open "Select * from IndrRgRefd where IgfCode=" & CStr(mOldIgfCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IgfVtmCode")
...
With tRecset
.open "Select top 1 * from IndrRgRefd where IgfPttCode=" & CStr(mPttCode) & " order by IgfDate desc,IgfVchNo desc,IgfCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="IndrRgRefd", mChkFieldName:="IgfVchNo", mChkFieldValue:=CStr(mIgfVchNo), mVtmFieldName:="IgfVtmCode", mVtmFieldValue:=Val(fcmbIgfVtmName.BoundText), mFinYrFieldName:="IgfCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
