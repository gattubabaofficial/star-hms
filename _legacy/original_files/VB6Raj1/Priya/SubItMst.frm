...
Dim dbPmtDatabase As New ADODB.Connection, dbStDatabase As New ADODB.Connection, dbGrpDatabase As New ADODB.Connection, datRecset As New ADODB.Recordset
Dim mSimName_old As String
Dim clsSIM As clsPmtMasterEntry, clsIT As clsProduct, mActiveControl As Object, clsListStru As clsSelectQueryStructure
...
datRecset.Open "Select * from SubItmMast order by SimName", dbPmtDatabase, adOpenKeyset, adLockOptimistic
Call SmryList
...
If txtSimName.Text <> mSimName_old Then
    If ChkIsDuplicate(dbPmtDatabase, "SubItmMast", "SimName", txtSimName.Text, "SimCode", Val(mskFormBoundField.Text)) = True Then
...
If FormAddEditMode = cFORM_ADDMODE Then
    ...
    .AddProdRef mItrRef1Name:="", mItrRef2Name:="", mItrRate:=Val(UnMyNumFmt(mskSimOpRate.Text))
...
Else
    ...
    .EditProdRef mItrRefCode:=.ItrRefCode(.LProdRef), mItrRef1Name:="", mItrRef2Name:="", mItrRate:=Val(UnMyNumFmt(mskSimOpRate.Text))
...
If clsIT.CanDeleteItem(mSimItmCode, mSkipConfirm:=True, mSkipDependency:=True) = True And clsSIM.CanDeleteMast(mSimCode, mSkipConfirm:=True) = True Then
