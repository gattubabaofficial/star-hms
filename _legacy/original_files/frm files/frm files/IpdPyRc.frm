...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsHmsSnglTranEntry, clsIRC As clsHmsIpdMultiTranEntry, clsIPY As clsHmsIpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsHOPT As clsHmsIpdOptionsEntry
...
Dim fcmbDueIpgByIpgNo As clsHmsFlexSearch, fcmbDueIpgByPttRegNo As clsHmsFlexSearch, fcmbDueIpgByPttName As clsHmsFlexSearch, fcmbPttName As clsHmsFlexSearch, fcmbPttRegNo As clsHmsFlexSearch, fcmbIrcVchNo As clsHmsFlexSearch
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from IndrPymtDtl where IpyCode=" & CStr(mIpyCode) & " and IpyICode=" & CStr(mIpyICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
clsHOPT.blnIndrPymtOptMast = True
clsHOPT.Init
clsHOPT.GetIndrPymtOptData
...
Set clsIPY = New clsHmsIpdMultiTranEntry
Set clsIPY.dbAcDatabase = dbAcDatabase
Set clsIPY.dbHmsDatabase = dbHmsDatabase
...
fcmbIpyVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_INDOOR_PYMT) & " order by VtmName"
fcmbIpyVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrPymtHdr inner join PatMast on IndrPymtHdr.IphPttCode = PatMast.PttCode where IphCode = " & CStr(mIpyCode) & " order by IphDate,IphTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("IphDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("IphDate"))
    AddSmryParaLayer mClear:=True
    
Else
    datRecset.open "Select * from IndrPymtHdr inner join PatMast on IndrPymtHdr.IphPttCode=PatMast.PttCode order by IphDate,IphTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mIpyPttRegNoNotZero_bln = True Then
...
fcmbIpyVtmName.BoundText = CStr(clsHOPT.GetIndrPymtOptBoundCode("IpyVtmCode")): txtIpyVtmName(0).Text = fcmbIpyVtmName.Text
...
With tRecset
.open "Select * from IndrPymtDtl where IpyCode=" & CStr(mIpyCode) & " order by IpySno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mIpyVtmCodeOld_lng = .mTrnVtmCode_lng
    clsHOPT.UpdateIndrPymtOld
...
With tRecset
.open "Select * from IndrPymtHdr where IphCode=" & CStr(mOldIrcCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    clsVTM.GetData .fields("IphVtmCode")
...
With tRecset
.open "Select top 1 * from IndrPymtHdr where IphPttCode=" & CStr(mPttCode) & " order by IphDate desc,IphVchNo desc,IphCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
Else
    ...
    If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="IndrPymtHdr", mChkFieldName:="IphVchNo", mChkFieldValue:=CStr(mIpyVchNo), mVtmFieldName:="IphVtmCode", mVtmFieldValue:=Val(fcmbIpyVtmName.BoundText), mFinYrFieldName:="IphCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
