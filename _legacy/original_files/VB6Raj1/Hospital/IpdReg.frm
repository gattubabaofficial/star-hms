...
Dim mBedStatusAccess As Boolean
...
Public Property Let BedStatusAccessCode(ByVal mAccessCode As Long)
mBedStatusAccess = True
mIpgCode = mAccessCode
End Property
...
ElseIf frIBedStat.Visible = True Then
    ...
    Else
        ...
        Mfgrd1.SetFocus
        If mBedStatusAccess = True Then     ''' mRemoteAccess = True And
            Rem Unload Me
            Me.Hide
            Exit Sub
        End If
...
End If
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    Rem If dtlAddMode = False And mIbsDischState = cIBS_STATE_SHIFTED And mIbsDischState_old = cIBS_STATE_OCCUPIED Then
    If dtlAddMode = False And mIbsDischState = cIBS_STATE_SHIFTED And mIbsDischState_old <> cIBS_STATE_SHIFTED Then
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    If .mCode_lng > 0 Then   ' updated successfully
        ...
        Set tRecset = dbHmsDatabase.Execute("Select * from IBedState where IbsCode = " & CStr(mIbsCode) & "")
        With tRecset
        ...
        If mIbsLIbsCode <> 0 Then
            Set tRecset = dbHmsDatabase.Execute("Select * from IBedState where IbsCode=" & CStr(mIbsLIbsCode) & "")
            With tRecset
...
Else
    ...
    If dtlAddMode = False And Cancel = False Then
        If mIbsDate <> mIbsDate_old Then
            Rem checking alteration for current IbsCode FromDate
            ...
            Rem checking alteration for previous IbsCode ToDate
            If Cancel = False Then
                mIbsCode = Val(Mfgrd3.TextMatrix(srow - 1, X3IBSCODE_COL))
                Set tRecset = dbHmsDatabase.Execute("Select * from ((IndrBlIbsDtl" _
                    & " inner join IBedState on IndrBlIbsDtl.IbbsIbsCode = IBedState.IbsCode)" _
                    & " inner join IndrBill on IndrBlIbsDtl.IbbsCode = IndrBill.IblCode and IndrBlIbsDtl.IbbsIblICode = IndrBill.IblICode)" _
                    & " left join IndrBlHdr on IndrBill.IblCode = IndrBlHdr.IbhCode" _
                    & " where IbbsIpgCode = " & CStr(Val(lblIbsIpgCode.Caption)) & " and IbbsIbsCode = " & CStr(mIbsCode) & " and IbbsToDate >= #" & CStr(ToSysDate(mIbsDate)) & "#" _
                    & " order by IbbsToDate Desc,IbbsToTime desc,IbbsICode desc" & "")
                With tRecset
                If .EOF = False Then
                    If .fields("IbbsToDate") > mIbsDate Then
                        If IsNull(.fields("IbhCode")) = False Then
                            ErrorBox "Bed Charges Already Charged in Indoor Bill No." & CStr(.fields("IbhVchNo")) & " Dated " & Dtoc(.fields("IbhDate")) & " " & MinToTime(.fields("IbhTime")) & " !!!"
                        Else
                            ErrorBox "Bed Charges Already Charged in Indoor Bill Dated " & Dtoc(.fields("IblDate")) & " Under Process !!!"
            ...
            End If
        End If
    ElseIf dtlAddMode = True And Cancel = False Then
        mIbsCode = Val(Mfgrd3.TextMatrix(srow - 1, X3IBSCODE_COL))
        Set tRecset = dbHmsDatabase.Execute("Select * from ((IndrBlIbsDtl" _
            & " inner join IBedState on IndrBlIbsDtl.IbbsIbsCode = IBedState.IbsCode)" _
            & " inner join IndrBill on IndrBlIbsDtl.IbbsCode = IndrBill.IblCode and IndrBlIbsDtl.IbbsIblICode = IndrBill.IblICode)" _
...
If mFormLoaded = False Then
    ...
    ElseIf mBedStatusAccess = True Then
        MoveRecToFirst datRecset
        ShowRecActiveFlexRow datRecset, Mfgrd1
        Mfgrd1.SetFocus
...
fcmbOldIpdByIpdNo.mFiltCond = "0 = 1"
...
fcmbOldIpdByPttRegNo.mFiltCond = "0 = 1"
...
fcmbOldIpdByPttName.mFiltCond = "0 = 1"
fcmbOldIpdByPttName.Init

Call RestoreDefaultSmryPara
If mRemoteAccess = True Or mBedStatusAccess = True Then
    optDetailed.Value = True
    AddSmryParaLayer mClear:=True
End If
...
If mRemoteAccess = True Or mBedStatusAccess = True Then
    ...
    dtpFromDate.Text = Dtoc(datRecset.fields("IpgDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("IpgDate"))
    AddSmryParaLayer mClear:=True
Else
...
Else
    If mPttRegNo <> mPttRegNo_old Or FormAddEditMode = cFORM_ADDMODE Then
        If PatAddEditMode = cFORM_ADDMODE And mPttRegNo <> mPttRegNo_old And clsHOPT.mIpgPttRegNoAddModeChange_bln = False Then
            AlertBox "Change Prohibited in Add Mode !!!"
            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
            mskPttRegNo.SetFocus
            GoTo EndSub
            
        ElseIf PatAddEditMode = cFORM_EDITMODE And mPttRegNo <> mPttRegNo_old And clsHOPT.mIpgPttRegNoEditModeChange_bln = False Then
...
If Val(mskPttAgeDy.Text) < 0 Then
    ...
    GoTo EndSub
End If
If True = False Then    ''' specially done for my data
...
If KeyAscii = vbKeyReturn Then
    ...
    Rem If (dtlAddMode = True And mLastIbsDischState = cIBS_STATE_OCCUPIED) Or dtlAddMode = False Then
    If (dtlAddMode = True And mLastIbsDischState <> cIBS_STATE_SHIFTED) Or dtlAddMode = False Then
...
Set tRecset = dbHmsDatabase.Execute("Select * from IBedState where IbsIpgCode = " & CStr(mIbsIpgCode) & " order by IbsCode")
With tRecset
If .EOF = False Then
    Do While .EOF = False
...
Else
    ...
    If dtlAddMode = False And Cancel = False Then
        If mIbsTime <> mIbsTime_old Then
            Rem check alteration for current IbsCode FromTime
            Set tRecset = dbHmsDatabase.Execute("Select * from ((IndrBlIbsDtl" _
                & " inner join IBedState on IndrBlIbsDtl.IbbsIbsCode = IBedState.IbsCode)" _
                & " inner join IndrBill on IndrBlIbsDtl.IbbsCode = IndrBill.IblCode and IndrBlIbsDtl.IbbsIblICode = IndrBill.IblICode)" _
                & " left join IndrBlHdr on IndrBill.IblCode = IndrBlHdr.IbhCode" _
            ...
            Rem check alteration for previous IbsCode ToTime
            mIbsCode = Val(Mfgrd3.TextMatrix(srow - 1, X3IBSCODE_COL))
            Set tRecset = dbHmsDatabase.Execute("Select * from ((IndrBlIbsDtl" _
                & " inner join IBedState on IndrBlIbsDtl.IbbsIbsCode = IBedState.IbsCode)" _
                & " inner join IndrBill on IndrBlIbsDtl.IbbsCode = IndrBill.IblCode and IndrBlIbsDtl.IbbsIblICode = IndrBill.IblICode)" _
            ...
            With tRecset
            If .EOF = False Then
                If .fields("IbbsToDate") = mIbsDate And .fields("IbbsToTime") > mIbsTime Then
                    If IsNull(.fields("IbhCode")) = False Then
                        ErrorBox "Bed Charges Already Charged in Indoor Bill No." & CStr(.fields("IbhVchNo")) & " Dated " & Dtoc(.fields("IbhDate")) & " " & MinToTime(.fields("IbhTime")) & " !!!"
                    Else
                        ErrorBox "Bed Charges Already Charged in Indoor Bill Dated " & Dtoc(.fields("IblDate")) & " Under Process !!!"
...
If Val(fcmbAraName.BoundText) = 0 Or Val(fcmbStnName.BoundText) = 0 Or Val(fcmbPcgName.BoundText) = 0 Or Val(fcmbAhName.BoundText) = 0 Then
    Set tRecset = dbHmsDatabase.Execute("Select * from PatMast where PttCode = (Select top 1 IpgPttCode from IndrReg order by IpgCode desc)")
    With tRecset
    If .EOF = False Then
        If Val(fcmbAraName.BoundText) = 0 Then
...
If Val(fcmbVtmName.BoundText) = 0 Or Val(fcmbCDctName.BoundText) = 0 Or Val(fcmbRByName.BoundText) = 0 Or Val(fcmbFDigName.BoundText) = 0 Or Val(fcmbBdmName.BoundText) = 0 Then
    Set tRecset = dbHmsDatabase.Execute("Select top 1 * from IndrReg order by IpgCode desc")
    With tRecset
    If .EOF = False Then
        If Val(fcmbVtmName.BoundText) = 0 Then
...
If PatAddEditMode = cFORM_ADDMODE And txtPttSMSNo.Text <> "" Then
    ChkDuplPatByContNo cnn:=dbHmsDatabase, mPttCode:=mPttCode, mContNo:=txtPttSMSNo.Text
End If
...
If optMonthwise.Value = True Or optDatewise.Value = True Or optPatientwise.Value = True Then
    If optMonthwise.Value = True Then
        ...
        clsListStru.AddFields mExpr:="IpgAdvAmt_sum", mTitle:="Advance", mShowItem:=True, mAlign:=7, mWidth:=1500
        Rem clsListStru.AddFields mExpr:="IpgDiscAmt_sum", mTitle:="Discount", mShowItem:=True, mAlign:=7, mWidth:=1500
...
ElseIf optDetailed.Value = True Then
    ...
    ' --------------------------------------
    clsListStru.AddFields mExpr:="IpgAdvAmt", mTitle:="Advance", mAlign:=7, mWidth:=1250, mShowItem:=True
    Rem clsListStru.AddFields mExpr:="IpgDiscAmt", mTitle:="Discount", mAlign:=7, mWidth:=1250, mShowItem:=True
    ' --------------------------------------
    ...
    datRecset.open "Select IndrReg.*,PatMast.*,PatCatgMst.PcgName,AreaMast.AraName,StsnMast.StnName,VTypMast.VtmName" _
     & ",format(IpgDate,'yyyymmdd')+format(IpgCode,'0000000') as tIpgSmryId" _
     & " from (IndrReg inner join (((PatMast" _
     ...
     & " where IpgDate between #" & CStr(ToSysDate(Ctod(dtpFromDate.Text))) & "# and #" & CStr(ToSysDate(Ctod(dtpToDate.Text))) & "#" _
     & IIf(mRemoteAccess = True Or mBedStatusAccess = True, " and IpgCode = " & CStr(mIpgCode) & "", "") _
     & " and IpgCode>0" _
     & IIf(mQryPttCode > 0, " and IpgPttCode=" & CStr(mQryPttCode), "") _
    ...
    Do While .EOF = False
        If optMonthwise.Value = True Or optDatewise.Value = True Or optPatientwise.Value = True Then
            If optMonthwise.Value = True Then
                ...
                Rem Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgDiscAmt_sum")) = ToMyNumFmt(0)  ' ToMyNumFmt(.Fields("IpgDiscAmt_sum"))
        ...
        ElseIf optDetailed.Value = True Then
            ...
            Rem Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("IpgDiscAmt")) = ToMyNumFmt(0)  ' ToMyNumFmt(.Fields("IpgDiscAmt"))
            ' ------------------------------------
...
If PatAddEditMode = cFORM_ADDMODE And txtPttTelNo.Text <> "" Then
    ChkDuplPatByContNo cnn:=dbHmsDatabase, mPttCode:=mPttCode, mContNo:=txtPttTelNo.Text
End If
...
Set tRecset = dbHmsDatabase.Execute("Select * from IndrReg where IpgCode=" & CStr(mOldIpgCode) & " order by IpgDate,IpgVchNo,IpgCode")
With tRecset
If .EOF = False Then
...
Set tRecset = dbHmsDatabase.Execute("Select top 1 * from IndrReg where IpgPttCode=" & CStr(mPttCode) & " order by IpgDate desc,IpgVchNo desc,IpgCode desc")
With tRecset
If .EOF = False Then
...
If fcmbOldIpdByIpdNo.CallFromText_Change = False Then
    ...
    If InterActiveChange(txtVtmName(0)) = True Then
        If fcmbOldIpdByIpdNo.LRecordCount = 0 Then
            fcmbOldIpdByIpdNo.mFiltCond = ""
            fcmbOldIpdByIpdNo.ReInit
        End If
...
If fcmbOldIpdByPttRegNo.CallFromText_Change = False Then
    ...
    If InterActiveChange(mskPttRegNo) = True Then
        If fcmbOldIpdByPttRegNo.LRecordCount = 0 Then
            fcmbOldIpdByPttRegNo.mFiltCond = ""
            fcmbOldIpdByPttRegNo.ReInit
        End If
...
If fcmbOldIpdByPttName.CallFromText_Change = False Then
    ...
    If InterActiveChange(txtPttName) = True Then
        If fcmbOldIpdByPttName.LRecordCount = 0 Then
            fcmbOldIpdByPttName.mFiltCond = ""
            fcmbOldIpdByPttName.ReInit
        End If
