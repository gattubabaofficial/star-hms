...
If mRemoteAccess = True Then
    datRecset.Open "Select * from UserRole where UrlCode=" & CStr(mUrlCode) & " order by UrlName", dbCmpDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.Open "Select * from UserRole order by UrlName", dbCmpDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtUrlName.Text <> mUrlName_old Then
    If ChkIsDuplicate(dbCmpDatabase, "UserRole", "UrlName", txtUrlName.Text, "UrlCode", Val(mskFormBoundField.Text)) = True Then
...
clsListStru.AddFields mExpr:="UrlName", mTitle:="Security Name", mAlign:=1, mWidth:=4000, mShowItem:=True
