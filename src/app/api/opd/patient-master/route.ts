...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let whereClause: any = { PttRecState: 1 };
    
    // For lookup filtering
    if (q) {
      if ('patMast' === 'patMast') whereClause.PttName = { contains: q, mode: 'insensitive' };
    }

    const records = await prisma.patMast.findMany({
      where: whereClause,
      include: { Category: true, Area: true, Station: true },
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const record = await prisma.patMast.create({ data });
