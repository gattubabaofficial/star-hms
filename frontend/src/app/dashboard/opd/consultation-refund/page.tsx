...
export default function OPDConsultationRefundPage() {
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API}/consultation-refunds?from_date=${fromDate}&to_date=${toDate}`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: 0 }}>
      <div className={styles.toolbar} style={{ flexWrap: "wrap", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)" }}>OPD Consultation Refunds</span>
      </div>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={12} style={{ padding: "40px 0", textAlign: "center", color: "var(--text-secondary)" }}>
                      No consultation refund records found.
                    </td>
                  </tr>
                ) : (
                  filtered.map(r => {
