...
Dim clsJRN As clsJournal, clsAH As clsAccountHead, clsIPG As clsHmsSnglTranEntry, clsIRC As clsHmsIpdMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry, clsHOPT As clsHmsIpdOptionsEntry
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    With tRecset
    .open "Select * from IndrRcpt where IrcCode=" & CStr(mIrcCode) & " and IrcICode=" & CStr(mIrcICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    ...
    If GetServDctDtlCount(cnn:=dbHmsDatabase, mTableName:="IndrRcDctDtl", mSdcCodeField:="IrdCode", mSdcCode:=.fields("IrcCode"), mSdcSubCodeField:="IrdIrcCode", mSdcSubCode:=.fields("IrcICode")) > 0 Then
...
If cmdOK.Caption = "&OK" And dtlSdcEditCanceled = False Then
    ...
    RefreshDatabase dbHmsDatabase
    With tRecset
    .open "Select * from IndrRcDctDtl where IrdCode=" & CStr(mIrcCode) & " and IrdIrcCode=" & CStr(mIrdIrcCode) & " and IrdICode=" & CStr(mIrdICode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
clsHOPT.blnIndrRcptOptMast = True
clsHOPT.Init
clsHOPT.GetIndrRcptOptData
...
fcmbIrcVtmName.mListSqlExpr = "Select * from VTypMast where VtmSysCode=" & CStr(cHMS_VTYPE_INDOOR_RCPT) & " order by VtmName"
fcmbIrcVtmName.Init
...
If mRemoteAccess = True Then
    datRecset.open "Select * from IndrHdr inner join PatMast on IndrHdr.IhrPttCode = PatMast.PttCode where IhrCode = " & CStr(mIrcCode) & " order by IhrDate,IhrTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
    dtpFromDate.Text = Dtoc(datRecset.fields("IhrDate"))
    dtpToDate.Text = Dtoc(datRecset.fields("IhrDate"))
    AddSmryParaLayer mClear:=True
    
Else
    datRecset.open "Select * from IndrHdr inner join PatMast on IndrHdr.IhrPttCode=PatMast.PttCode order by IhrDate,IhrTime", dbHmsDatabase, adOpenKeyset, adLockOptimistic
End If
Mfgrd1.FixedCols = 0
Call SmryList
...
If mPttRegNo < 0 Then
    ...
    GoTo EndSub
Else
    If mPttRegNo = 0 And clsHOPT.mIrcPttRegNoNotZero_bln = True Then
    ...
    If mIrcVchNo <> mIrcVchNo_old Or Val(fcmbIrcVtmName.BoundText) <> mIrcVtmCode_old Or Ctod(dtpIrcDate.Text) <> mIrcDate_old Or FormAddEditMode = cFORM_ADDMODE Then
        If ChkIsDuplVTypeNo(dbHmsDatabase, dbAcDatabase, mTableName:="IndrHdr", mChkFieldName:="IhrVchNo", mChkFieldValue:=mIrcVchNo, mKeyFieldName:="IhrCode", mKeyFieldValue:=mIrcCode, mVtmFieldName:="IhrVtmCode", mVtmFieldValue:=Val(fcmbIrcVtmName.BoundText), mFinYrFieldName:="IhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IhrDate", mDateFieldValue:=Ctod(dtpIrcDate.Text)) = True Then
            ...
            If FormAddEditMode = cFORM_ADDMODE Then
                mIrcVchNo = GetNextVTypeNo(dbHmsDatabase, dbAcDatabase, dbComDatabase, mTableName:="IndrHdr", mFieldName:="IhrVchNo", mVtmFieldName:="IhrVtmCode", mVtmFieldValue:=Val(fcmbIrcVtmName.BoundText), mFinYrFieldName:="IhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode, mDateFieldName:="IhrDate", mDateFieldValue:=Ctod(dtpIrcDate.Text), mModuleCode:=mFormUIdCode, mModuleName:=Me.Name, mAcsPermFileName:=mFormIrcAcsPermFileName)
            Else
...
ElseIf mIrcVchNo > 0 Then
    If Index = 0 Then
        ...
        End If
    Else
        If ChkIsExistVTypeNo(dbHmsDatabase, mTableName:="IndrHdr", mChkFieldName:="IhrVchNo", mChkFieldValue:=CStr(mIrcVchNo), mVtmFieldName:="IhrVtmCode", mVtmFieldValue:=Val(fcmbIrcVtmName.BoundText), mFinYrFieldName:="IhrCmpCode", mFinYrFieldValue:=sFinYrCmpCode) = False Then
...
fcmbIrcVtmName.BoundText = CStr(clsHOPT.GetIndrRcptOptBoundCode("IrcVtmCode")): txtIrcVtmName(0).Text = fcmbIrcVtmName.Text
...
With tRecset
.open "Select * from IndrRcpt where IrcCode=" & CStr(mIrcCode) & " order by IrcSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
...
If FormAddEditMode = cFORM_ADDMODE Then
    If .AddNewHdr() = True Then
        ...
        dbHmsDatabase.Execute "Delete from IndrRcDctDtl where IrdIrcCode IN (Select IrcICode from IndrRcpt as irc inner join ServMast as srv on irc.IrcSrvCode=srv.SrvCode where srv.SrvMultiDctIncl=False and irc.IrcCode=" & CStr(mIrcCode) & ") and IrdCode=" & CStr(mIrcCode) & ""
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mIrcVtmCodeOld_lng = .mTrnVtmCode_lng
    clsHOPT.UpdateIndrRcptOld
...
ElseIf optDetailed.Value = True Then
    ...
    datRecset.open "Select IndrHdr.*,PatMast.*,PatCatgMst.PcgName,AreaMast.AraName,StsnMast.StnName,VTypMast.VtmName" _
     & ",format(IhrDate,'yyyymmdd')+format(IhrCode,'0000000') as tIrcSmryId" _
     & " from (IndrHdr inner join (((PatMast" _
     ...
     & " on IndrHdr.IhrPttCode = PatMast.PttCode)" _
...
With tRecset
.open "Select * from IndrHdr where IhrCode=" & CStr(mOldIrcCode) & "", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
With tRecset
.open "Select top 1 * from IndrHdr where IhrPttCode=" & CStr(mPttCode) & " order by IhrDate desc,IhrVchNo desc,IhrCode desc", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
...
If dtlAddEditMode = True And dtlAddMode = False And mDataOk = True And InterActiveChange(cmdOK) = True Then
    ...
    If clsSRV.mSrvMultiDctIncl_bln = False Then
        If GetServDctDtlCount(cnn:=dbHmsDatabase, mTableName:="IndrRcDctDtl", mSdcCodeField:="IrdCode", mSdcCode:=mIrcCode, mSdcSubCodeField:="IrdIrcCode", mSdcSubCode:=Val(mskIrcICode.Text)) > 0 Then
...
With tRecset
.open "Select IndrPymtDtl.*,IndrPymtHdr.IphVtmCode,IndrPymtHdr.IphVchNo from IndrPymtDtl inner join IndrPymtHdr on IndrPymtDtl.IpyCode=IndrPymtHdr.IphCode where IpyIhrCode=" & CStr(mIrcCode) & " order by IpyDate", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select IndrRefdDtl.*,IndrRefdHdr.IfhVtmCode,IndrRefdHdr.IfhVchNo from IndrRefdDtl inner join IndrRefdHdr on IndrRefdDtl.IfdCode=IndrRefdHdr.IfhCode where IfdIhrCode=" & CStr(mIrcCode) & " order by IfdDate", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
...
With tRecset
.open "Select * from IndrRcDctDtl where IrdCode=" & CStr(mIrdCode) & " and IrdIrcCode=" & CStr(mIrdIrcCode) & " order by IrdSno", dbHmsDatabase, adOpenKeyset, adLockOptimistic
If .RecordCount > 0 Then
    Do While .EOF = False
