...
Begin VB.Form frmPatientMast 
   ...
   Begin VB.Frame frFormSmry 
      ...
      Top             =   840
      Width           =   13815
      Begin VB.TextBox txtSearchPttContNo 
         Appearance      =   0  'Flat
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   8.25
...
Dim mSearchPttContNoStartRow As Integer
...
Dim clsAH As clsAccountHead, clsPAT As clsHmsMasterEntry, clsPCG As clsHmsMasterEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
Dim clsHOPT As clsHmsOpdOptionsEntry
Dim fcmbAhName As clsComFlexSearch, fcmbAraName As clsComFlexSearch, fcmbStnName As clsComFlexSearch
Dim fcmbPttName As clsHmsFlexSearch, fcmbPttRegNo As clsHmsFlexSearch, fcmbPcgName As clsHmsFlexSearch
...
If Screen.ActiveControl.Name = txtSearchPttContNo.Name Then
    txtSearchPttContNo_KeyDown vbKeyReturn, 0
...
ElseIf Screen.ActiveControl.Name = txtSearchPttContNo.Name Then
    txtVoid.SetFocus
    txtSearchPttContNo.Text = ""
    Mfgrd1.SetFocus
...
clsHOPT.blnPatMastOptMast = True
clsHOPT.Init
clsHOPT.GetPatMastOptData
...
If mRemoteAccess = True Then
    datRecset.open "Select * from (((PatMast ptt inner join PatCatgMst pcg on ptt.PttPcgCode = pcg.PcgCode) inner join AreaMast ara on ptt.PttAraCode = ara.AraCode) inner join StsnMast stn on ptt.PttStnCode = stn.StnCode) inner join AchdMast ah on ptt.PttAhCode = ah.AhCode where ptt.PttCode = " & CStr(mPttCode) & " order by ptt.PttName", dbComDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from (((PatMast ptt inner join PatCatgMst pcg on ptt.PttPcgCode = pcg.PcgCode) inner join AreaMast ara on ptt.PttAraCode = ara.AraCode) inner join StsnMast stn on ptt.PttStnCode = stn.StnCode) inner join AchdMast ah on ptt.PttAhCode = ah.AhCode order by ptt.PttName", dbComDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
Else
    If mPttRegNo <> mPttRegNo_old Or FormAddEditMode = cFORM_ADDMODE Then
        If FormAddEditMode = cFORM_ADDMODE And mPttRegNo <> mPttRegNo_old And clsHOPT.mPttRegNoAddModeChange_bln = False Then
            AlertBox "Change Prohibited in Add Mode !!!"
            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
            mskPttRegNo.SetFocus
            GoTo EndSub
            
        ElseIf FormAddEditMode = cFORM_EDITMODE And mPttRegNo <> mPttRegNo_old And clsHOPT.mPttRegNoEditModeChange_bln = False Then
            AlertBox "Change Prohibited in Edit Mode !!!"
            mskPttRegNo.Text = ToMyNumFmt(mPttRegNo_old, mDecimals:=0)
            mskPttRegNo.SetFocus
            GoTo EndSub
            
        ElseIf mPttRegNo = 0 And clsHOPT.mPttRegNoNotZero_bln = True Then
            AlertBox "Prohibited Zero Patient Reg.No. !!!"
            If FormAddEditMode = cFORM_ADDMODE Then
                If clsHOPT.mPttRegNoAutoNumber_bln = True Then
        ...
        ElseIf mPttRegNo <> 0 Then
            If ChkIsDuplicate(dbHmsDatabase, "PatMast", "PttRegNo", mPttRegNo, "PttCode", mPttCode) = True Then
...
fcmbAraName.BoundText = CStr(clsHOPT.GetPatMastOptBoundCode("PttAraCode")): txtAraName.Text = fcmbAraName.Text
fcmbStnName.BoundText = CStr(clsHOPT.GetPatMastOptBoundCode("PttStnCode")): txtStnName.Text = fcmbStnName.Text
fcmbPcgName.BoundText = CStr(clsHOPT.GetPatMastOptBoundCode("PttPcgCode")): txtPcgName.Text = fcmbPcgName.Text
fcmbAhName.BoundText = CStr(clsHOPT.GetPatMastOptBoundCode("PttAhCode")): txtAhName.Text = fcmbAhName.Text
...
If .mCode_lng > 0 Then   ' updated successfully
    ...
    clsHOPT.mPttAraCodeOld_lng = clsPAT.mPttAraCode_lng
    clsHOPT.mPttStnCodeOld_lng = clsPAT.mPttStnCode_lng
    clsHOPT.mPttPcgCodeOld_lng = clsPAT.mPttPcgCode_lng
    clsHOPT.mPttAhCodeOld_lng = clsPAT.mPttAhCode_lng
    clsHOPT.UpdatePatMastOld
...
If txtPttSMSNo.Text <> "" Then
    ...
    End If
End If
If txtPttSMSNo.Text <> "" Then  ''' FormAddEditMode = cFORM_ADDMODE And
...
clsListStru.AddFields mExpr:="PttRegDate", mTitle:="Reg.Date", mAlign:=4, mWidth:=1500, mShowItem:=True
clsListStru.AddFields mExpr:="PttRefName", mTitle:="Care of", mAlign:=1, mWidth:=2500, mShowItem:=True
clsListStru.AddFields mExpr:="PttContNo", mTitle:="Contact No.", mAlign:=1, mWidth:=2000, mShowItem:=True
...
If .RecordCount > 0 Then
    Do While .EOF = False
        ...
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PttRegDate")) = Dtoc(.fields("PttRegDate"))
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PttRefName")) = .fields("PttRefName")
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("PttContNo")) = .fields("PttTelNo") & "," & .fields("PttSmsNo")
...
If txtPttTelNo.Text <> "" Then
    ...
    End If
End If
If txtPttTelNo.Text <> "" Then      ''' FormAddEditMode = cFORM_ADDMODE And
...
Private Sub txtSearchPttContNo_GotFocus()
FlashActiveControl txtSearchPttContNo, True
mSearchPttContNoStartRow = Mfgrd1.FixedRows
End Sub

Private Sub txtSearchPttContNo_KeyDown(KeyCode As Integer, Shift As Integer)
If KeyCode = vbKeyReturn And Shift = 0 And Trim(txtSearchPttContNo.Text) <> "" Then
    SearchRawFlex MfgrdX:=Mfgrd1, mColno:=clsListStru.GetItemIndex("PttContNo"), mTextstr:=Trim(txtSearchPttContNo.Text), mDefStartRow:=mSearchPttContNoStartRow, mDefSearchMatchOpt:=cSEARCH_MIDSTR
    mSearchPttContNoStartRow = Mfgrd1.Row + 1
    If mSearchPttContNoStartRow >= Mfgrd1.Rows - 1 Then
        mSearchPttContNoStartRow = Mfgrd1.FixedRows
    End If
End If
...
Private Sub txtSearchPttContNo_LostFocus()
FlashActiveControl txtSearchPttContNo, False
End Sub
