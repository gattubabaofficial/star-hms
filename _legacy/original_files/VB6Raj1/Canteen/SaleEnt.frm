...
Dim clsAH As clsAccountHead, clsSTK As clsStockJrn, clsSAL As clsCtnMultiTranEntry, clsARA As clsComMastEntry, clsSTN As clsComMastEntry, clsVTM As clsComMastEntry
...
Dim fcmbPrtName As clsCtnFlexSearch, fcmbSimName As clsCtnFlexSearch, fcmbSimRefNo As clsCtnFlexSearch
...
If cmdOK.Caption = "&OK" And dtlEditCanceled = False Then
    ...
    .Open "Select * from SaleDtl" _
        ...
        & " order by SalSno", dbCtnDatabase, adOpenKeyset, adLockOptimistic
...
Set clsSAL = New clsCtnMultiTranEntry
Set clsSAL.dbAcDatabase = dbAcDatabase
Set clsSAL.dbStDatabase = dbStDatabase
Set clsSAL.dbCtnDatabase = dbCtnDatabase
...
Set fcmbSimRefNo = New clsCtnFlexSearch
Set fcmbSimRefNo.dbCtnDatabase = dbCtnDatabase
...
With tRecset
.Open "Select top 1 * from SaleHdr where SahVtmCode IN (" & mFormVchTypeList & ") and SahCmpCode = " & CStr(sFinYrCmpCode) & " order by SahCode desc", dbCtnDatabase, adOpenKeyset, adLockOptimistic
If .EOF = False Then
...
If .mTrnCode_lng > 0 Then   ' updated successfully
    ...
    If .mShrCode_lng > 0 Then
        If FormAddEditMode = cFORM_ADDMODE Then
            ...
            dbCtnDatabase.Execute "Update SaleHdr set SahStkCode=" & CStr(mSalStkCode) & " where SahCode=" & CStr(mSalCode)
            dbCtnDatabase.Execute "Update SaleDtl set SalStkCode=" & CStr(mSalStkCode) & " where SalCode=" & CStr(mSalCode)
