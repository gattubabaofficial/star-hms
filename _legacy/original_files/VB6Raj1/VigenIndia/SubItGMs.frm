...
Dim mMigCode As Long
Dim mRemoteAccess As Boolean, mEntrySaved As Boolean, mEntryAborted As Boolean
Dim dbMdsDatabase As New ADODB.Connection, dbMdsDatabase As New ADODB.Connection, datRecset As New ADODB.Recordset
Dim mMigName_old As String
Dim clsMIG As clsMdsMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure

Public Property Let RemoteAccessCode(ByVal mAccessCode As Long)
mRemoteAccess = True
mMigCode = mAccessCode
End Property
...
Public Property Get MigName() As String
MigName = txtMigName.Text
End Property
...
Call SmryList
txtSearch1Text_InterActiveChange txtMigName.Text

''If txtSearch1Text.Text <> txtMigName.Text Then
''    txtSearch1Text.Text = txtMigName.Text
''Else
''    Call txtSearch1Text_Change
''End If
''datRecset.Find "MigName='" & txtMigName.Text & "'", Start:=1
''ShowRecActiveFlexRow datRecset, Mfgrd1
...
datRecset.open "Select * from MedItmGrpMst" _
    & " order by MigName" _
...
If Val(mskFormBoundField.Text) < 1 And FormAddEditMode = cFORM_EDITMODE Then
    ...
    Exit Function
End If
If Trim(txtMigName.Text) = "" Then
    ErrorBox "Invalid Group Name !!!"
    txtMigName.SetFocus
    Exit Function
End If

If txtMigName.Text <> mMigName_old Then
    If ChkIsDuplicate(dbMdsDatabase, "MedItmGrpMst", "MigName", txtMigName.Text, "MigCode", Val(mskFormBoundField.Text)) = True Then
...
Private Sub txtMigDesc_GotFocus()
FlashActiveControl txtMigDesc, True
End Sub

Private Sub txtMigDesc_LostFocus()
FlashActiveControl txtMigDesc, False
End Sub

Private Sub txtMigDesc_Validate(Cancel As Boolean)
txtMigDesc.Text = ToMyWord(txtMigDesc.Text)
If txtMigDesc.Text = "" Then
    txtMigDesc.Text = txtMigName.Text
End If
...
txtMigName.Text = ToMyWord(txtMigName.Text)
If Trim(txtMigName.Text) = "" Then
...
If CanUserAddOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsMIG.BeginMast(0) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
...
clsMIG.Clear
mMigCode = clsMIG.ActiveCode

mskFormBoundField.Text = mMigCode
txtMigName.Text = ""
txtMigDesc.Text = ""

mMigName_old = txtMigName.Text
...
If CanUserEditOpt(mUsrCode:=MyCompany.mUserCode, mMenuOptName:=Me.SecuMenuName) = False Then
    ...
    Exit Sub
End If
If clsMIG.BeginMast(datRecset.fields("MigCode")) = False Then
    Exit Sub
End If
mEntrySaved = False: mEntryAborted = False
FormAddEditMode = cFORM_EDITMODE
With datRecset
mMigCode = .fields("MigCode")

clsMIG.GetData mMigCode

mskFormBoundField.Text = CStr(mMigCode)

txtMigName.Text = .fields("MigName")
txtMigDesc.Text = .fields("MigDesc")
End With
...
.mName_str = txtMigName.Text
.mMigDesc_str = txtMigDesc.Text
If FormAddEditMode = cFORM_ADDMODE Then
    If .AddNew() = True Then
...
mMigCode = datRecset.fields("MigCode")
mRecAbsPos = datRecset.AbsolutePosition

If clsMIG.CanDeleteMast(mMigCode) = True Then
    If clsMIG.BeginMast(mMigCode) = True Then
        clsMIG.DeleteMast mMigCode
        clsMIG.UpdateMast mMigCode
        clsMIG.EndMast mMigCode
...
clsListStru.AddFields mExpr:="MigCode", mAttrib:=cFAT_SYSTEM
clsListStru.AddFields mExpr:="MigName", mTitle:="Group Name", mAlign:=flexAlignLeftCenter, mWidth:=3500, mShowItem:=True
...
If .RecordCount > 0 Then
    Do While .EOF = False
        Mfgrd1.TextMatrix(srow, clsListStru.GetItemIndex("MigCode")) = .fields("MigCode")
...
SearchFlex Mfgrd1, clsListStru.GetItemIndex("MigName"), mText
