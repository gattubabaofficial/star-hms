...
Dim clsRFG As clsVgnMasterEntry, mActiveControl As Object, clsListStru As clsSelectQueryStructure
...
If mRemoteAccess = True Then
    datRecset.open "Select * from RefCatgMst where RfgCode=" & CStr(mRfgCode) & " order by RfgName", dbVgnDatabase, adOpenKeyset, adLockOptimistic
Else
    datRecset.open "Select * from RefCatgMst order by RfgName", dbVgnDatabase, adOpenKeyset, adLockOptimistic
End If
Call SmryList
...
If txtRfgName.Text <> mRfgName_old Then
    If ChkIsDuplicate(dbVgnDatabase, "RefCatgMst", "RfgName", txtRfgName.Text, "RfgCode", Val(mskFormBoundField.Text)) = True Then
